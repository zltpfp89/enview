...
enview.portal.Page.prototype =
{
	...
	init : function( version, isEditing, userId, pageInfo, decorations, node, langKnd, domainId )
	{
		...
		if( pageInfo.targetUrl != null && pageInfo.targetUrl != '' && pageInfo.targetUrl != '?' && pageInfo.useIframe == "true" ) {
			...
			var height = "1000";
			if( pageInfo.pageInfo01 != null && pageInfo.pageInfo01 != '') {
				height = pageInfo.pageInfo01;
			}
			var buff = "";
			buff += "<iframe name='" + "IframePortlet_" + pageInfo.id + "' ";
			buff += "id='IframePortlet_" + pageInfo.id + "' ";
			// iframeautoresizer용 class 추가 
			buff += "class='autoResize' ";
			//--
			...
			// iframe autoresizer 호출
			if( window.iFrameResize) {
				$('.autoResize').iFrameResize({
				});
			}
		}
	},
...
enview.portal.Notice.prototype =
{
	...
	doRetrieve : function(pageId)
	{
		...
		var url = portalPage.getContextPath() + "/notice/activeListForAjax.admin";
		portalPage.m_ajax.send("POST", portalPage.getContextPath() + "/notice/activeListForAjax.admin", param, false, { success: function(data){
			setTimeout( function(){noticeManager.doRetrieveHandler(data)}, 100);
		}});
		/*
        $.ajax({
            url: url,
