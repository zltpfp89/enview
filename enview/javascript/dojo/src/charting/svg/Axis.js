...
if(dojo.render.svg.capable){dojo.extend(dojo.charting.Axis, {renderLines: function(
plotArea, 
plot, 
plane
...
if(plane == "x"){l.setAttribute("y1",area.top);
l.setAttribute("y2",area.bottom);
l.setAttribute("x1",v);
l.setAttribute("x2",v);
}
else if (plane == "y"){l.setAttribute("y1",v);
l.setAttribute("y2",v);
...
if(plane == "x"){l.setAttribute("y1",coord);
...
textSize,
anchor
){function createLabel(label, x, y, textSize, anchor){var text = document.createElementNS(dojo.svg.xmlns.svg, "text");
...
for(var i=0; i<this._labels.length; i++){var v = this.getCoord(this._labels[i].value, plotArea, plot);
if(plane == "x"){g.appendChild(createLabel(this._labels[i].label, v, coord, textSize, anchor));
}
...
if(plane == "x"){line.setAttribute("y1", coord);
...
g.appendChild(text);
}} else {line.setAttribute("x1", coord);
...
if(this.showLabel && this.label){var x = isMax?(coord+(textSize*2)+(textSize/2)):(coord-(textSize*4));
