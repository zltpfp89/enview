...
drawVectorNode:function(){dojo.svg.g.suspend();
...
drawPlotArea:function(){dojo.svg.g.suspend();
...
drawDataGroup:function(){dojo.svg.g.suspend();
...
drawAxes:function(){dojo.svg.g.suspend();
...
destroy:function(){while(this.domNode.childNodes.length>0){this.domNode.removeChild(this.domNode.childNodes.item(0));
}
this.vectorNode=this.plotArea=this.dataGroup=this.axisGroup=null;
},
render:function(){dojo.svg.g.suspend();
if (this.dataGroup){while(this.dataGroup.childNodes.length>0){this.dataGroup.removeChild(this.dataGroup.childNodes.item(0));
}} else {this.init();
}
...
dojo.widget.svg.Chart.Plotter=new function(){var self=this;
var plotters = {};
...
var h=Math.abs(yA-y);
if (parseFloat(series.values[i].value)<chart.properties.axes.x.plotAt){var oy=yA;
yA=y;
y=oy;
}
...
if (i>0){dx=x-self.getX(series.values[i-1].x, chart);
...
plotters["scatter"]=function(series, chart){var r=7;
for (var i=0; i<series.values.length; i++){var x=self.getX(series.values[i].x, chart);
var y=self.getY(series.values[i].value, chart);
