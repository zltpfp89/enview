...
dojo.gfx.Colorspace.prototype.convert = function(col, model_from, model_to){var k = model_from+'_to_'+model_to;
if (this[k]){return this[k](col);
}else{if (this.chainMaps[k]){var cur = model_from;
var models = this.chainMaps[k].concat();
models.push(model_to);
for(var i=0; i<models.length; i++){col = this.convert(col, cur, models[i]);
cur = models[i];
}
return col;
}else{dojo.debug("Can't convert from "+model_from+' to '+model_to);
}}
}
dojo.gfx.Colorspace.prototype.munge = function(keys, args){if (dojo.lang.isArray(args[0])){args = args[0];
}
var out = new Array();
for (var i=0; i<keys.length; i++){out[keys.charAt(i)] = args[i];
}
return out;
}
dojo.gfx.Colorspace.prototype.getWhitePoint = function(){var x = 0;
var y = 0;
var t = 0;
switch(this.stdObserver){case '2' :
switch(this.whitePoint){case 'E'   : x=1/3    ; y=1/3    ; t=5400; break;
...
case 'A'   : x=0.44757; y=0.40745; t=2856; break;
case 'B'   : x=0.34842; y=0.35161; t=4874; break;
case 'C'   : x=0.31006; y=0.31616; t=6774; break;
case '9300': x=0.28480; y=0.29320; t=9300; break;
case 'F2'  : x=0.37207; y=0.37512; t=4200; break;
case 'F7'  : x=0.31285; y=0.32918; t=6500; break;
case 'F11' : x=0.38054; y=0.37691; t=4000; break;
...
case 'A'   : x=0.45117; y=0.40594; t=2856; break;
case 'B'   : x=0.3498 ; y=0.3527 ; t=4874; break;
case 'C'   : x=0.31039; y=0.31905; t=6774; break;
case 'F2'  : x=0.37928; y=0.36723; t=4200; break;
case 'F7'  : x=0.31565; y=0.32951; t=6500; break;
case 'F11' : x=0.38543; y=0.37110; t=4000; break;
default: dojo.debug('White point '+this.whitePoint+" isn't defined for Std. Observer "+this.strObserver);
};
break;
default:
...
dojo.gfx.Colorspace.prototype.getPrimaries = function(){var m = [];
switch(this.RGBWorkingSpace){case 'adobe_rgb_1998'	: m = [2.2, 'D65', 0.6400, 0.3300, 0.297361, 0.2100, 0.7100, 0.627355, 0.1500, 0.0600, 0.075285]; break;
...
if (p.wp != this.whitePoint){var r = this.XYZ_to_xyY( this.chromaticAdaptation( this.xyY_to_XYZ([p.xr, p.yr, p.Yr]), p.wp, this.whitePoint ) );
...
dojo.gfx.Colorspace.prototype.XYZ_to_xyY = function(){var src = this.munge('XYZ', arguments);
var sum = src.X + src.Y + src.Z;
if (sum == 0){var wp = this.getWhitePoint();
var x = wp.x;
var y = wp.y;
}else{var x = src.X / sum;
var y = src.Y / sum;
}
var Y = src.Y;
return [x, y, Y];
}
dojo.gfx.Colorspace.prototype.xyY_to_XYZ = function(){var src = this.munge('xyY', arguments);
if (src.y == 0){var X = 0;
var Y = 0;
var Z = 0;
}else{var X = (src.x * src.Y) / src.y;
var Y = src.Y;
var Z = ((1 - src.x - src.y) * src.Y) / src.y;
}
return [X, Y, Z];
}
dojo.gfx.Colorspace.prototype.RGB_to_XYZ = function(){var src = this.munge('RGB', arguments);
var m = this.getRGB_XYZ_Matrix();
var pr = this.getPrimaries();
if (this.RGBWorkingSpace == 's_rgb'){var r = (src.R > 0.04045) ? Math.pow(((src.R + 0.055) / 1.055), 2.4) : src.R / 12.92;
...
}else{var r = Math.pow(src.R, pr.gamma);
...
dojo.gfx.Colorspace.prototype.XYZ_to_RGB = function(){var src = this.munge('XYZ', arguments);
...
var g = rgb[0][1];
var b = rgb[0][2];
if (this.RGBWorkingSpace == 's_rgb'){var R = (r > 0.0031308) ? (1.055 * Math.pow(r, 1.0/2.4)) - 0.055 : 12.92 * r;
var G = (g > 0.0031308) ? (1.055 * Math.pow(g, 1.0/2.4)) - 0.055 : 12.92 * g;
var B = (b > 0.0031308) ? (1.055 * Math.pow(b, 1.0/2.4)) - 0.055 : 12.92 * b;
}else{var R = Math.pow(r, 1/pr.gamma);
var G = Math.pow(g, 1/pr.gamma);
var B = Math.pow(b, 1/pr.gamma);
}
return [R, G, B];
}
dojo.gfx.Colorspace.prototype.XYZ_to_Lab = function(){var src = this.munge('XYZ', arguments);
...
dojo.gfx.Colorspace.prototype.Lab_to_XYZ = function(){var src = this.munge('Lab', arguments);
var wp = this.getWhitePoint();
...
dojo.gfx.Colorspace.prototype.Lab_to_LCHab = function(){var src = this.munge('Lab', arguments);
var L = src.L;
...
dojo.gfx.Colorspace.prototype.LCHab_to_Lab = function(){var src = this.munge('LCH', arguments);
...
dojo.gfx.Colorspace.prototype.chromaticAdaptation = function(col, src_w, dst_w){col = this.munge('XYZ', [col]);
...
switch(this.chromaticAdaptationAlg){case 'xyz_scaling':
...
dojo.gfx.Colorspace.prototype.getRGB_XYZ_Matrix = function(){var wp = this.getWhitePoint();
var pr = this.getPrimaries();
var Xr = pr.xr / pr.yr;
var Yr = 1;
...
dojo.gfx.Colorspace.prototype.getXYZ_RGB_Matrix = function(){var m = this.getRGB_XYZ_Matrix();
return dojo.math.matrix.inverse(m);
}
dojo.gfx.Colorspace.prototype.XYZ_to_Luv = function(){var src = this.munge('XYZ', arguments);
var wp = this.getWhitePoint();
...
dojo.gfx.Colorspace.prototype.Luv_to_XYZ = function(){var src = this.munge('Luv', arguments);
var wp = this.getWhitePoint();
...
dojo.gfx.Colorspace.prototype.Luv_to_LCHuv = function(){var src = this.munge('Luv', arguments);
var L = src.L;
...
dojo.gfx.Colorspace.prototype.LCHuv_to_Luv = function(){var src = this.munge('LCH', arguments);
var H_rad = src.H * (Math.PI / 180);
var L = src.L;
...
dojo.gfx.Colorspace.colorTemp_to_whitePoint = function(T){if (T < 4000){dojo.debug("Can't find a white point for temperatures under 4000K");
return [0,0];
}
if (T > 25000){dojo.debug("Can't find a white point for temperatures over 25000K");
return [0,0];
}
var T1 = T;
var T2 = T * T;
...
if (T <= 7000){var x = (-4.6070 * ten9 / T3) + (2.9678 * ten6 / T2) + (0.09911 * ten3 / T) + 0.244063;
}else{var x = (-2.0064 * ten9 / T3) + (1.9018 * ten6 / T2) + (0.24748 * ten3 / T) + 0.237040;
}
var y = -3.000 * x * x + 2.870 * x - 0.275;
return [x, y];
}
dojo.gfx.Colorspace.prototype.RGB_to_CMY = function(){var src = this.munge('RGB', arguments);
var C = 1 - src.R;
var M = 1 - src.G;
var Y = 1 - src.B;
return [C, M, Y];
}
dojo.gfx.Colorspace.prototype.CMY_to_RGB = function(){var src = this.munge('CMY', arguments);
var R = 1 - src.C;
var G = 1 - src.M;
var B = 1 - src.Y;
return [R, G, B];
}
dojo.gfx.Colorspace.prototype.RGB_to_CMYK = function(){var src = this.munge('RGB', arguments);
...
dojo.gfx.Colorspace.prototype.CMYK_to_RGB = function(){var src = this.munge('CMYK', arguments);
var R = 1 - Math.min(1, src.C * (1-src.K) + src.K);
var G = 1 - Math.min(1, src.M * (1-src.K) + src.K);
var B = 1 - Math.min(1, src.Y * (1-src.K) + src.K);
return [R, G, B];
}
dojo.gfx.Colorspace.prototype.CMY_to_CMYK = function(){var src = this.munge('CMY', arguments);
...
dojo.gfx.Colorspace.prototype.CMYK_to_CMY = function(){var src = this.munge('CMYK', arguments);
var C = Math.min(1, src.C * (1-src.K) + src.K);
var M = Math.min(1, src.M * (1-src.K) + src.K);
var Y = Math.min(1, src.Y * (1-src.K) + src.K);
return [C, M, Y];
}
dojo.gfx.Colorspace.prototype.RGB_to_HSV = function(){var src = this.munge('RGB', arguments);
...
var S = (V == 0) ? 0 : delta / V;
if (S == 0){H = 0;
}else{if (src.R == V){H = 60 * (src.G - src.B) / delta;
}else{if (src.G == V){H = 120 + 60 * (src.B - src.R) / delta;
}else{if (src.B == V){H = 240 + 60 * (src.R - src.G) / delta;
}}
}
if (H < 0){H += 360;
}}
...
dojo.gfx.Colorspace.prototype.HSV_to_RGB = function(){var src = this.munge('HSV', arguments);
if (src.H == 360){ src.H = 0;}
var r = null;
var g = null;
var b = null;
if (src.S == 0){var R = src.V;
var G = src.V;
var B = src.V;
}else{var hTemp = src.H / 60;
var i = Math.floor(hTemp);
var f = hTemp - i;
var p = src.V * (1 - src.S);
var q = src.V * (1 - (src.S * f));
var t = src.V * (1 - (src.S * (1 - f)));
switch(i){case 0: R = src.V; G = t    ; B = p    ; break;
...
dojo.gfx.Colorspace.prototype.RGB_to_HSL = function(){var src = this.munge('RGB', arguments);
var min = Math.min(src.R, src.G, src.B);
...
if (delta > 0) {if ((max == src.R) && (max != src.G)){H += (src.G - src.B) / delta;
}
if ((max == src.G) && (max != src.B)){H += (2 + (src.B - src.R) / delta);
}
if ((max == src.B) && (max != src.R)){H += (4 + (src.R - src.G) / delta);
}
H *= 60;
}
H = (H == 0) ? 360 : H;
...
dojo.gfx.Colorspace.prototype.HSL_to_RGB = function(){var src = this.munge('HSL', arguments);
while (src.H < 0){ src.H += 360; }
while (src.H >= 360){ src.H -= 360; }
var R = 0;
var G = 0;
var B = 0;
if (src.H < 120){R = (120 - src.H) / 60;
G = src.H / 60;
B = 0;
}else if (src.H < 240){R = 0;
G = (240 - src.H) / 60;
B = (src.H - 120) / 60;
}else{R = (src.H - 240) / 60;
G = 0;
...
if (src.L < 0.5){R = src.L * R;
G = src.L * G;
B = src.L * B;
}else{R = (1 - src.L) * R + 2 * src.L - 1;
