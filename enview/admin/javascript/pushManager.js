...
doPushSend : function ()
	{
		 var pram = "userId=" +  document.getElementById("pushrreciveid").value;
			 pram += "&deviceKey=" + document.getElementById("pushrrecivedeviceKey").value;
			 pram += "&phoneType=" + document.getElementById("pushrrecivedevicephoneType").value;
			 pram += "&pushtitle=" + document.getElementById("pushtitle").value;
			 pram += "&pushcontent=" + document.getElementById("pushcontent").value;
	
			 
			 if(document.getElementById("pushrreciveid").value=="")
				 {
				    alert("수신자를 입력해 주세요");
				 }
			 else if(document.getElementById("pushtitle").value=="")
				 {
				    alert("제목을  입력해 주세요");
				 }
			 else if(document.getElementById("pushcontent").value=="")
			     {
			        alert("내용을 입력해 주세요");
			     }
				 ...
				 {
			       this.m_ajax.send("POST", this.m_contextPath + "/pushManager/PushSend.admin", pram, false, {success: function(data) { aUserManager.doSelectHandler(data); }});
				 }
	},
	...
	doRetrieve : function () 
	{
		...
		this.m_ajax.send("POST", this.m_contextPath + "/pushManager/listForAjax.admin", param, false, {success: function(data) { aUserManager.doRetrieveHandler(data); }});
*/	},
	doRetrieveHandler : function( resultObject )
	{
		...
		this.m_utility.setListData(
				...
				new Array('userId','deviceKey','phoneType'),
	...
	{
		//alert("호출");
		var param = "userId=" + document.getElementById('search').value;
		//alert(param);
		...
		if(param=="")
			{
			  this.m_ajax.send("POST", this.m_contextPath + "/pushManager/list.admin", param, false, {success: function(data) { aUserManager.doSelectHandler(data); }});
			}
		else
			{
			this.m_ajax.send("POST", this.m_contextPath + "/pushManager/findId.admin", param, false, {success: function(data) { aUserManager.doSearchHandler(data); }});
			//alert("됩니까?");
			}
	...
	doDefaultSelect : function ()
	{
		...
		this.m_ajax.send("POST", this.m_contextPath + "/pushManager/detailForAjax.admin", param, false, {success: function(data) { aUserManager.doSelectHandler(data); }});
	},
doSelect : function (obj)
	{
		...
		if(document.getElementById('UserManager[' + rowSeq + '].checkRow').checked)
			{
			  document.getElementById('UserManager[' + rowSeq + '].checkRow').checked = false;
			  aUserManager.doRemove();
			}
		else
			{
		...
		//document.getElementById("pushrreciveid").value = document.getElementById('UserManager[' + rowSeq + '].userId').value;
		//document.getElementById("pushrrecivedeviceKey").value = document.getElementById('UserManager[' + rowSeq + '].deviceKey').value;
	...
	{
		alert("전송되었습니다");
		/*document.getElementById("pushrreciveid").value = "";
		document.getElementById("pushrrecivedeviceKey").value="";*/
		document.getElementById("pushtitle").value="";
		document.getElementById("pushcontent").value="";
		/*document.getElementById("phoneType").value="";*/
	},
	
	doPushClear : function()
	{
	/*	document.getElementById("pushrreciveid").value = "";
		document.getElementById("pushrrecivedeviceKey").value="";*/
		document.getElementById("pushtitle").value="";
...
doRemove : function () // 메세지 전송에 내용 추가
{	
	...
	if( rowCnts == "" )
	{
		 document.getElementById("pushrreciveid").value = "";
		 document.getElementById("pushrrecivedeviceKey").value = "";
		 document.getElementById("pushrrecivedevicephoneType").value = "";
	}
        ...
        {     	
        	...
        	deviceKeyList += document.getElementById('UserManager[' + rowCntArray[i] + '].deviceKey').value;
        	phoneTypeList += document.getElementById('UserManager[' + rowCntArray[i] + '].phoneType').value;
        }   
	        document.getElementById("pushrreciveid").value = userIdList;
	        document.getElementById("pushrrecivedeviceKey").value = deviceKeyList;
	   	    document.getElementById("pushrrecivedevicephoneType").value = phoneTypeList;
		}
},
all : function () 
{
  ...
  doRetrieve : function () 
	{
		...
		this.m_ajax.send("POST", this.m_contextPath + "/pushManager/listForAjax.admin", param, false, {success: function(data) { aUserChooser.doRetrieveHandler(data); }});
	},
	doRetrieveHandler : function( resultObject ) {
		this.m_utility.setListData(
			"UserChooser",
