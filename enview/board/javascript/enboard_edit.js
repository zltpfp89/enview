...
//			    uploadUrl:  "/dhtmlx/vault/handler/enboard_handler.jsp?boardId=" + ebEdit.boardId + "&subId=sub01",           // html4/html5 upload url
			    uploadUrl:  "/board/fileMngr?cmd=upload&boardId=" + ebEdit.boardId + "&subId=sub01",           // html4/html5 upload url
			    autoStart : false,
			    buttonUpload : false,
			    buttonClear : true
			});
			...
			ebEdit.vaultUploader.attachEvent("onUploadFail", function(file, extra){
				alert(extra.info);
			});
	...
	// 임시저장
	tempSave : function (rtnURI) {
		if( document.setTransfer.delFlag.value== '' ||   document.setTransfer.delFlag.value=='T') {
			document.setTransfer.delFlag.value= 'T';
			return ebEdit.save( rtnURI);
		}  else {
			alert('Unable to tempSave! delFlag=' + document.setTransfer.delFlag.value);
		}
	},
