...
nhn.husky.SE_PhotoUpload = jindo.$Class({
    ...
    $ON_APPLY_PHOTO : function(){
		...
		formPhoto.action = parent.ebUtil.getContextPath() + '/board/fileMngr?cmd=upload&boardId=' + window.parent.ebEdit.boardRid;
		formPhoto.size.value = window.parent.ebEdit.thumbSize;
		formPhoto.boardId.value = window.parent.ebEdit.boardRid;
		formPhoto.cmntId.value = window.parent.document.getElementById('cmntId')?'&cmntId='+window.parent.document.getElementById('cmntId').value:'';
		formPhoto.cafe.value = window.parent.cfGate?'cafe=gate':'';
