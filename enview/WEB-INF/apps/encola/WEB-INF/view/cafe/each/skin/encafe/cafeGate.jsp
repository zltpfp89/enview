...
<%--
<c:if test="${eachForm.view == 'eachHome' || eachForm.view == 'eachHome,eachHome'}">
 --%>
	...
	<div id="div_cafe_gate" class="cafe_gate CF0201_bgColor">
		<c:if test="${!isEditMode}">
			...
			<div class="boardNameBorder CF0501_nmBrdrStyle CF0501_nmBrdrColor">
				<div class="title">
					<a class="boardName CF0501_nmFontColor CF0501_nmFontNm"><util:message key="cf.title.cafeNotice"/></a>
				</div>
				<span class="boardPipe">|</span>
				<span class="boardDesc"><util:message key="cf.info.advertise"/></span>
			</div>
			<div class="boardList">
		...
		<c:if test="${isEditMode}">
			<div id="smarteditor"></div>
			<div id="gateHtmlDiv" align=center style="overflow:hidden">
				<div id="editorCntt"></div>
				<textarea id=gateHtmlEditor style="display:none;height:150px"><c:out value="${gateHtml}"/></textarea>
				...
				<div class="apply_box">
					<util:message key="cf.info.htmlEdit"/>
					<a class="save_btn" onclick="javascript:cfGate.applyGateHtml(); return false;" style="padding-left:0px;">
						...
						<span class="btn_txt bt03 w03" style="padding-right: 4px;"><util:message key="cf.title.save"/></span>
					</a>
				</div>
			</div>
		</c:if>
