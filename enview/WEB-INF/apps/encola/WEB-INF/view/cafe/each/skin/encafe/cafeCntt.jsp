...
<c:if test="${eachForm.cmd == 'selectMenu'}">
	<c:if test="${!empty menuVO}">
		<iframe class="title_content" id="cafeBoardFrame" name="cafeBoardFrame" frameborder="0" scrolling=no height="600"
			<% if (cnttType.startsWith("2") || cnttType.startsWith("8")) { %>
				src="<c:out value="${empty menuVO.cnttUrl ? 'about:blank' : menuVO.cnttUrl}"/><c:out value="${paramMark}"/>cmntId=<c:out value="${cmntVO.cmntId}"/>&userId=<c:out value="${mmbrVO.userId}"/>&mmbrGrd=<c:out value="${mmbrVO.mmbrGrd}"/>"
			<% } else if (cnttType.startsWith("1") || cnttType.startsWith("9")) { %>
