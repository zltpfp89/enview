...
<div class="p_21">
	...
	<div id="Portlet_tab_normal" class="Portlet_tab_normal m1">
		<ul>
			...
			<li class="m${boardIdSeq.count}"><a href="#" <c:if test="${boardIdSeq.first}">class='first'</c:if>><span><util:message key='ev.prop.notices.${boardId}'/></span></a>
				<ul>
				<c:if test="${boardIdSeq.first}">
				<c:forEach items="${bltnList}" var="bltn" varStatus="status" end="5">
					<li><a href="/portal${domainPath}${menuPath}${boardId}.page?bltnNo=${bltn.bltnNo}&cmd=READ" class="title ellipsis">${bltn.bltnSubj}</a><img src="${themePath}/images/main/new_icon.gif" alt="새로운 글" /><span class="date">${bltn.regDatim}</span></li>
				</c:forEach>
				</c:if>
				</ul>
	...
	<div id="Portlet_tab_normal" class="Portlet_tab_normal m1">
		<ul>
			<c:forEach items="${boardIdList}" var="boardId" varStatus="boardIdSeq">
			<li class="m${boardIdSeq.count}"><a href="#" <c:if test="${boardIdSeq.first}">class='first'</c:if> url='<portlet:resourceURL id="${boardId}"/>'><span><util:message key='ev.prop.notices.${boardId}'/></span></a>
				<ul>
