...
<html>
	...
	<c:if test="${windowId == null}" var="result">
		...
		<script type='text/javascript' src="<%=request.getContextPath()%>/hancer/javascript/rss/default/service.js"></script>
	...
	<body>
		<div id="RssManager_myEntryListPage">
			<form id="listForm" name="listForm" method="post" action="<%=request.getContextPath()%>/hancer/rss/entryList.hanc">
				<div class="webpanel">
					<div id="RssManager"> 
						<div>
							<table width="99%">
								<tr>
									<td align="right">
										<select id="categoryList" name="categoryList" onchange="javascript:enEntry.selectCategory();">
											<option value="-1">카테고리</option>
											<c:forEach items="${categoryList }" var="cate">
												<option value="<c:out value="${cate.id }"/>"<c:if test="${form.categoryId == cate.id }">selected="selected"</c:if>><c:out value="${cate.name }"/></option>
											</c:forEach>
										</select>
						...
						<div class="webgridpanel">				
							<table id="grid-table" width="100%" border="0" cellspacing="0" cellpadding="0">
								...
								<tbody id="RssManager_ListBody">
									<c:if test="${total == 0}">
									<tr>
										<td class="webgridbody" align="center" colspan="100">뉴스 목록이 없습니다.</td>
									</tr>
									</c:if>
