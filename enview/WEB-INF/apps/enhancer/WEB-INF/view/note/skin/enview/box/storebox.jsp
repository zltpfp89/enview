...
<form id="receivebox" name="receivebox" method="post" onsubmit="return false;">
	<div id="popupMenu" class="popupMenu">
		<div id="block" class="block" onclick="javascript:enNote.blockUser('Store',<c:out value="${notePage.currentPage}"/>);"><util:message key="hn.note.label.block"/></div>
		<div id="unblock" class="block" onclick="javascript:enNote.unblockUser('Store',<c:out value="${notePage.currentPage}"/>);"><util:message key="hn.note.label.unblock"/></div>		
	</div>
	<div class="searchArea">
		<h1 class="title"><util:message key="hn.note.label.storeBox"/></h1>
		<p class="search">
			...
			<input type="text" class="txt_110" id="searchValue" value="<c:out value="${form.searchValue }"/>" onkeyup="javascript:enNote.searchStoreNotes(event);"/>
			<a href="javascript:enNote.searchStoreNotes(event);" class="btn_type02"><span><util:message key="hn.note.label.search"/></span></a>
	...
	<!-- 버튼영역 -->
	<div class="btnArea">
		<a href="javascript:enNote.deleteStoreNotes();" class="btn_type01" title="delete"><span><util:message key="hn.note.label.delete"/></span></a>
		<span class="pageBtn">
	...
	<div class="tableArea">
		...
		<caption>메세지리스트입니다.</caption>
			...
			<c:if test="${empty form.noteFormList}">
				<tr><td colspan="5"><util:message key="hn.note.label.hasNotStoreNote"/></td></tr>
			</c:if>
			<c:forEach items="${form.noteFormList}" var="note" varStatus="idx">
				<tr class="dataRow">
					...
					<td id="send_<c:out value="${note.noteId }"/>">
						<a class="senderLink column <c:if test="${note.isBlocked == 1}">blocked</c:if>" href="javascript:enNote.popupMenu(<c:out value="${note.noteId }"/>,'<c:out value="${note.senderName}"/>','<c:out value="${note.senderId}"/>',<c:out value="${ note.isBlocked }"/>);"><c:out value="${note.senderName}"/>(<c:out value="${note.senderId}"/>)</a>
					</td>
					<td>
						<c:out value="${note.deleRcvrName}"/>(<c:out value="${note.deleRcvrId}"/>)<c:if test="${note.receiverAmount > 1 }">외 <c:out value="${note.receiverAmount-1 }"/>명</c:if>
					</td>
					<td class="left">
						...
						<c:if test="${( note.fileIds != null && note.fileIds !='' )  }"><img src="<%=request.getContextPath()%>/hancer/images/note/enview/icon_file.png"/></c:if>
					</td>						
