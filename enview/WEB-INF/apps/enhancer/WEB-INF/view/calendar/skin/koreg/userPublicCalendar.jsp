...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="${cPath }/koreg/css/style.css" />
	...
	<body class="calbody">
		<input type="hidden" id="isOnlyUserPublic" value="Y"/>
		<input type="hidden" id="isWithUserPublic" value="Y"/>
		...
		<div id="wrap" class="wrap">
			<div id="leftArea" style="position: absolute;">
...
<%-- 						<label id="publicCalendarLabel" class="publicCalendarLabel" for="publicCalendarCaratButton"><util:message key='hn.title.calendar.public'/></label> --%>
					...
					<div class="publicCalendarList calendarList" id="publicCalendarList">
						<c:forEach items="${publicCalList}" var="cal" varStatus="status">
...
<%-- 								<label class="calendarName"><c:out value="${cal.name}"/></label> --%>
								<c:out value="${cal.name}"/><span class="calendarPopupButton triangleButton publicCalendarRButton ui-icon ui-icon-triangle-1-s" calendarId="<c:out value="${cal.calendarId}"/>"></span>
							...
							<input type="hidden" id="imwun" value="${status.last }" />
						</c:forEach>
					</div>
				</div>
			</div>
