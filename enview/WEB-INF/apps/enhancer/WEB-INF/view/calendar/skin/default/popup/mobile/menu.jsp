...
<div id="calendarPopup" class="calendarPopup">
	<c:if test="${form.cmd != 'my' }">
		<ul class=popupMenus>
			...
			<c:if test="${form.isPublic != 'Y' && user.isOwner == 'Y' }">
			...
			<c:if test="${form.isPublic == 'Y' }">
				<li tabindex="0" class="popupRow"onclick="enCal.iCalendar('<c:out value="${form.calendarId}"/>');"><util:message key='hn.title.calendar.ical'/></li>
			</c:if>
		</ul>
		<c:if test="${ ( form.isPublic == 'Y' && ((user != null && user.isOwner == 'Y') || fn:indexOf(user.permissions, 'D') > -1) ) || form.isPublic != 'Y' }">
