...
// 			}
		...
		<style>
/* 		body,h1,h2,h3,h4,input,button,select{font:600 16px NanumGothic,나눔고딕,ngwoff,ngeot,Helvetica,sans-serif;} */
	...
	<div id="print">
		...
		<div id="wrap" class="wrap">
			<div id="leftArea" class="" style="position: absolute;">
				<div class="myCalendarArea">
					<div id="myCalendar" class="caratToggle calendarInfo">
						<span id="myCalendarCaratButton" class="caratButton ui-icon <c:if test="${empty calList }"> ui-icon-carat-1-e</c:if><c:if test="${not empty calList }"> ui-icon-carat-1-s</c:if>"></span>
<%-- 						<label id="myCalendarLabel" class="myCalendarLabel" for="myCalendarCaratButton"><util:message key='hn.title.calendar.my'/></label> --%>
						<util:message key='hn.title.calendar.my'/>
						<span id="myCalPop" class="triangleButton myCalendarRButton ui-icon ui-icon-triangle-1-s"></span>
					</div>
					<div class="myCalendarList calendarList" id="myCalendarList">
...
<%-- 								<label class="calendarName"><c:out value="${cal.name}"/></label> --%>
								<c:out value="${cal.name}"/>
								<span class="calendarPopupButton triangleButton myCalendarRButton ui-icon ui-icon-triangle-1-s" calendarId="<c:out value="${cal.calendarId}"/>"></span>
								...
								<input type="hidden" id="<c:out value="${cal.calendarId}"/>_isUserPublic" value="<c:out value="${cal.isPublic}"/>"/>
								<input type="hidden" name="calPermissions" id="<c:out value="${cal.calendarId}"/>_permissions" value="CRUDA"/>
								...
								<input type="hidden" id="<c:out value="${cal.calendarId}"/>_isUsers" value="Y"/>
							</div>
						</c:forEach>
					</div>
					<div class="myCalendarList calendarList" id="jointCalendarList">
						<c:forEach items="${jointCalList}" var="cal">
							<div class="calendarInfo" id="<c:out value="${cal.calendarId}"/>_CalToggle" title="<c:out value="${cal.name}"/>">
								...
								<input type="hidden" id="<c:out value="${cal.calendarId}"/>_bgColor" name="bgColor" value="<c:out value="${cal.bgColor}"/>"/>
<%-- 								<label class="calendarName"><c:out value="${cal.name}"/></label> --%>
								<c:out value="${cal.name}"/><span class="calendarPopupButton triangleButton myCalendarRButton ui-icon ui-icon-triangle-1-s" calendarId="<c:out value="${cal.calendarId}"/>"></span>
								<input type="hidden" name="calendarId" value="<c:out value="${cal.calendarId}"/>"/>
								<input type="hidden" name="<c:out value="${cal.calendarId}"/>_selected" value="1"/>
								<input type="hidden" id="<c:out value="${cal.calendarId}"/>_isPublic" value="<c:out value="${cal.isPublic}"/>"/>
								<input type="hidden" id="<c:out value="${cal.calendarId}"/>_isUserPublic" value="<c:out value="${cal.isPublic}"/>"/>
...
<%-- 						<label id="publicCalendarLabel" class="publicCalendarLabel" for="publicCalendarCaratButton"><util:message key='hn.title.calendar.public'/></label> --%>
						<util:message key='hn.title.calendar.public'/>
					</div>
...
<%-- 								<label class="calendarName"><c:out value="${cal.name}"/></label> --%>
								<c:out value="${cal.name}"/>
								<span class="calendarPopupButton triangleButton publicCalendarRButton ui-icon ui-icon-triangle-1-s" calendarId="<c:out value="${cal.calendarId}"/>"></span>
								<input type="hidden" name="calendarId" value="<c:out value="${cal.calendarId}"/>"/>
								<input type="hidden" name="<c:out value="${cal.calendarId}"/>_selected" value="1"/>
		 ...
		 <div class="btnArea">
             <div class="rightArea" style="margin-right: 210px; margin-top: 10px;">
             	<a href="#;" id="printBtn" class="btn_gray">
             		<span>인 쇄</span>
             	</a>
             </div>
         </div>
