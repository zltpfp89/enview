				...
				<tr class="listTr CF0802_extraBrdrColor<c:if test="${status.last && status.index == rowCnt-1}"> last</c:if>">
					<td class="listTd CF0501_subjFontColor CF0501_subjFontSize CF0501_subjFontNm">
						<c:if test="${list.bltnLev != '1'}">
							...
							<img align="middle" src="<%=request.getContextPath()%>/board/images/board/skin/cafe.norm/imgRe.gif"/>
						</c:if>
						...
						<c:if test="${boardVO.ttlNewYn == 'Y'}">
							<c:if test="${list.recentBltn == 'Y'}"><img align="middle" src="<%=request.getContextPath()%>/board/images/board/skin/cafe.norm/imgNew.gif"/></c:if>
						</c:if>          
					</td>
