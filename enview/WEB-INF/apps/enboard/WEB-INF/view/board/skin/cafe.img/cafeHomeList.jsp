	...
	<c:if test="${brdType == 'IMG2'}">
		...
		<li class="img2_box<c:if test="${!status.first && status.count%lastCol == 0}">_last</c:if>">
			<dl class="img2_data">				
				...
				<dd class="img_subject txt_over_el CF0501_subjFontColor"><a onclick="cfCntt.readBulletin('<c:out value="${list.boardId}"/>','<c:out value="${list.bltnNo}"/>', <c:out value="${list.readable}"/>)"><c:out value="${list.bltnSubj}"/></a></dd>
	...
	<c:if test="${brdType == 'IMG3'}">
		...
		<li class="img3_box<c:if test="${!status.first && status.count%lastCol == 0}">_last</c:if> count_<c:out value="${status.count}"/>">
			<dl class="img3_data">				
				...
				<dd class="img3_subject txt_over_el CF0501_subjFontColor"><a onclick="cfCntt.readBulletin('<c:out value="${list.boardId}"/>','<c:out value="${list.bltnNo}"/>', <c:out value="${list.readable}"/>)"><c:out value="${list.bltnSubj}"/></a></dd>
