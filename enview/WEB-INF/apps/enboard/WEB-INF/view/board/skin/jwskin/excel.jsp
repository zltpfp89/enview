...
<%
	...
	// 기초 데이터 준비
	String[] fldNms  = {"boardRow","bltnSubj","userNick","regDatim","bltnCateId","bltnReadCnt","bltnReplyCnt","bltnMemoCnt","bltnCntt"};
	String[] fldTtls = {"번호","제목","작성자","작성일","게시물카테고리","조회수","답글수","댓글수","본문"};
	String[] fldYns  = new String[9];

	for (int i=0; i<fldNms.length; i++) {
		fldYns[i] = request.getParameter(fldNms[i]);
	}
	...
	for(int i=0; i<bltnList.size(); i++) {
		...
		if (extnFldLen > 0) {
			...
			for (int j=0; j<extnFldNms.length; j++) {
				if ("Y".equals (extnFldYns[j])) {
					...
					cell = row.createCell (cnum++);
					cell.setCellValue (""+beVO.getClass().getDeclaredMethod (nmMethod, clazz).invoke (beVO, clazz));
				}
			}
		}
		for (int j=0; j<fldNms.length; j++) {
			...
			if ("Y".equals (fldYns[j]) && ((!("bltnCntt".equals (fldNms[j]))) || ("bltnCntt".equals (fldNms[j]) && bltnVO.getReadable()))) {
				nmMethod = "get" + fldNms[j].substring(0,1).toUpperCase() + fldNms[j].substring(1);
				cell = row.createCell (cnum++);
				cell.setCellValue (""+bltnVO.getClass().getDeclaredMethod (nmMethod, clazz).invoke (bltnVO, clazz));
			}
		}
	}
