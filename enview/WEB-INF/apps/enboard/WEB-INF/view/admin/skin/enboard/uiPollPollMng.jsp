				...
				<form name="apmPollEditForm" onsubmit="return false">
					...
					<div class="btnArea"> 
						<div class="rightArea">
							...
							<a id="poll_save" href="javascript:aPM.getPollMngr().doSave('save')" class="btn_B"><span><util:message key="ev.title.save"/></span></a>
							<a id="poll_del" href="javascript:aPM.getPollMngr().doSave('del')" class="btn_B"><span><util:message key="ev.title.remove"/></span></a>
						</div>
					</div>
					<!-- btnArea//-->
					<c:if test="${!empty boardVO}">
							...
							<div class="board">
								<table id="grid-table" cellpadding="0" cellspacing="0" summary="게시판" class="table_board"> 
									...
									<tr>
										...
										<%--&nbsp;&nbsp;&nbsp;&nbsp;(<b>복사</b>: 동일설문을 다수의 카테고리에 배속. <b><util:message key="ev.title.remove"/></b>: 설문을 해당 카테고리에서만 제거.)--%>
										</td>
									</tr>
									<tr>
										...
										<%--설문에서 카테고리간 '복사'기능이 필요없는 것 같아 막아버림. '복사'가 없으면 '삭제'도 의미없음.2010.07.23.KWShin--%>
											...
											<%--span class="btn_pack medium"><a href="javascript:aPM.getPollMngr().doCateBoardMove('DELETE')"><util:message key="ev.title.remove"/></a></span--%>
										</td>
										<td class="L">
