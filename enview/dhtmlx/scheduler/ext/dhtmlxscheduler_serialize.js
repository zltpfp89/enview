...
for(var r=n[i],s=[],d=0;d<a.length;d++)t=a[d][1]?a[d][1](r[a[d][0]]):r[a[d][0]],s.push(' "'+a[d][0]+'": '+this._serialize_json_value(t));e.push("{"+s.join(",")+"}")}return"["+e.join(",\n")+"]"},e.toICal=function(t){var a="BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//dhtmlXScheduler//NONSGML v2.2//EN\nDESCRIPTION:",n="END:VCALENDAR",i=e.date.date_to_str("%Y%m%dT%H%i%s"),r=e.date.date_to_str("%Y%m%d"),s=[],d=this._get_serializable_data();for(var o in d){var _=d[o];s.push("BEGIN:VEVENT"),_._timed&&(_.start_date.getHours()||_.start_date.getMinutes())?s.push("DTSTART:"+i(_.start_date)):s.push("DTSTART:"+r(_.start_date)),
_._timed&&(_.end_date.getHours()||_.end_date.getMinutes())?s.push("DTEND:"+i(_.end_date)):s.push("DTEND:"+r(_.end_date)),s.push("SUMMARY:"+_.text),s.push("END:VEVENT")}return a+(t||"")+"\n"+s.join("\n")+"\n"+n}});
