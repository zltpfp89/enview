...
common.file.prototype = {	
	...
	edit_init : function() {
		...
		if( commonFile.smarteditor) {
			...
			if( commonFile.limitCount > 0 && document.setUpload) {	//파일첨부가 가능한 상태이면...
				skinUrl= contextPath + "/board/smarteditor/SmartEditor2Skin.html";	//사진업로드 가능한 스마트 에디터 스킨
			}
	...
	setForm : function () {
		...
		if(document.getElementById("selectFileName")){
			...
			if(fn_commonMSieversion() > 6 && fn_commonMSieversion() < 10 ){
			    if(inputFile.value != "") {
