	...
	ConvertToHtml : function(A) {
		if (FCKConfig.FullPage) {
			...
			if (!FCKRegexLib.HtmlOpener.test(A))
				A = '<html dir="' + FCKConfig.ContentLangDirection + '">' + A
						+ '</html>';
			if (!FCKRegexLib.HeadOpener.test(A))
				A = A.replace(FCKRegexLib.HtmlOpener,
						'$&<head><title></title></head>');
			return A;
		} else {
			var B = FCKConfig.DocType + '<html dir="'
					+ FCKConfig.ContentLangDirection + '"';
			if (FCKBrowserInfo.IsIE && FCKConfig.DocType.length > 0
					&& !FCKRegexLib.Html4DocType.test(FCKConfig.DocType))
				B += ' style="overflow-y: scroll"';
			B += '><head><title></title></head><body'
					+ FCKConfig.GetBodyAttributes() + '>' + A
					+ '</body></html>';
			return B;
		}
	},
...
var FCK = {
	...
	StartEditor : function() {
		this.TempBaseTag = FCKConfig.BaseHref.length > 0 ? '<base href="'
	...
	FixBody : function() {
		...
		while (D) {
			...
			case 1:
				...
				if (!FCKListsLib.BlockElements[G] && G != 'li'
						&& !D.getAttribute('_fckfakelement')
						&& D.getAttribute('_moz_dirty') == null)
					F = true;
				break;
			case 3:
	...
	GetData : function(A) {
		...
		var D = FCK.DataProcessor.ConvertToDataFormat(C ? B.documentElement
				: B.body, !C, FCKConfig.IgnoreEmptyParagraphValue, A);
	...
	ProtectTags : function(A) {
		...
		if (FCKBrowserInfo.IsIE)
			B += B.length > 0 ? '|ABBR|XML|EMBED|OBJECT'
	...
	SetData : function(A, B) {
		...
		if (FCKBrowserInfo.IsIE && FCK.EditorDocument) {
			FCK.EditorDocument.detachEvent("onselectionchange",
	...
	GetNamedCommandState : function(A) {
		try {
			if (FCKBrowserInfo.IsSafari && FCK.EditorWindow
					&& A.IEquals('Paste'))
				return 0;
	...
	Preview : function() {
		...
		if (FCK_IS_CUSTOM_DOMAIN && FCKBrowserInfo.IsIE) {
			...
			E = 'javascript:void( (function(){document.open() ;document.domain="'
					+ document.domain
					+ '" ;document.write( window.opener._FCKHtmlToLoad );document.close() ;window.opener._FCKHtmlToLoad = null ;})() )';
		}
		;
		var F = window
				.open(
	...
	InsertElement : function(A) {
		...
		if (FCKListsLib.BlockElements[B] != null) {
			...
			;
			C.InsertNode(A);
			var D = FCKDomTools.GetNextSourceElement(A, false, null, [ 'hr',
					'br', 'param', 'img', 'area', 'input' ], true);
			if (!D && FCKConfig.EnterMode != 'br') {
				D = this.EditorDocument.body.appendChild(this.EditorDocument
						.createElement(FCKConfig.EnterMode));
	...
	_KeyDownListener : function(A) {
		...
		if (FCK.EditorWindow) {
			if (!FCK._IsFunctionKey(A.keyCode) && !(A.ctrlKey || A.metaKey)
					&& !(A.keyCode == 46))
				FCK._KeyDownUndo();
		}
		;
...
function _FCK_PaddingNodeListener() {
	...
	if (!FCKBrowserInfo.IsIE && FCKDomTools.PaddingNode) {
		...
		if (A && A.rangeCount == 1) {
			...
			if (B.collapsed && B.startContainer == FCK.EditorDocument.body
					&& B.startOffset == 0) {
		...
		if (C && C.nodeName.IEquals('body')) {
			if (FCK.EditorDocument.body.childNodes.length == 1
				...
				if (FCKSelection
						._GetSelectionDocument(FCK.EditorDocument.selection) != FCK.EditorDocument)
					return;
				var B = FCK.EditorDocument.body.createTextRange();
				var F = false;
				if (!D.childNodes.firstChild) {
					D.appendChild(FCKTools.GetElementDocument(D)
							.createTextNode('\ufeff'));
					F = true;
				}
				;
...
function _FCK_EditingArea_OnLoad() {
	...
	FCK.MouseDownFlag = false;
	FCKTools.AddEventListener(FCK.EditorDocument, 'mousemove',
			_FCK_MouseEventsListener);
	FCKTools.AddEventListener(FCK.EditorDocument, 'mousedown',
			_FCK_MouseEventsListener);
	...
	FCK.EnterKeyHandler = new FCKEnterKey(FCK.EditorWindow,
			FCKConfig.EnterMode, FCKConfig.ShiftEnterMode, FCKConfig.TabSpaces);
...
var FCKTempBin = {
	...
	ToHtml : function() {
		for ( var i = 0; i < this.Elements.length; i++) {
			this.Elements[i] = '<div>&nbsp;' + this.Elements[i].outerHTML
					+ '</div>';
	...
	ToElements : function() {
		...
		for ( var i = 0; i < this.Elements.length; i++) {
			if (this.Elements[i].isHtml) {
				...
				this.Elements[i] = A.firstChild
						.removeChild(A.firstChild.lastChild);
			}
		}
	}
};
...
FCK.InitializeBehaviors = function() {
	...
	this._ExecCheckCaret = function(evt) {
		...
		var E = function() {
			...
			if (!(I == 'a'
					|| (!FCKBrowserInfo.IsOpera && String(H.parentNode.contentEditable) == 'false') || (!(FCKListsLib.BlockElements[I] || FCKListsLib.NonEmptyBlockElements[I]) && B == 35)))
				return;
			...
			if (J) {
				...
				while (H.parentNode
						&& H.parentNode != FCK.EditorDocument.body
						&& H.parentNode != FCK.EditorDocument.documentElement
						&& H == H.parentNode.lastChild
						&& (!FCKListsLib.BlockElements[H.parentNode.tagName
								.toLowerCase()] && !FCKListsLib.NonEmptyBlockElements[H.parentNode.tagName
								.toLowerCase()]))
					H = H.parentNode;
				if (FCKListsLib.BlockElements[I]
						|| FCKListsLib.EmptyElements[I]
						|| H == FCK.EditorDocument.body) {
					...
					while (K) {
						...
						if (FCKListsLib.BlockElements[L]
								|| FCKListsLib.EmptyElements[L]
								|| FCKListsLib.NonEmptyBlockElements[L])
							break;
						K = K.nextSibling;
					}
					;
	...
	this.EditorDocument.addEventListener('dblclick', this._DblClickListener,
			true);
	this.EditorDocument.addEventListener('keydown', this._KeyDownListener,
			false);
	if (FCKBrowserInfo.IsGecko) {
		...
		this.EditorDocument.addEventListener('mousedown',
				function(ev) {
					var N = ev.srcElement;
					if (N.nodeName.IEquals('IMG', 'HR', 'INPUT', 'TEXTAREA',
							'SELECT')) {
	...
	if (FCKBrowserInfo.IsGecko || FCKBrowserInfo.IsOpera) {
		this.EditorDocument.addEventListener('keypress', this._ExecCheckCaret,
				false);
		this.EditorDocument.addEventListener('click', this._ExecCheckCaret,
				false);
	}
	;
...
FCK.CreateLink = function(A, B) {
	...
	if (A.length > 0) {
		...
		var E = this.EditorDocument.evaluate("//a[@href='" + D + "']",
...
if (document.location.protocol == 'file:') {
	...
	FCKConfig.BasePath = sFullProtocol
			+ FCKConfig.BasePath.substring(0, FCKConfig.BasePath
					.lastIndexOf('/') + 1);
} else
	FCKConfig.BasePath = document.location.protocol
			+ '//'
			+ document.location.host
			+ document.location.pathname.substring(0,
...
var FCKDomTools = {
	...
	EnforcePaddingNode : function(A, B) {
		...
		try {
			if (A.body.lastChild
					&& (A.body.lastChild.nodeType != 1 || A.body.lastChild.tagName
		...
		this.PaddingNode = C;
		if (A.body.childNodes.length == 1
				&& A.body.firstChild.nodeType == 1
				&& A.body.firstChild.tagName.toLowerCase() == 'br'
				&& (A.body.firstChild.getAttribute('_moz_dirty') != null || A.body.firstChild
	...
	CheckAndRemovePaddingNode : function(A, B, C) {
		...
		if (!D)
			return;
		try {
			if (D.parentNode != A.body
					|| D.tagName.toLowerCase() != B
					|| (D.childNodes.length > 1)
					|| (D.firstChild && D.firstChild.nodeValue != '\xa0' && String(
							D.firstChild.tagName).toLowerCase() != 'br')) {
	...
	GetNodeAddress : function(A, B) {
		...
		while (A && A != FCKTools.GetElementDocument(A).documentElement) {
			...
			for ( var i = 0; i < D.childNodes.length; i++) {
				...
				if (B === true && F.nodeType == 3 && F.previousSibling
						&& F.previousSibling.nodeType == 3)
					continue;
				E++;
				if (D.childNodes[i] == A)
					break;
	...
	GetNodeFromAddress : function(A, B, C) {
		...
		for ( var i = 0; i < B.length; i++) {
			...
			for ( var j = 0; j < D.childNodes.length; j++) {
				...
				if (C === true && G.nodeType == 3 && G.previousSibling
						&& G.previousSibling.nodeType == 3)
					continue;
				F++;
				if (F == E) {
					D = G;
			...
			if (J.indent == H) {
				...
				if (J.grandparent.nodeName.IEquals([ 'ul', 'ol' ]))
					...
					if (FCKConfig.EnterMode.IEquals([ 'div', 'p' ])
							&& !J.grandparent.nodeName.IEquals('td'))
				...
				for ( var i = 0; i < J.contents.length; i++)
					I.appendChild(J.contents[i].cloneNode(true));
				if (I.nodeType == 11) {
					if (I.lastChild && I.lastChild.getAttribute
							&& I.lastChild.getAttribute('type') == '_moz')
				...
				if (I.nodeName.IEquals(FCKConfig.EnterMode) && I.firstChild) {
					this.TrimNode(I);
					if (FCKListsLib.BlockBoundaries[I.firstChild.nodeName
							.toLowerCase()]) {
	...
	GetNextSibling : function(A, B) {
		A = A.nextSibling;
		while (A && !B && A.nodeType != 1
				&& (A.nodeType != 3 || A.nodeValue.length == 0))
			A = A.nextSibling;
		return A;
	},
	...
	CheckIsEmptyElement : function(A, B) {
		...
		while (C) {
			if (C.nodeType == 1) {
				if (D
						|| !FCKListsLib.InlineNonEmptyElements[C.nodeName
								.toLowerCase()])
					return false;
				if (!B || B(C) === true)
					D = C;
	...
	SetOpacity : function(A, B) {
		if (FCKBrowserInfo.IsIE) {
			B = Math.round(B * 100);
			A.style.filter = (B > 100 ? ''
					: 'progid:DXImageTransform.Microsoft.Alpha(opacity=' + B
							+ ')');
		} else
			A.style.opacity = B;
	},
	...
	GetPositionedAncestor : function(A) {
		...
		while (B != FCKTools.GetElementDocument(B).documentElement) {
			if (this.GetCurrentElementStyle(B, 'position') != 'static')
				return B;
			if (B == FCKTools.GetElementDocument(B).documentElement
					&& currentWindow != w)
	...
	ScrollIntoView : function(A, B) {
		...
		if (B === false) {
			E += A.offsetHeight || 0;
			E += parseInt(this.GetCurrentElementStyle(A, 'marginBottom') || 0,
					10) || 0;
		}
		;
...
FCKTools.FixCssUrls = function(A, B) {
	if (!A || A.length == 0)
		return B;
	return B.replace(/url\s*\(([\s'"]*)(.*?)([\s"']*)\)/g, function(match,
			opener, path, closer) {
		if (/^\/|^\w?:/.test(path))
			return match;
		else
...
FCKTools.GetStyleHtml = (function() {
	...
	var C = function(cssFileUrl, markTemp) {
		if (cssFileUrl.length == 0)
			return '';
		var B = markTemp ? ' _fcktemp="true"' : '';
		return '<link href="' + cssFileUrl
				+ '" type="text/css" rel="stylesheet" ' + B + '/>';
	};
...
FCKTools.IsStrictMode = function(A) {
	return ('CSS1Compat' == (A.compatMode || (FCKBrowserInfo.IsSafari ? 'CSS1Compat'
			: null)));
};
...
FCKTools.GetDocumentPosition = function(w, A) {
	...
	while (B
			&& !(D == w && (B == w.document.body || B == w.document.documentElement))) {
	...
	if (FCKDomTools.GetCurrentElementStyle(w.document.body, 'position') != 'static'
			|| (FCKBrowserInfo.IsIE && FCKDomTools.GetPositionedAncestor(A) == null)) {
...
FCKTools.GetVoidUrl = function() {
	if (FCK_IS_CUSTOM_DOMAIN)
		return "javascript: void( function(){document.open();document.write('<html><head><title></title></head><body></body></html>');document.domain = '"
				+ FCK_RUNTIME_DOMAIN + "';document.close();}() ) ;";
	if (FCKBrowserInfo.IsIE) {
...
FCKImagePreloader.prototype = {
	...
	Start : function() {
		...
		for ( var i = 0; i < A.length; i++) {
			...
			FCKTools.AddEventListenerEx(B, 'load', _FCKImagePreloader_OnImage,
					this);
			FCKTools.AddEventListenerEx(B, 'error', _FCKImagePreloader_OnImage,
					this);
			B.src = A[i];
...
var FCKLanguageManager = FCK.Language = {
	...
	GetActiveLanguage : function() {
		...
		;
		if (window.parent.enLangKnd)
			this.DefaultLanguage = window.parent.enLangKnd;
...
FCKXHtml.GetXHTML = function(A, B, C) {
	FCKDomTools.CheckAndRemovePaddingNode(FCKTools.GetElementDocument(A),
			FCKConfig.EnterMode);
	...
	if (FCKConfig.DocType.length > 0
			&& FCKRegexLib.HtmlDocType.test(FCKConfig.DocType))
...
FCKXHtml._AppendNode = function(A, B) {
	...
	switch (B.nodeType) {
	case 1:
		if (FCKBrowserInfo.IsGecko && B.tagName.toLowerCase() == 'br'
				&& B.parentNode.tagName.toLowerCase() == 'pre') {
			var C = '\r';
		...
		if (FCKBrowserInfo.IsGecko
				&& (B.hasAttribute('_moz_editor_bogus_node') || B
						.getAttribute('type') == '_moz')) {
			if (B.nextSibling)
				return false;
			else {
		...
		if (F)
			...
			E = this._AppendChildNodes(E, B,
					Boolean(FCKListsLib.NonEmptyBlockElements[D]));
		if (!E)
			return false;
		A.appendChild(E);
		break;
...
FCKUndo._CheckIsBookmarksEqual = function(A, B) {
	...
	if (FCKBrowserInfo.IsIE) {
		...
		return this._CompareCursors(A.Start, B.Start) == 0
				&& this._CompareCursors(A.End, B.End) == 0;
	}
};
FCKUndo.SaveUndoStep = function() {
	...
	if (this.CurrentIndex > 0
			&& A == this.SavedData[this.CurrentIndex][0]
			&& this._CheckIsBookmarksEqual(B,
					this.SavedData[this.CurrentIndex][1]))
		return;
	else if (this.CurrentIndex == 0 && this.SavedData.length
			&& A == this.SavedData[0][0]) {
...
FCKEditingArea.prototype.Start = function(A, B) {
	...
	if (this.Mode == 0) {
		if (FCK_IS_CUSTOM_DOMAIN)
			A = '<script>document.domain="' + FCK_RUNTIME_DOMAIN
					+ '";</script>' + A;
		if (FCKBrowserInfo.IsIE)
			A = A
			...
			if (E && F) {
				var G = A.substr(E[1].length, A.length - E[1].length
						- F[1].length);
				A = E[1] + '&nbsp;' + F[1];
				if (FCKBrowserInfo.IsGecko
						&& (G.length == 0 || FCKRegexLib.EmptyParagraph.test(G)))
					G = '<br type="_moz">';
				this._BodyHTML = G;
			} else
				this._BodyHTML = A;
		...
		if (FCK_IS_CUSTOM_DOMAIN && FCKBrowserInfo.IsIE) {
			...
			H.src = 'javascript:void( (function(){document.open() ;document.domain="'
					+ document.domain
					+ '" ;document.write( window.parent._FCKHtmlToLoad );document.close() ;window.parent._FCKHtmlToLoad = null ;})() )';
...
FCKEditingArea.prototype.MakeEditable = function() {
	...
	if (FCKBrowserInfo.IsIE) {
		...
		try {
			...
			FCKTools.AddEventListener(this.Window.frameElement,
					'DOMAttrModified',
...
function FCKEditingArea_Document_AttributeNodeModified(A) {
	...
	B._timer = FCKTools.SetTimeout(FCKEditingArea_MakeEditableByMutation, 1000,
			B);
};
...
FCKEditingArea.prototype._FocusIE = function() {
	...
	if (B.childNodes.length > 0
			|| !(FCKListsLib.BlockElements[C] || FCKListsLib.NonEmptyBlockElements[C])) {
		return;
	}
	;
...
FCKStyle.prototype = {
		...
		if (A.CheckIsCollapsed()) {
			...
			for ( var i = 0; i < I.Elements.length; i++) {
				...
				if (this.CheckElementRemovable(O)) {
							...
							for ( var Q in E) {
								if (FCKDomTools.HasAttribute(O, Q)) {
									...
									case 'style':
										...
										break;
									case 'class':
										if (FCKDomTools.GetAttributeValue(O, Q) != this
												.GetFinalAttributeValue(Q))
											continue;
									default:
					...
					J.push(O);
				L = L
						&& ((K && !FCKDomTools.GetNextSibling(O)) || (!K && !FCKDomTools
								.GetPreviousSibling(O)));
					...
					for ( var Q in E) {
						if (FCKDomTools.HasAttribute(Z, Q)) {
							...
							case 'style':
								...
								break;
							case 'class':
								if (FCKDomTools.GetAttributeValue(Z, Q) != this
										.GetFinalAttributeValue(Q))
									continue;
							default:
	...
	CheckElementRemovable : function(A, B) {
		...
		if (C == this.Element) {
			...
			for ( var F in D) {
				if (F == '_length')
					continue;
				if (this._CompareAttributeValues(F, FCKDomTools
						.GetAttributeValue(A, F), (this
						.GetFinalAttributeValue(F) || ''))) {
					E = true;
					if (!B)
						break;
				} else {
		...
		if (G) {
			...
			for ( var i = 0; i < D.length; i++) {
				...
				if (FCKDomTools.HasAttribute(A, H)) {
					var I = D[i][1];
					if (I == null
							|| (typeof I == 'string' && FCKDomTools
									.GetAttributeValue(A, H) == I)
							|| I.test(FCKDomTools.GetAttributeValue(A, H)))
						return true;
				}
			}
		}
			...
			for ( var F in B) {
				if (FCKDomTools.HasAttribute(E, F)) {
					...
					case 'style':
						...
						break;
					case 'class':
						if (FCKDomTools.GetAttributeValue(E, F) != this
								.GetFinalAttributeValue(F))
							continue;
					default:
		...
		if (C) {
			for ( var i = 0; i < C.length; i++) {
				...
				if (FCKDomTools.HasAttribute(A, D)) {
					var E = C[i][1];
					if (E == null
							|| (E.test && E.test(FCKDomTools.GetAttributeValue(
									A, D)))
							|| (typeof E == 'string' && FCKDomTools
									.GetAttributeValue(A, D) == E))
	...
	GetFinalAttributeValue : function(A) {
		...
		if (B && this._Variables)
			B = B.Replace(FCKRegexLib.StyleVariableAttName,
					this._GetVariableReplace, this);
		return B;
	},
	GetFinalStyleValue : function() {
		var A = this._GetStyleText();
		if (A.length > 0 && this._Variables) {
			A = A.Replace(FCKRegexLib.StyleVariableAttName,
					this._GetVariableReplace, this);
	...
	_CheckAndSplitPre : function(A) {
		...
		while (C) {
			var D = C.nextSibling;
			if (D && D.nextSibling && C.nodeName.IEquals('br')
					&& D.nodeName.IEquals('br')) {
				FCKDomTools.RemoveNode(C);
				C = D.nextSibling;
				FCKDomTools.RemoveNode(D);
				B = FCKDomTools.InsertAfterNode(B || A, FCKDomTools
						.CloneElement(A));
				continue;
			}
			;
	...
	_ApplyInlineStyle : function(A, B, C) {
		...
		while (M) {
			...
			if (!P || G[P]) {
				if ((FCK.DTD[M.parentNode.nodeName.toLowerCase()] || FCK.DTD.span)[F]
						|| !FCK.DTD[F]) {
					if (!A.CheckHasRange())
						A.SetStart(M, 3);
					if (O != 1 || M.childNodes.length == 0) {
						...
						;
						A.SetEnd(Q, 4);
						if (Q == Q.parentNode.lastChild
								&& !G[Q.parentNode.nodeName.toLowerCase()])
							N = true;
					} else {
						A.SetEnd(M, 3);
					}
	...
	_FixBookmarkStart : function(A) {
		...
		while ((B = A.nextSibling)) {
			if (B.nodeType == 1
					&& FCKListsLib.InlineNonEmptyElements[B.nodeName
							.toLowerCase()]) {
				if (!B.firstChild)
	...
	_CreateElementAttribsForComparison : function(A) {
		...
		for ( var i = 0; i < A.attributes.length; i++) {
			...
			if (D.specified) {
				B[D.nodeName.toLowerCase()] = FCKDomTools.GetAttributeValue(A,
						D).toLowerCase();
				C++;
			}
		}
...
var FCKStyles = FCK.Styles = {
	...
	_GetRemoveFormatTagsRegex : function() {
		var A = new RegExp('^(?:'
				+ FCKConfig.RemoveFormatTags.replace(/,/g, '|') + ')$', 'i');
	...
	RemoveAll : function() {
		...
		while (I) {
			...
			if (E.test(I.nodeName))
				...
				FCKDomTools
						.RemoveAttributes(I, FCKConfig.RemoveAttributesArray);
			I = J;
		}
		;
		A.SelectBookmark(B);
	...
	GetStyles : function() {
		...
		if (!A) {
			A = this._GetStyles = FCKTools.Merge(this._LoadStylesCore(), this
					._LoadStylesCustom(), this._LoadStylesXml());
		}
		;
		return A;
	},
...
FCKElementPath._CheckHasBlock = function(A) {
	...
	for ( var i = 0, count = B.length; i < count; i++) {
		var C = B[i];
		if (C.nodeType == 1
				&& FCKListsLib.BlockElements[C.nodeName.toLowerCase()])
			return true;
	}
	;
	return false;
...
FCKDomRange.prototype = {
	...
	CheckStartOfBlock : function() {
		...
		while (F) {
			switch (F.nodeType) {
			case 1:
				if (!FCKListsLib.InlineChildReqElements[F.nodeName
						.toLowerCase()])
					return A.IsStartOfBlock = false;
				break;
			case 3:
	...
	CreateBookmark : function(A) {
		var B = {
			StartId : (new Date()).valueOf() + Math.floor(Math.random() * 1000)
					+ 'S',
			EndId : (new Date()).valueOf() + Math.floor(Math.random() * 1000)
					+ 'E'
		};
	...
	CreateBookmark2 : function() {
		...
		;
		if (D.nodeType == 1 && D.childNodes[A.Start[0]]
				&& D.childNodes[A.Start[0]].nodeType == 3) {
		...
		;
		if (E.nodeType == 1 && E.childNodes[A.End[0]]
				&& E.childNodes[A.End[0]].nodeType == 3) {
	...
	MoveToBookmark2 : function(A) {
		var B = FCKDomTools.GetNodeFromAddress(this.Window.document, A.Start
				.slice(0, -1), true);
		var C = FCKDomTools.GetNodeFromAddress(this.Window.document, A.End
				.slice(0, -1), true);
		this.Release(true);
	...
	Expand : function(A) {
		...
		case 'inline_elements':
			if (this._Range.startOffset == 0) {
				...
				if (B) {
					while (FCKListsLib.InlineNonEmptyElements[B.nodeName
							.toLowerCase()]) {
			...
			if ((B.nodeType == 3 && C >= B.nodeValue.length)
					|| (B.nodeType == 1 && C >= B.childNodes.length)
					|| (B.nodeType != 1 && B.nodeType != 3)) {
				if (B.nodeType != 1)
		...
		case 'list_contents':
			...
			if (this.StartBlock && FCKConfig.EnterMode != 'br'
				...
				while (B
						&& (B.nodeType != 1 || (B != this.StartBlockLimit && !D[B.nodeName
								.toLowerCase()]))) {
					this._Range.setStartBefore(B);
			...
			if (this.EndBlock && FCKConfig.EnterMode != 'br'
					&& A == 'block_contents'
					&& this.EndBlock.nodeName.toLowerCase() != 'li')
...
FCKDomRangeIterator.prototype = {
	GetNextParagraph : function() {
		...
		var D;
		var E;
		var F = this.ForceBrBreak ? FCKListsLib.ListBoundaries
				: FCKListsLib.BlockBoundaries;
		if (!this._LastNode) {
		...
		while (H) {
			...
			if (!K) {
				...
				if (F[M] && (!FCKBrowserInfo.IsIE || H.scopeName == 'HTML')) {
					...
					if (B) {
						B.SetEnd(H, 3, true);
						if (M != 'br')
							this._NextNode = FCKDomTools.GetNextSourceNode(H,
									true, null, I)
									|| H;
					}
					;
			...
			if ((J || C) && B) {
				...
				if (B.StartNode == B.EndNode
						&& B.StartNode.parentNode == B.StartBlockLimit
						&& B.StartNode.getAttribute
						&& B.StartNode.getAttribute('_fck_bookmark'))
					B = null;
				else
					break;
			}
		...
		if (!A) {
			...
			;
			A = B.StartBlock;
			if (!A && !this.EnforceRealBlocks
					&& B.StartBlockLimit.nodeName.IEquals('DIV', 'TH', 'TD')
					&& B.CheckStartOfBlock() && B.CheckEndOfBlock()) {
				A = B.StartBlockLimit;
			} else if (!A
					|| (this.EnforceRealBlocks && A.nodeName.toLowerCase() == 'li')) {
				A = this.Range.Window.document
						.createElement(FCKConfig.EnterMode == 'p' ? 'p' : 'div');
				...
				this._NextNode = A == I ? null : FCKDomTools.GetNextSourceNode(
						B.EndNode, true, null, I);
				return A;
			}
		}
		;
		...
		;
		if (!this._NextNode)
			this._NextNode = (C || A == I) ? null : FCKDomTools
					.GetNextSourceNode(A, true, null, I);
		return A;
	}
};
...
var FCKEnterKey = function(A, B, C, D) {
	...
	E
			.SetKeystrokes([ [ 13, 'Enter' ], [ SHIFT + 13, 'ShiftEnter' ],
					[ 8, 'Backspace' ], [ CTRL + 8, 'CtrlBackspace' ],
					[ 46, 'Delete' ] ]);
	this.TabText = '';
...
FCKEnterKey.prototype.DoBackspace = function() {
	...
	B.MoveToSelection();
	if (FCKBrowserInfo.IsIE
			&& this._CheckIsAllContentsIncluded(B, this.Window.document.body)) {
	...
	if (!C) {
		if (FCKBrowserInfo.IsIE
				&& this.Window.document.selection.type.toLowerCase() == "control") {
	...
	if (B.StartBlockLimit == B.EndBlockLimit && H && I) {
		...
		if (B.CheckStartOfBlock()) {
			var K = B.StartBlock;
			var L = FCKDomTools.GetPreviousSourceElement(K, true, [ 'BODY',
...
FCKEnterKey.prototype.DoCtrlBackspace = function() {
	...
	A.MoveToSelection();
	if (FCKBrowserInfo.IsIE
			&& this._CheckIsAllContentsIncluded(A, this.Window.document.body)) {
...
FCKEnterKey.prototype.DoDelete = function() {
	...
	if (B.CheckIsCollapsed() && B.CheckEndOfBlock(FCKBrowserInfo.IsGeckoLike)) {
		...
		var E = FCKDomTools.GetNextSourceElement(C, true,
				[ B.StartBlockLimit.nodeName ], [ 'UL', 'OL', 'TR' ], true);
		if (D) {
...
FCKEnterKey.prototype._ExecuteEnterBlock = function(A, B) {
	...
	if (D) {
		...
		;
		if (!G && !H) {
			if (F.nodeName.IEquals('li') && F.firstChild
					&& F.firstChild.nodeName.IEquals([ 'ul', 'ol' ]))
				F.insertBefore(FCKTools.GetElementDocument(F).createTextNode(
						'\xa0'), F.firstChild);
			if (F)
			...
			if (K) {
				for ( var i = 0, len = K.Elements.length; i < len; i++) {
					...
					if (L == K.Block || L == K.BlockLimit)
						break;
					if (FCKListsLib.InlineChildReqElements[L.nodeName
							.toLowerCase()]) {
...
FCKEnterKey.prototype._ExecuteEnterBr = function(A) {
	...
	if (B.StartBlockLimit == B.EndBlockLimit) {
		...
		if (!F && D && (/^H[1-6]$/).test(E)) {
			...
			G = E.IEquals('pre');
			if (G)
				H = this.Window.document
						.createTextNode(FCKBrowserInfo.IsIE ? '\r' : '\n');
			else
				H = this.Window.document.createElement('br');
			B.InsertNode(H);
			if (FCKBrowserInfo.IsGecko)
				FCKDomTools.InsertAfterNode(H, this.Window.document
						.createTextNode(''));
...
FCKEnterKey.prototype._CheckIsAllContentsIncluded = function(A, B) {
	...
	if (A.EndContainer == B || A.EndContainer == B.lastChild) {
		var E = A.EndContainer.nodeType == 3 ? A.EndContainer.length
...
var FCKEmbedAndObjectProcessor = (function() {
	...
	var B = function(el) {
		...
		var D;
		var E = D = FCKDocumentProcessor_CreateFakeImage('FCK__UnknownObject',
				C);
	...
	return FCKTools.Merge(FCKDocumentProcessor.AppendNew(),
			{
				...
				RefreshView : function(placeHolder, original) {
					if (original.getAttribute('width') > 0)
						placeHolder.style.width = FCKTools
								.ConvertHtmlSizeToStyle(original
										.getAttribute('width'));
					if (original.getAttribute('height') > 0)
						placeHolder.style.height = FCKTools
								.ConvertHtmlSizeToStyle(original
										.getAttribute('height'));
				},
...
FCKEmbedAndObjectProcessor
		.AddCustomHandler(function(A, B) {
			if (!(A.nodeName.IEquals('embed') && (A.type == 'application/x-shockwave-flash' || /\.swf($|#|\?)/i
					.test(A.src))))
				return;
...
if (FCKBrowserInfo.IsSafari) {
	FCKDocumentProcessor.AppendNew().ProcessDocument = function(A) {
		var B = A.getElementsByClassName ? A
				.getElementsByClassName('Apple-style-span')
				: Array.prototype.filter.call(A.getElementsByTagName('span'),
						function(item) {
...
FCKSelection.GetType = function() {
	...
	if (B && B.rangeCount == 1) {
		...
		if (C.startContainer == C.endContainer
				...
				&& C.startContainer.nodeType == 1
				&& FCKListsLib.StyleObjectElements[C.startContainer.childNodes[C.startOffset].nodeName
						.toLowerCase()]) {
			A = 'Control';
		}
	}
...
FCKSelection.GetSelectedElement = function() {
	...
	if (!A || A.rangeCount < 1)
		return null;
	var B = A.getRangeAt(0);
	if (B.startContainer != B.endContainer || B.startContainer.nodeType != 1
			|| B.startOffset != B.endOffset - 1)
		return null;
...
FCKXml.prototype = {
	LoadUrl : function(A) {
		...
		if (C.status == 200 || C.status == 304
				|| (C.status == 0 && C.readyState == 4)) {
			B = C.responseXML;
			if (!B)
				B = (new DOMParser()).parseFromString(C.responseText,
						'text/xml');
		} else
			B = null;
		if (B) {
		...
		if (!B || !B.firstChild) {
			this.Error = true;
			if (window.confirm('Error loading "' + A + '" (HTTP Status: '
					+ C.status
	...
	SelectNodes : function(A, B) {
		if (this.Error)
			return [];
		var C = [];
		var D = this.DOMDocument.evaluate(A, B ? B : this.DOMDocument,
				this.DOMDocument
	...
	SelectSingleNode : function(A, B) {
		if (this.Error)
			return null;
		var C = this.DOMDocument.evaluate(A, B ? B : this.DOMDocument,
				this.DOMDocument
...
FCKDialogCommand.prototype.Execute = function() {
	FCKDialog.OpenDialog('FCKDialog_' + this.Name, this.Title, this.Url,
			this.Width, this.Height, this.CustomValue, null, this.Resizable);
};
		...
		if (!/:\/\//.test(B)) {
			...
			if (/^\//.test(B)) {
				try {
					...
					C = D.document.location.protocol + '://'
							+ D.parent.document.location.host;
				}
			}
			;
			B = C + B;
	...
	Execute : function() {
		if (FCK.Selection.GetType() == 'Control') {
			...
			if (oAnchor.href.length != 0) {
				...
				if (FCKBrowserInfo.IsIE)
					oAnchor.className = oAnchor.className.replace(
...
FCKTextColorCommand.prototype.SetColor = function(A) {
	FCKUndo.SaveUndoStep();
	var B = FCKStyles.GetStyle('_FCK_'
...
function FCKTextColorCommand_MoreOnClick(A, B) {
	...
	FCKDialog.OpenDialog('FCKDialog_Color', FCKLang.DlgColorTitle,
			'dialog/fck_colorselector.html', 410, 320, FCKTools.Bind(B,
					B.SetColor));
};
FCKTextColorCommand.prototype._CreatePanelBody = function(A, B) {
	function CreateSelectionDiv() {
		...
		FCKTools.AddEventListenerEx(C, 'mouseover',
				FCKTextColorCommand_OnMouseOver);
		FCKTools.AddEventListenerEx(C, 'mouseout',
	...
	FCKTools.AddEventListenerEx(C, 'click', FCKTextColorCommand_AutoOnClick,
			this);
	if (!FCKBrowserInfo.IsIE)
		C.style.width = '96%';
	...
	while (H < G.length) {
		...
		for ( var i = 0; i < 8; i++, H++) {
			...
			if (H >= G.length)
				...
				FCKTools.AddEventListenerEx(C, 'click',
						FCKTextColorCommand_OnClick, [ this, L ]);
		}
	}
	;
	if (FCKConfig.EnableMoreFontColors) {
		...
		FCKTools.AddEventListenerEx(C, 'click',
				FCKTextColorCommand_MoreOnClick, this);
	}
	;
	if (!FCKBrowserInfo.IsIE)
		C.style.width = '96%';
...
FCKListCommand.prototype = {
		...
		while (J) {
			while ((I = H.GetNextParagraph())) {
				...
				for ( var i = L.Elements.length - 1; i >= 0; i--) {
					...
					if (P.nodeName.IEquals([ 'ol', 'ul' ])) {
						...
						if (Q)
							...
							F.push(Q);
							FCKDomTools.SetElementMarker(G, P,
									'_FCK_ListGroupObject', Q);
						}
						;
						N = true;
						break;
				...
				if (R._FCK_ListGroupObject)
					...
					FCKDomTools.SetElementMarker(G, R, '_FCK_ListGroupObject',
							Q);
					F.push(Q);
				}
			}
	...
	_RemoveList : function(A, B) {
		...
		if (A.root.nextSibling == null
				|| A.root.nextSibling.nodeName.IEquals('br')) {
...
var FCKJustifyCommand = function(A) {
	...
	this.IsDefaultAlign = (A == 'left' && B == 'ltr')
			|| (A == 'right' && B == 'rtl');
...
FCKJustifyCommand.prototype = {
	Execute : function() {
		...
		while ((F = E.GetNextParagraph())) {
			...
			if (D) {
				var G = F.className.replace(FCKJustifyCommand
						._GetClassNameRegex(), '');
				if (B == 0) {
					if (G.length > 0)
						G += ' ';
	...
	GetState : function() {
		...
		if (FCKBrowserInfo.IsIE)
			...
			C = FCK.EditorWindow.getComputedStyle(B, '').getPropertyValue(
					'text-align');
...
var FCKIndentCommand = function(A, B) {
	this.Name = A;
	this.Offset = B;
	this.IndentCSSProperty = FCKConfig.ContentLangDirection.IEquals('ltr') ? 'marginLeft'
			: 'marginRight';
};
FCKIndentCommand._InitIndentModeParameters = function() {
	if (FCKConfig.IndentClasses && FCKConfig.IndentClasses.length > 0) {
		...
		this._ClassNameRegex = new RegExp('(?:^|\\s+)('
				+ FCKConfig.IndentClasses.join('|') + ')(?=$|\\s)');
	} else
...
FCKIndentCommand.prototype = {
	Execute : function() {
		...
		A.MoveToSelection();
		var B = A.CreateBookmark();
		var C = FCKDomTools.GetCommonParentNode(
				A.StartNode || A.StartContainer, A.EndNode || A.EndContainer, [
						'ul', 'ol' ]);
		if (C)
	...
	GetState : function() {
		...
		if (FCKIndentCommand._UseIndentClasses) {
			...
			;
			if ((this.Name == 'outdent' && H == 0)
					|| (this.Name == 'indent' && H == FCKConfig.IndentClasses.length))
				return -1;
			return 0;
		} else {
		...
		while ((E = B.GetNextParagraph())) {
			...
			if (FCKIndentCommand._UseIndentClasses) {
				...
				if (G < 1)
					...
					E.className = (H.length > 0 ? H + ' ' : '')
							+ FCKConfig.IndentClasses[G - 1];
			} else {
	...
	_IndentList : function(A, B) {
		...
		for ( var i = J._FCK_ListArray_Index; i <= K._FCK_ListArray_Index; i++)
			M[i].indent += I;
		for ( var i = K._FCK_ListArray_Index + 1; i < M.length
				&& M[i].indent > N; i++)
			M[i].indent += I;
...
FCKBlockQuoteCommand.prototype = {
	Execute : function() {
		...
		if (FCKBrowserInfo.IsIE) {
			...
			var F;
			if (D && D.parentNode.nodeName.IEquals('blockquote')
					&& !D.previousSibling) {
				F = D;
			...
			;
			if (E && E.parentNode.nodeName.IEquals('blockquote')
					&& !E.previousSibling) {
				F = E;
				while ((F = F.nextSibling)) {
					if (FCKListsLib.BlockElements[F.nodeName.toLowerCase()]) {
		...
		if (A == 0) {
			...
			while ((H = G.GetNextParagraph()))
				I.push(H);
			if (I.length < 1) {
				para = B.Window.document.createElement(FCKConfig.EnterMode
						.IEquals('p') ? 'p' : 'div');
				B.InsertNode(para);
			...
			while (K.length > 0) {
				...
				if (H.nodeName.IEquals('blockquote')) {
					var M = FCKTools.GetElementDocument(H)
			...
			while ((H = G.GetNextParagraph())) {
				...
				if (Q && R && !R._fckblockquotemoveout) {
					O.push(R);
					FCKDomTools.SetElementMarker(P, R, '_fckblockquotemoveout',
							true);
				}
			}
			;
			...
			var U = function(N) {
				for ( var i = 0; i < N.childNodes.length; i++) {
					if (FCKListsLib.BlockElements[N.childNodes[i].nodeName
							.toLowerCase()])
						return false;
				}
				;
			...
			if (FCKConfig.EnterMode.IEquals('br')) {
				while (S.length) {
					...
					if (W.nodeName.IEquals('div')) {
						var M = FCKTools.GetElementDocument(W)
								.createDocumentFragment();
						var c = a
								&& W.previousSibling
								&& !FCKListsLib.BlockBoundaries[W.previousSibling.nodeName
										.toLowerCase()];
						if (a && c)
							M.appendChild(FCKTools.GetElementDocument(W)
									.createElement('br'));
						var d = W.nextSibling
								&& !FCKListsLib.BlockBoundaries[W.nextSibling.nodeName
										.toLowerCase()];
						while (W.firstChild)
...
var FCKCoreStyleCommand = function(A) {
	...
	this.IsActive = false;
	FCKStyles.AttachStyleStateChange(this.StyleName, this._OnStyleStateChange,
			this);
};
...
FCKCommands.GetCommand = function(A) {
	...
	case 'DocProps':
		B = new FCKDialogCommand('DocProps', FCKLang.DocProps,
				'dialog/fck_docprops.html', 400, 380,
	...
	case 'Anchor':
		B = new FCKDialogCommand('Anchor', FCKLang.DlgAnchorTitle,
				'dialog/fck_anchor.html', 370, 160);
		break;
	case 'AnchorDelete':
	...
	case 'BulletedList':
		B = new FCKDialogCommand('BulletedList', FCKLang.BulletedListProp,
				'dialog/fck_listprop.html?UL', 370, 160);
		break;
	case 'NumberedList':
		B = new FCKDialogCommand('NumberedList', FCKLang.NumberedListProp,
				'dialog/fck_listprop.html?OL', 370, 160);
		break;
	case 'About':
		B = new FCKDialogCommand('About', FCKLang.About,
				'dialog/fck_about.html', 420, 330, function() {
					return 0;
				});
		break;
	case 'Find':
		B = new FCKDialogCommand('Find', FCKLang.DlgFindAndReplaceTitle,
				'dialog/fck_replace.html', 340, 230, null, null, 'Find');
		break;
	case 'Replace':
		B = new FCKDialogCommand('Replace', FCKLang.DlgFindAndReplaceTitle,
				'dialog/fck_replace.html', 340, 230, null, null, 'Replace');
		break;
	case 'Image':
		B = new FCKDialogCommand(
				'Image',
				FCKLang.DlgImgTitle,
				'dialog/fck_image.jsp?'
						+ (window.parent.document.setUpload ? '&systemId='	+ window.parent.document.setUpload.systemId.value : ''), 450, 390);
		break;
	case 'Flash':
		B = new FCKDialogCommand(
				'Flash',
				FCKLang.DlgFlashTitle,
				'dialog/fck_flash.jsp?'
						+ (window.parent.ebEdit ? '&boardId='
								+ window.parent.ebEdit.boardRid : '')
						+ (window.parent.document.getElementById('cmntId') ? '&cmntId='
								+ window.parent.document
										.getElementById('cmntId').value
								: ''), 450, 390);
		break;
	case 'SpecialChar':
		B = new FCKDialogCommand('SpecialChar', FCKLang.DlgSpecialCharTitle,
				'dialog/fck_specialchar.html', 400, 290);
		break;
	case 'Smiley':
	...
	case 'Table':
		B = new FCKDialogCommand('Table', FCKLang.DlgTableTitle,
				'dialog/fck_table.html', 480, 250);
		break;
	case 'TableProp':
		B = new FCKDialogCommand('Table', FCKLang.DlgTableTitle,
				'dialog/fck_table.html?Parent', 480, 250);
		break;
	case 'TableCellProp':
		B = new FCKDialogCommand('TableCell', FCKLang.DlgCellTitle,
				'dialog/fck_tablecell.html', 550, 240);
		break;
	case 'Style':
	...
	case 'Blockquote':
		...
		break;
	case 'CreateDiv':
		B = new FCKDialogCommand('CreateDiv', FCKLang.CreateDiv,
				'dialog/fck_div.html', 380, 210, null, null, true);
		break;
	case 'EditDiv':
		B = new FCKDialogCommand('EditDiv', FCKLang.EditDiv,
				'dialog/fck_div.html', 380, 210, null, null, false);
		break;
	case 'DeleteDiv':
	...
	case 'InsertUnorderedList':
		...
		break;
	case 'ShowBlocks':
		B = new FCKShowBlockCommand('ShowBlocks',
				FCKConfig.StartupShowBlocks ? 1 : 0);
		break;
	case 'Undefined':
...
FCKPanel.prototype.Show = function(x, y, A, B, C) {
	...
	if (this._Popup) {
		...
		if (FCKBrowserInfo.IsIE7) {
			if (this.ParentPanel
					&& this.ParentPanel.ResizeForSubpanel(this, D,
							E.offsetHeight)) {
		...
		if (FCK.ToolbarSet.CurrentInstance.GetInstanceObject('FCKPanel')._OpenedPanel
			...
			FCK.ToolbarSet.CurrentInstance.GetInstanceObject('FCKPanel')._OpenedPanel
					.Hide(false, true);
		FCKDomTools.SetElementStyles(E, {
			B : B ? B + 'px' : '',
			C : C ? C + 'px' : ''
		...
		var F = FCKTools.GetDocumentPosition(this._Window,
...
FCKToolbarButtonUI.prototype.Create = function(A) {
	...
	FCKTools.AddEventListenerEx(C, 'mouseover', FCKToolbarButtonUI_OnMouseOver,
			this);
	FCKTools.AddEventListenerEx(C, 'mouseout', FCKToolbarButtonUI_OnMouseOut,
			this);
...
FCKSpecialCombo.prototype.AddItem = function(A, B, C, D) {
	...
	FCKTools
			.AddEventListenerEx(E, 'mouseover', FCKSpecialCombo_ItemOnMouseOver);
	FCKTools.AddEventListenerEx(E, 'mouseout', FCKSpecialCombo_ItemOnMouseOut);
	FCKTools.AddEventListenerEx(E, 'click', FCKSpecialCombo_ItemOnClick, [
			this, A ]);
...
FCKSpecialCombo.prototype.Create = function(A) {
	...
	FCKTools.AddEventListenerEx(G, 'mouseover', FCKSpecialCombo_OnMouseOver,
			this);
	FCKTools
			.AddEventListenerEx(G, 'mouseout', FCKSpecialCombo_OnMouseOut, this);
...
FCKToolbarStyleCombo.prototype.CreateItems = function(A) {
	...
	for ( var D in C) {
		var E = C[D];
		var F = E.GetType() == 2 ? D : FCKToolbarStyleCombo_BuildPreview(E,
				E.Label || D);
		var G = A.AddItem(D, F);
		G.Style = E;
	}
...
FCKToolbarStyleCombo.prototype.StyleCombo_OnBeforeClick = function(A) {
	...
	for ( var i in A.Items) {
		...
		if ((D && G.Element == D) || (!D && G.GetType() != 2)) {
			F.style.display = '';
			if ((C && G.CheckActive(C))
					|| (!C && G.CheckElementRemovable(B, true)))
				A.SelectItem(G.Name);
		} else
...
FCKToolbarPanelButton.prototype.RegisterPanel = function(A) {
	...
	B.style.top = '0px';
	var C = A._FCKToolbarPanelButtonLineDiv = B.appendChild(A.Document
			.createElement('IMG'));
...
FCKToolbarItems.GetItem = function(A) {
	...
	case 'Indent':
		...
		break;
	case 'Blockquote':
		B = new FCKToolbarButton('Blockquote', FCKLang.Blockquote, null, null,
				false, true, 73);
		break;
	case 'CreateDiv':
		B = new FCKToolbarButton('CreateDiv', FCKLang.CreateDiv, null, null,
				false, true, 74);
		break;
	case 'Link':
	...
	case 'PageBreak':
		...
		break;
	case 'Rule':
		B = new FCKToolbarButton('Rule', FCKLang.InsertLineLbl,
	...
	case 'ImageButton':
		...
		break;
	case 'ShowBlocks':
		B = new FCKToolbarButton('ShowBlocks', FCKLang.ShowBlocks, null, null,
				null, true, 72);
		break;
	default:
...
var FCKDialog = (function() {
	...
	var G = function() {
		...
		FCKDomTools.SetElementStyles(C, {
			'width' : Math
					.max(H.scrollWidth, H.clientWidth, E.scrollWidth || 0)
					- 1 + 'px',
	...
	return {
		OpenDialog : function(dialogName, dialogTitle, dialogPage, width,
			...
			var L = FCKBrowserInfo.IsIE
					&& (!FCKBrowserInfo.IsIE7 || !FCKTools
							.IsStrictMode(D.document));
			if (L)
		...
		DisplayMainCover : function() {
			...
			var R = FCK.ToolbarSet.CurrentInstance
					.GetInstanceObject('frameElement');
...
FCKMenuBlock.prototype.AddItem = function(A, B, C, D, E) {
	...
	F.OnActivate = FCKTools.CreateEventListener(FCKMenuBlock_Item_OnActivate,
			this);
	this._Items.push(F);
	return F;
};
...
FCKContextMenu.prototype.SetMouseClickWindow = function(A) {
	if (!FCKBrowserInfo.IsIE) {
		...
		if (FCKBrowserInfo.IsOpera
				&& !('oncontextmenu' in document.createElement('foo'))) {
			this._Document.addEventListener('mousedown',
					FCKContextMenu_Document_OnMouseDown, false);
			this._Document.addEventListener('mouseup',
					FCKContextMenu_Document_OnMouseUp, false);
		}
		;
...
function FCKContextMenu_Document_OnMouseDown(e) {
	...
	while (A) {
		if (A._FCKContextMenu) {
			...
			;
			B.style.cssText = 'position:absolute;top:' + (e.clientY - 2)
					+ 'px;left:' + (e.clientX - 2)
...
function FCKContextMenu_AttachedElement_OnContextMenu(A, B, C) {
	if ((B.CtrlDisable && (A.ctrlKey || A.metaKey))
		...
		return {
			AddItems : function(menu, tag, tagName) {
				...
				if (C) {
					...
					D.AddItem('TableInsertCellBefore',
							FCKLang.InsertCellBefore, 69);
					D.AddItem('TableInsertCellAfter', FCKLang.InsertCellAfter,
							58);
					D.AddItem('TableDeleteCells', FCKLang.DeleteCells, 59);
					if (FCKBrowserInfo.IsGecko)
						D.AddItem('TableMergeCells', FCKLang.MergeCells, 60,
								FCKCommands.GetCommand('TableMergeCells')
										.GetState() == -1);
					else {
						D.AddItem('TableMergeRight', FCKLang.MergeRight, 60,
								FCKCommands.GetCommand('TableMergeRight')
										.GetState() == -1);
						D.AddItem('TableMergeDown', FCKLang.MergeDown, 60,
								FCKCommands.GetCommand('TableMergeDown')
										.GetState() == -1);
					}
					;
					D.AddItem('TableHorizontalSplitCell',
							FCKLang.HorizontalSplitCell, 61, FCKCommands
									.GetCommand('TableHorizontalSplitCell')
									.GetState() == -1);
					D.AddItem('TableVerticalSplitCell',
							FCKLang.VerticalSplitCell, 61, FCKCommands
									.GetCommand('TableVerticalSplitCell')
									.GetState() == -1);
					D.AddSeparator();
					D
							.AddItem('TableCellProp', FCKLang.CellProperties,
									57, FCKCommands.GetCommand('TableCellProp')
											.GetState() == -1);
					menu.AddSeparator();
					D = menu.AddItem('Row', FCKLang.RowCM);
					D.AddItem('TableInsertRowBefore', FCKLang.InsertRowBefore,
							70);
					D
							.AddItem('TableInsertRowAfter',
									FCKLang.InsertRowAfter, 62);
					D.AddItem('TableDeleteRows', FCKLang.DeleteRows, 63);
					menu.AddSeparator();
					D = menu.AddItem('Column', FCKLang.ColumnCM);
					D.AddItem('TableInsertColumnBefore',
							FCKLang.InsertColumnBefore, 71);
					D.AddItem('TableInsertColumnAfter',
							FCKLang.InsertColumnAfter, 64);
