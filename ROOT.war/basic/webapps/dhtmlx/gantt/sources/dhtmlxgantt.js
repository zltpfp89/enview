...
gantt._init_dnd = function () {
	...
	dnd.attachEvent("onDragMove", this.bind(function (obj, e) {
		...
		function allowedLevel (next, item){
			return (next.$level == item.$level || gantt.config.order_branch_free);
		}
...
gantt._render_grid_header_resize = function () {
	var columns = this.getGridColumns(),
		width = 0,
		lineHeigth = this.config.scale_height,
	...
	if (this.config.grid_resize) {
		...
		grid_resize_el.className = "gantt_grid_resize_wrap";
		grid_resize_el.style.top = "0px";
		grid_resize_el.style.height = this.$grid.offsetHeight + "px";
		grid_resize_el.innerHTML = "<div class='gantt_grid_resize'></div>";
		grid_resize_el.setAttribute(this.config.grid_resizer_attribute, true);

		this.$grid_scale.appendChild(grid_resize_el);
		grid_resize_el.style.left = Math.max(0, this.config.grid_width - 1 - grid_resize_el.offsetWidth/2) + "px";
	}
};
...
gantt._init_resize = function () {
	...
	dnd.attachEvent("onDragMove", this.bind(function (obj, e) {
		var dd = dnd.config,
			...
			pos = this._get_position(this.$grid_scale),
			left = parseInt(dd.marker.style.left, 10),
			min_right = this.config.min_grid_column_width,
			max_left = this.$grid_data.offsetWidth - this.config.min_grid_column_width * (columns.length-dd.drag_index-2),// 1 for current column + 1 for last column
			marker_left = 0,
			marker_width = 0;

		left -= pos.x - 1;
		for (var i=0; i<dd.drag_index; i++) {
	...
	dnd.attachEvent("onDragEnd", this.bind(function (obj, e) {
		...
		if (this.config.keep_grid_width) {
			...
			for (var i=target_index+1, l=columns.length; i<l; i++) {
				if (columns[i].name == "add")
					columns[i].width = columns[i].width*1 || 44;
				else
					columns[i].width = null;
			}
		} else {
			// in other case we set a new grid width and call gantt render
			for (var i=target_index, l=columns.length; i<l; i++) {
				columns_width += columns[i].width;
			}
	...
	grid_dnd.attachEvent("onDragMove", this.bind(function (obj, e) {
		...
		for(var i = 0; i < columns.length; i++) {
			if(!columns[i].hide) {
				min_left += columns[i].min_width ? columns[i].min_width : this.config.min_grid_column_width;
			}
		}
...
gantt._need_dynamic_loading = function needLoading(id){
	if(gantt.config.branch_loading && gantt._load_url){
		...
		if (!was_rendered && gantt._has_children(id)) {
			return true;
		}
	}
	return false;
...
gantt._scale_helpers.processIgnores = function(config){
...
gantt._init_task = function(task){
	...
	} else if(task.end_date && task.duration !== undefined){
...
gantt.form_blocks.typeselect = {
	render : function(sns){
		...
		var filter = sns.filter || function(){ return true;};
		for(var i in types){
		...
		if(original){
			...
			host[methodname] = function get_prefetched_value(){
				if(optimizer.active){
					...
					var args = optimizer.get_arguments_hash(Array.prototype.slice.call(arguments));
...
gantt._init_html_area = function(node){
	...
	//It will be nice to have a destructor function, to fully remove not necesary gantt instances
	if (window.gantt !== gantt)
		this._obj.gantt = gantt;
			...
			this.each(function() {
				if (this && this.getAttribute){
					if (!this.gantt){
						...
						var newgantt = window.gantt.$container?Gantt.getGanttInstance():window.gantt;
...
window.gantt = Gantt.getGanttInstance(); // default instance


if (dhtmlx){
	dhtmlx.attaches = dhtmlx.attaches || [];

	dhtmlx.attaches.attachGantt = function(start, end, newgantt){
		var obj = document.createElement("DIV");
		...
		newgantt = newgantt || (window.gantt.$container?Gantt.getGanttInstance():window.gantt);

		this.vs[this.av].sched = newgantt;
		this.vs[this.av].schedId = obj.id;

		newgantt.destructor=function(){};
		newgantt.init(obj.id, start, end);
