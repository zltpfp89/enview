	...
	var welUploadInputBox = $Element("txtUploadFile");
		...
		try{
			if( !oNavigator.ie ){
				if(!!oNavigator.safari && oNavigator.version <= 5){
					...
					bSupportDragAndDropAPI = false;		//파일첨부 드래그 모드 미사용
					//bSupportDragAndDropAPI = true;	//파일첨부 드래그 모드 사용
				}
			} else {
				bSupportDragAndDropAPI = false;
			}
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("txtUploadFile"),{
 			//sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.php',	//샘플 URL입니다.
 	        //sCallback : '/smarteditor/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
 			sUrl  : parent.opener.parent.ebUtil.getContextPath() + '/board/fileMngr?cmd=upload&cafe='+document.editor_upimage.cafe.value+'&boardId='+document.editor_upimage.boardId.value,	//샘플 URL입니다.
