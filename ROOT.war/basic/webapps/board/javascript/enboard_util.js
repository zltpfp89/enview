...
enboard.util.prototype = {
	...
	chkSelect : function (obj, initIndex, msg, isSet) {
		...
		if( isselect ) return true;
			...
			alert(msg+' '+ebUtil.getMessage('mm.info.select'));
	...
	chkCheckCnt : function( obj, cnt, msg, isSet ) {
		...
		if (cnt < 2 && chk > 0) {
			...
			if( cnt > 1) {
				msg += ' ' + ebUtil.getMessage('mm.info.select.multi', cnt);
			} else {
				msg += ' ' + ebUtil.getMessage('mm.info.select');
			}
			alert(msg);
			if( isSet == 'true' ) {
				if (isArray) obj[0].focus();
	...
	chkValue : function( obj, msg, isSet ) {
		if( obj.value.trim() == '' ) {
			alert( msg + ' ' + ebUtil.getMessage('mm.info.input'));
			if (isSet == 'true') obj.focus();
			return false;
		}
		return true;
...
enboard.util.MultiLangMngr.prototype = {
	...
	doSave : function() {
		...
		if ("ins" == act) {
			if (!ebUtil.chkSelect (document.getElementById("multiLang_langKnd"), 0, "언어를", 'true')) return;
		}
		if (this.m_tblNm == "board") {
			if (!ebUtil.chkValue (document.getElementById("multiLang_title1"), "게시판 명을", 'true')) return;
			if (!ebUtil.chkValue (document.getElementById("multiLang_title2"), "게시판 타이틀을", 'true')) return;
		} else if (this.m_tblNm == "poll") {
			if (!ebUtil.chkValue (document.getElementById("multiLang_title1"), "설문 명을", 'true')) return;
			if (!ebUtil.chkValue (document.getElementById("multiLang_title2"), "설문 설명을", 'true')) return;
		} else if (this.m_tblNm == "bltnCate" ) {
			if (!ebUtil.chkValue (document.getElementById("multiLang_title1"), "카테고리 명을", 'true')) return;
		} else if (this.m_tblNm == "bltnExtnProp" ) {
			if (!ebUtil.chkValue (document.getElementById("multiLang_title1"), "확장필드 타이틀을", 'true')) return;
		}
