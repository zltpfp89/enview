 ...
 if(this._globalIdStorage[i]==itemId)
{
 ...
 //alert("_globalIdStorageSize=" + this._globalIdStorageSize);
 for(var i=0;i<this._globalIdStorageSize;i++)
 if(this._globalIdStorage[i]==itemId)
{
...
dhtmlXTreeObject.prototype.setItemStyle=function(itemId,style_string){
 ...
 //alert("parentId="+parentId + ", temp=" + temp);
 ...
 if((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "item"))
{
 ...
 var name = "";
 if( nodx.firstChild ) {
	name = nodx.firstChild.nodeValue;
	//alert("name=" + name);
 }
 ...
 if((!dhtmlObject.waitUpdateXML)||(dhtmlObject.waitUpdateXML.toString().search(dhtmlObject.dlmtr+cId+dhtmlObject.dlmtr)!=-1))
{
 ...
 if(disheckbox){
 if(achecked!=null)this._setCheck(newNode,convertStringToBoolean(achecked));
 ...
 if((node)&&(node.i_sel))
{
 ...
 if(!mode){
 if((e)&&(e.button==2)) alert(that.arFunc);
...
dhtmlXTreeObject.prototype._correctCheckStates=function(dhtmlObject){
 ...
 //alert("itemObject.id=" + itemObject.id);
 ...
 else
{
 ...
 var img=document.createElement((itemObject.id==this.rootId)?"div":"img");
 ...
 else
{
 ...
 //td2.oncontextmenu=new Function("return false;");
 ...
 tr.oncontextmenu=Function("this.childNodes[0].parentObject.treeNod.selectItem(this.childNodes[0].parentObject.id); this.childNodes[0].parentObject.treeNod.arFunc(this.childNodes[0].parentObject.id, this.childNodes[0]);return false;");
}
 return table;
};
 ...
 for(var i=0;i<this.nodeCut.length;i++)
{
 ...
 //alert("_moveNode : itemObject=" + itemObject + ", itemObject.treeNod=" + itemObject.treeNod);
 ...
 if(itemObject.unParsed)
{
 ...
 //alert("_moveNodeTo : itemObject=" + itemObject + ", itemObject.treeNod=" + itemObject.treeNod);
 ...
 if(this.dragFunc)if(!this.dragFunc(this.subKey, itemObject.id,targetObject.id,(beforeNode?beforeNode.id:null),itemObject.treeNod,targetObject.treeNod))return false;
 ...
 for(var i=0;i<temp.childsCount;i++)
{
 ...
 var temp=this._globalIdStorageFind(itemId);
 if((!temp)||(!temp.parentObject))return null;
 
 //alert("temp=" + temp + ", temp.htmlNode=" + temp.htmlNode.innerHTML);
 
 return temp.htmlNode;
};
...
dhtmlXTreeObject.prototype._drag=function(e, sourceHtmlObject,dhtmlObject,targetHtmlObject){
 ...
 //alert("z=" + z + ", sourceHtmlObject=" + sourceHtmlObject + ", sourceHtmlObject.parentObject=" + sourceHtmlObject.parentObject);
 //alert("targetHtmlObject=" + targetHtmlObject + ", targetHtmlObject.parentObject=" + targetHtmlObject.parentObject + ", targetHtmlObject.parentObject.treeNod=" + targetHtmlObject.parentObject.treeNod);
 //alert("z=" + z + ", z.treeNod=" + z.treeNod);
 if((z)&&(z._clearMove))z._clearMove("");

 if((!this.dragMove)||(this.dragMove()))
{
 if((!z)||(!z._clearMove))var col=new Array(sourceHtmlObject.parentObject);
 else var col=z._dragged;

 for(var i=0;i<col.length;i++){
 //alert("_drag : itemObject=" + col[i]);
 ...
 if(!tobj)
{
 ...
 if((!this._onDrInFunc)||(this._onDrInFunc(fobj.id,tobj.id,fobj.treeNod,tobj.treeNod)))
{
...
dhtmlXTreeObject.prototype.moveItem=function(itemId,mode,targetId,targetTree)
{
 ...
 //alert("itemId=" + itemId + ", sNode=" + sNode + ", mode=" + mode);
 switch(mode){
...
dhtmlXTreeObject.prototype.setIconSize=function(newWidth,newHeight,itemId)
{
 ...
 this.loadXML(this.XMLsource+this._escape(id)+".xml");
 else
 this.loadXML(this.XMLsource+getUrlSymbol(this.XMLsource)+"id="+this._escape(id));
};
