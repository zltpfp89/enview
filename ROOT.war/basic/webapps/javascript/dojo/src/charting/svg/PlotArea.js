...
if(dojo.render.svg.capable){dojo.require("dojo.svg");
dojo.extend(dojo.charting.PlotArea, {initializePlot: function(plot){plot.destroy();
...
for(var i=0; i<this.plots.length; i++){this.nodes.plots.appendChild(this.initializePlot(this.plots[i]));
}
...
for(var p in axes){var obj = axes[p];
