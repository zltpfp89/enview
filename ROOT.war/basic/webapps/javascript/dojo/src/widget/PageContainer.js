...
dojo.widget.defineWidget("dojo.widget.PageContainer", dojo.widget.HtmlWidget, {isContainer: true,
doLayout: true,
...
postCreate: function(args, frag) {if(this.children.length){dojo.lang.forEach(this.children, this._setupChild, this);
var initialChild;
if(this.selectedChild){this.selectChild(this.selectedChild);
}else{for(var i=0; i<this.children.length; i++){if(this.children[i].selected){this.selectChild(this.children[i]);
break;
}}
if(!this.selectedChildWidget){this.selectChild(this.children[0]);
}}
}},
addChild: function(child){dojo.widget.PageContainer.superclass.addChild.apply(this, arguments);
this._setupChild(child);
this.onResized();
if(!this.selectedChildWidget){this.selectChild(child);
}},
_setupChild: function( page){page.hide();
page.domNode.style.position="relative";
dojo.event.topic.publish(this.widgetId+"-addChild", page);
},
removeChild: function( page){dojo.widget.PageContainer.superclass.removeChild.apply(this, arguments);
...
selectChild: function( page,  callingWidget){page = dojo.widget.byId(page);
...
forward: function(){var index = dojo.lang.find(this.children, this.selectedChildWidget);
this.selectChild(this.children[index+1]);
},
back: function(){var index = dojo.lang.find(this.children, this.selectedChildWidget);
this.selectChild(this.children[index-1]);
},
...
_showChild: function( page) {if(this.doLayout){var content = dojo.html.getContentBox(this.containerNode);
...
if(container){dojo.lang.forEach(container.children, this.onAddChild, this);
}
...
onAddChild: function( page){var button = dojo.widget.createWidget(this.buttonWidget,
{label: page.label,
...
onSelectChild: function( page){if(this._currentChild){var oldButton=this.pane2button[this._currentChild];
...
onButtonClick: function( page){var container = dojo.widget.byId(this.containerId);
container.selectChild(page, false, this);
},
onCloseButtonClick: function( page){var container = dojo.widget.byId(this.containerId);
container.closeChild(page);
},
onKey: function( evt){if( (evt.keyCode == evt.KEY_RIGHT_ARROW)||
(evt.keyCode == evt.KEY_LEFT_ARROW) ){var current = 0;
var next = null;
var current = dojo.lang.find(this.children, this.pane2button[this._currentChild]);
if(evt.keyCode == evt.KEY_RIGHT_ARROW){next = this.children[ (current+1) % this.children.length ];
}else{next = this.children[ (current+ (this.children.length-1)) % this.children.length ];
}
...
setSelected: function(){dojo.html.addClass(this.domNode, "current");
...
dojo.lang.extend(dojo.widget.Widget, {label: "",
selected: false,
closable: false,
