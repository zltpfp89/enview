...
function eventInitSetup() {
	...
	$("#toggleCriBtn").on("click", function() {	
		if($('#sheet:visible').length) {
	        ...
	        $(this).removeClass("btn_white folding").addClass("btn_white unfold");
	        $('#sheet').hide();
		} else {
			$(this).find("span").html("리스트 접기");			
			$(this).removeClass("btn_white unfold").addClass("btn_white folding");
			$('#sheet').show();
		}
		//화면사이즈 갱신
		autoResize();
...
function initDatePicker(){
	...
	var searchArea = $("div[id=dataDivLi]");
...
function selectList(rcptNum, isInit){
	...
	if(isInit && !isNull(rcptNum)){
		...
		b0101VO.rcptOrdNumSc = $("#rcptOrdNumSc").val();
		...
		b0101VO.srchStartDateSc = $("#srchStartDateSc").val();
		b0101VO.srchEndDateSc = $("#srchEndDateSc").val();
		b0101VO.dateDivSc = $("#dateDivSc").val();
	}
...
function callBackSelectListSuccess(data, rcptNum, isInit){
	...
	var QCELLProp = {
            ...
            "columns"	: [
            	...
            	{width: '6%',	key: 'poIncNum',			title: ['검찰사건번호'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	...
            	{width: '10%',	key: 'beDt',				title: ['착수일자'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '10%',	key: 'acptDt',				title: ['수리일자'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	...
            	{width: '10%',	key: 'rltActCriNmCdDesc',	title: ['위반법률 및 죄명'],			sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	...
            	{width: '8%',	key: 'criStatDesc',			title: ['상태'],			sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}}
            	],
			//"rowheader"	: "sequence",
			"frozencols" : 5
        };
...
function callBackSelectIncidentSuccess(data){
	...
	spHtml.append('		   <td class="L"><input type="text" class="w100per" id="poIncNum" name="poIncNum" data-type="required" value="'+(isNull(b0101VOMap.poIncNum) ? "" : b0101VOMap.poIncNum )+'" title="검찰사건번호" maxlength="30"/> </td>                                                                                                                                                                                                      ');
	...
	spHtml.append('		   <td class="L"><input type="text" class="w100per" id="stock" name="stock" data-stock="" value="'+(isNull(b0101VOMap.stock) ? "" : b0101VOMap.stock )+'" title="종목"  maxlength="100"/> </td>                                                                                                                                                                                                      ');
	...
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">접수일시</th>                                                                                                                                                                                                                     ');
	spHtml.append('		   <td class="L"><input type="text" class="w100per calendar datepicker" name="rcptDt" id="rcptDt" value="'+(isNull(b0101VOMap.rcptDt) ? "" : b0101VOMap.rcptDt )+'" title="접수일시" readonly />');
	spHtml.append('		   <th class="C">이첩기관</th>                                                                                                                                                                                                                      ');
	spHtml.append('		   <td class="L">서울남부지방검찰청</td>');
	spHtml.append('	   </tr>                                                                                                                                                                                                                                             ');	
	...
	spHtml.append('		   <td class="L">                                                                                                                                                                                                                               ');
	spHtml.append('			<label for="dirNm"></label><input type="text" class="w100per" id="dirNm" name="dirNm" value="'+(isNull(b0101VOMap.dirNm) ? "" : b0101VOMap.dirNm )+'"   maxlength="10"/>             ');
	spHtml.append('		</td>                                                                                                                                                                                                                                           ');
	spHtml.append('	   </tr>    ');
	spHtml.append('</table>');
	...
	spHtml.append('				<label for="incOccrAreaAddr"></label><input type="text" class="w50per" name="incOccrAreaAddr" id="incOccrAreaAddr" value="'+(isNull(b0101VOMap.incOccrAreaAddr) ? "" : b0101VOMap.incOccrAreaAddr )+'"  maxlength="300"/>                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
	spHtml.append('		<tr>                                                                                                                          ');
	spHtml.append('			<th class="C first th_line" scope="col" ><span class="table_title">범죄일시</span></th>                                         ');
	spHtml.append('			<td class="L td_line">                                                                                                      ');
	spHtml.append('				<label for="criDt"></label><input type="text" class="w100per" name="criDt" id="criDt" value="'+(isNull(b0101VOMap.criDt) ? "" : b0101VOMap.criDt )+'"  maxlength="300"/>                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
	...
	spHtml.append('			<td class="L">                                                                                                      ');
	spHtml.append('			   <label for="vicmNm"></label><input type="text" class="w100per" id="vicmNm" name="vicmNm" value="'+b0101VOMap.vicmNm+'"  maxlength="300"/>                                                                                                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
	...
	spHtml.append('			<td class="L">                                                                                                      ');
	spHtml.append('			   <label for="damgDegr"></label><input type="text" class="w100per" id="damgDegr" name="damgDegr" value="'+b0101VOMap.damgDegr+'"  maxlength="300"/>                                                                                                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
	...
	spHtml.append('			<td class="L">                                                                                                      ');
	spHtml.append('			   <label for="seizNum"></label><input type="text" class="w100per" id="seizNum" name="seizNum" value="'+b0101VOMap.seizNum+'"  maxlength="300"/>                                                                                                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
	...
	spHtml.append('			<td class="L">                                                                                                      ');
	spHtml.append('			   <label for="criPendIncFileNum"></label><input type="text" class="w100per" id="criPendIncFileNum" name="criPendIncFileNum" value="'+b0101VOMap.criPendIncFileNum+'"  maxlength="300"/>                                                                                                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
...
function handleTab(){
	...
	//수사착수 전단계 : 사건정보 노출
	if(scriStatCd == 21 || scriStatCd == 30){
		$(".tab_mini_wrap>ul>li:eq(0)").show();		//사건정보
		$(".tab_mini_wrap>ul>li:eq(1)").hide();		//송치정보
		//$(".tab_mini_wrap>ul>li:eq(2)").hide();		//서식관리
		$(".tab_mini_wrap>ul>li:eq(2)").hide();		//변경이력
		$(".tab_mini_wrap>ul>li:eq(3)").hide();		//수사대장
		$(".tab_mini_wrap>ul>li:eq(4)").hide();		//통계원표
		$(".tab_mini_wrap>ul>li:eq(5)").hide();		//수사기록철
	//수사 착수단계	
	}else if(scriStatCd == 20 || scriStatCd==99 ){
		$(".tab_mini_wrap>ul>li:eq(0)").show();		//사건정보
		$(".tab_mini_wrap>ul>li:eq(1)").hide();		//송치정보
		//$(".tab_mini_wrap>ul>li:eq(2)").hide();		//서식관리
		$(".tab_mini_wrap>ul>li:eq(2)").show();		//변경이력
		$(".tab_mini_wrap>ul>li:eq(3)").hide();		//수사대장
		$(".tab_mini_wrap>ul>li:eq(4)").hide();		//통계원표
		$(".tab_mini_wrap>ul>li:eq(5)").hide();		//수사기록철
	...
	//수사단계 : 사건정보, 수사정보, 통계원표 노출
		...
		$(".tab_mini_wrap>ul>li:eq(0)").show();		//사건정보
		$(".tab_mini_wrap>ul>li:eq(1)").show();		//송치정보
		$(".tab_mini_wrap>ul>li:eq(2)").show();		//변경이력
		$(".tab_mini_wrap>ul>li:eq(3)").show();		//문서관리
		
		if(scriStatCd == 53 || scriStatCd ==58){	//지휘"가"[53], 재지휘"가"[58]
			$(".tab_mini_wrap>ul>li:eq(4)").show();		//통계원표
		}else{
			$(".tab_mini_wrap>ul>li:eq(4)").hide();		//통계원표
		}
	...
	//송치단계 : 사건정보, 수사정보, 통계원표, 송치정보 노출
		...
		$(".tab_mini_wrap>ul>li:eq(1)").hide();		//송치정보
		$(".tab_mini_wrap>ul>li:eq(2)").show();		//변경이력
		$(".tab_mini_wrap>ul>li:eq(3)").hide();		//문서관리
		$(".tab_mini_wrap>ul>li:eq(4)").hide();		//통계원표
		$(".tab_mini_wrap>ul>li:eq(5)").hide();		//수사기록철
	}
...
function handleTab1BtnGroup() {
	...
	if(b0101VOMap.criStatCd == "21" || b0101VOMap.criStatCd == "31"){
		// 사건삭제 - 수현
		taskTrstVOSB_tab1.append(" <a href=\"javascript:Fn_b_deleteIncMast("+b0101VOMap.rcptNum+");\" class=\"btn_light_blue_line appointed\" name=\"incDeleteBtn\"> <span>사건삭제</span></a>");
		//워크플로우가 만들어졌고, 워크플로우 타야함
		taskTrstVOSB_tab1.append('<a name="updateIncidentBtn" href="javascript:updateIncident(1);" class="btn_light_blue save" data-always="y"><span>임시저장</span></a>');
		...
		if(isRootInc){
			$.each(b0101VOMap.taskTrstVOList, function(index, taskTrstVO) {
				if (SJPBRole.getHasAnyRole(taskTrstVO.taskRoleCd)) {	//역할에 맞는 버튼만 보이도록 처리	
					taskTrstVOSB_tab1.append(" <a href=\"javascript:void(0);\" class=\"btn_light_blue enrollment\" name=\"trstBtn\" value=\""+taskTrstVO.trstStatNm+"\" data-update-type=\"2\" data-trst-stat-nm=\""+taskTrstVO.trstStatNm+"\" data-trst-stat-num=\""+taskTrstVO.trstStatNum+"\" data-cri-stat-cd=\""+taskTrstVO.criStatCd+"\" data-wf-num=\""+taskTrstVO.wfNum+"\" data-always=\"y\"><span>"+taskTrstVO.trstStatNm+"</span></a> ");			
				}
			});
		}
		
	}else if(b0101VOMap.criStatCd == "20" || b0101VOMap.criStatCd == "22" ){//사건이첩
		
		isSpUpdate = true;	//피의자수정 
		taskTrstVOSB_tab1.append('<a name="updateIncidentBtn" href="javascript:updateIncident(1);" class="btn_mint enrollment" data-always="y"><span>저 장</span></a>');
	...
	}else if(b0101VOMap.criStatCd == "23" ){//사건중지  부서장승인, 수사사건부 등록
	...
	//}else if(b0101VOMap.criStatCd == "20" || b0101VOMap.criStatCd == "42" || b0101VOMap.criStatCd == "43" || b0101VOMap.criStatCd == "45" || b0101VOMap.criStatCd == "46" ){
		...
		if(isRootInc){
			...
			//	taskTrstVOSB_tab1.append(" <a href=\"javascript:Fn_b_deleteIncMast("+b0101VOMap.rcptNum+");\" class=\"btn_light_blue\" name=\"incDeleteBtn\"> <span>사건삭제</span></a>");
			//}
			if(isTrstBtnView){
				$.each(b0101VOMap.taskTrstVOList, function(index, taskTrstVO) {
					if (SJPBRole.getHasAnyRole(taskTrstVO.taskRoleCd)) {	//역할에 맞는 버튼만 보이도록 처리
						taskTrstVOSB_tab1.append(" <a href=\"javascript:void(0);\" class=\"btn_light_blue enrollment\" name=\"trstBtn\" value=\""+taskTrstVO.trstStatNm+"\" data-trst-stat-nm=\""+taskTrstVO.trstStatNm+"\" data-trst-stat-num=\""+taskTrstVO.trstStatNum+"\" data-cri-stat-cd=\""+taskTrstVO.criStatCd+"\" data-wf-num=\""+taskTrstVO.wfNum+"\" data-always=\"y\"><span>"+taskTrstVO.trstStatNm+"</span></a> ");			
					}
				});
			}
	...
	//담당수사관 배정중
		...
		*/
		...
		isSpUpdate = true;	//피의자수정 
		//taskTrstVOSB_tab1.append('<a name="updateIncidentBtn" href="javascript:updateIncident(1);" class="btn_light_blue enrollment" data-always="y"><span>저 장</span></a>');
		...
		if(isRootInc){
			$.each(b0101VOMap.taskTrstVOList, function(index, taskTrstVO) {
				if (SJPBRole.getHasAnyRole(taskTrstVO.taskRoleCd)) {	//역할에 맞는 버튼만 보이도록 처리	
					taskTrstVOSB_tab1.append(" <a href=\"javascript:void(0);\" class=\"btn_light_blue enrollment\" name=\"trstBtn\" value=\""+taskTrstVO.trstStatNm+"\" data-update-type=\"30\" data-trst-stat-nm=\""+taskTrstVO.trstStatNm+"\" data-trst-stat-num=\""+taskTrstVO.trstStatNum+"\" data-cri-stat-cd=\""+taskTrstVO.criStatCd+"\" data-wf-num=\""+taskTrstVO.wfNum+"\" data-always=\"y\"><span>"+taskTrstVO.trstStatNm+"</span></a> ");			
				}
			});
		}
	...
	}else if(b0101VOMap.criStatCd >= 72){
...
//			|| (SJPBRole.getDrhfRoleYn() && b0101VOMap.criStatCd == "46")
		...
		$("#apprArea_EditButtons a:first").attr("class", "btn_light_blue");
		...
		$("#taskComn").val("").attr("disabled",false);
			...
			$("#apprArea_EditButtons").html(trstBtnGroupHtml_tab1);
			$("#apprArea_EditButtons a:first").attr("class", "btn_light_blue save");
			$("#taskNmTD").html(getTaskNm(b0101VOMap.taskNm));
			$("#criArea02").hide();
			if (trstBtnGroupHtml_tab1 != "") {
				$("#apprArea02").show();
		...
		$("#btnArea_EditButtons a:last").attr("class", "btn_light_blue save");
		$("#criArea02").show();
		$("#apprArea02").hide();
...
function callBackInsertIncMastViewSuccess(data){
	...
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">검찰사건번호 <em class=\"red\">*</em></th>                                                                                                                                                                                                                      ');
	spHtml.append('		   <td class="L"><input type="text" class="w100per" id="poIncNum" name="poIncNum" data-poIncNum="" value="" data-type="required" title="검찰사건번호" maxlength="30"/> </td>                                                                                                                                                                                                      ');
	spHtml.append('		   <th class="C">발각형태 <em class=\"red\">*</em></th>                                                                                                                                                                                                                      ');
	spHtml.append('		   <td class="L">                                  ');
	spHtml.append('	   			<div class="inputbox w80per">                                                       ');	
	spHtml.append('	   				<p class="txt"></p>                                                   ');	
	...
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">종목</th>                                                                                                                                                                                                                      ');
	spHtml.append('		   <td class="L"><input type="text" class="w100per" id="stock" name="stock" data-stock="" value=""  maxlength="100"/> </td>                                                                                                                                                                                                      ');
	spHtml.append('		   <th class="C">착수일시</th>                                                                                                                                                                                                                      ');
	...
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">접수일시</th>                                                                                                                                                                                                                     ');
	spHtml.append('		   <td class="L"><input type="text" class="w100per calendar datepicker" name="rcptDt" id="rcptDt" title="접수일시" readonly />');
	spHtml.append('		   <th class="C">이첩기관</th>                                                                                                                                                                                                                      ');
	spHtml.append('		   <td class="L">서울남부지방검찰청</td>');
	spHtml.append('	   </tr>                                                                                                                                                                                                                                             ');	
	...
	spHtml.append('		   <td class="L">                                                                                                                                                                                                                               ');
	spHtml.append('			<p class="searchinput">                                                                                                                                                                                                                     ');
	spHtml.append('			<label for="dirNm"></label><input type="text" class="w100per" id="dirNm" name="dirNm" value=""  maxlength="9"/>            ');
	spHtml.append('			</p>                                                                                                                                                                                                                                        ');
	spHtml.append('		</td>                                                                                                                                                                                                                                           ');
	spHtml.append('	   </tr>                                                                                                                                                                                                                                            ');
...
//	spHtml.append('   </tbody>                                                                                                                                                                                                                                          ');
	...
	spHtml.append('   <tbody>                                                                                                                                                                                                                                           ');
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C line_right" rowspan="16">피의자 정보</th>                                                                                                                                                                                             ');
	...
	var vioTypeCodeHtml = new StringBuffer();

	
	$.each(data.vioTypeCodeList, function(k, vioType) {
		
		vioTypeCodeHtml.append('						<input type="checkbox" id="vioType_0_0_'+k+'" value="'+vioType.code+'" name="vioType" /><label for="vioType_0_0_'+k+'">'+vioType.codeName+'</label>   </br>                                                                                                       ');
	});
	
	var vioHght = (data.vioTypeCodeList.length)*36+5;
	
	spHtml.append('	   			<li style="height:'+vioHght+'px;"><span class="title">위반유형 <em class=\"red\">*</em></br>(복수선택 가능)</span>                                                     ');	
	spHtml.append(vioTypeCodeHtml);
	spHtml.append('	   			</li>                                                                                   ');	
	
	
	spHtml.append('	   			<li><span class="title">위반내용 <em class=\"red\">*</em></span>                                                     ');	
	spHtml.append('	   				<label for="vioCont_0_0"></label><input type="text" class="w80per" id="vioCont_0_0" name="vioCont" data-type="name" data-optional-value=false maxlength="100" title="위반내용"/>');
	spHtml.append('	   			</li>                                                                                   ');	
	
	spHtml.append('	   			<li><span class="title">위반조항 <em class=\"red\">*</em></span>                                                     ');	
	spHtml.append('	   				<label for="actVioClaNm_0_0"></label><input type="text" class="w80per" id="actVioClaNm_0_0" name="actVioClaNm" data-type="name" data-optional-value=false maxlength="300" title="위반조항"/>');
	spHtml.append('	   			</li>                                                                                   ');	
	...
	*/
	...
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">내사할 사항</th>                                                                                                                                                                                                                       ');
	spHtml.append('		   <td class="C" colspan="3">                                                                                                                                                                                                                       ');
	spHtml.append('			   <label for="incText_0"></label><input type="text" class="w100per" id="incText_0" name="incText"  maxlength="300" />                                                                                                                                                    ');
	spHtml.append('		   </td>                                                                                                                                                                                                                                        ');
	spHtml.append('	   </tr>                                                                                                                                                                                                                                             ');
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">처리결과</th>                                                                                                                                                                                                                       ');
	spHtml.append('		   <td class="C" colspan="3">                                                                                                                                                                                                                       ');
	spHtml.append('			   <label for="proRst_0"></label><input type="text" class="w100per" id="proRst_0" name="proRst" maxlength="300" />                                                                                                                                                    ');
	spHtml.append('		   </td>                                                                                                                                                                                                                                        ');	
	spHtml.append('	   </tr>                                                                                                                                                                                                                                             ');
	
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">비고</th>                                                                                                                                                                                                                       ');
	spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                               ');
	spHtml.append('			   <label for="incComn_0"></label><input type="text" class="w100per" id="incComn_0" name="incComn"  maxlength="300"/>                                                                                                                                                    ');		
	spHtml.append('	  	   </td>                                                                                                                                                                                                                                             ');
	spHtml.append('	   </tr>                                                                                                                                                                                                                                             ');
	
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">부당이득</th>                                                                                                                                                                                                                       ');
	spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                               ');
	spHtml.append('			   <label for="unprofit_0"></label><input type="text" class="w100per" id="unprofit_0" name="unprofit"  maxlength="300"/>                                                                                                                                                    ');		
	spHtml.append('	  	   </td>                                                                                                                                                                                                                                             ');
	spHtml.append('	   </tr>                                                                                                                                                                                                                                             ');
	...
	spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                   ');
	spHtml.append('			   <label for="cathWrnt_0"></label><input type="text" class="w100per" id="cathWrnt_0" name="cathWrnt" maxlength="300"/>                                                                                                                                                    ');
	spHtml.append('		   </td>                                                                                                                                                                                                                                        ');
	spHtml.append('	   </tr>                                                                                                                                                                                                                                            ');
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">긴급체포</th>                                                                                                                                                                                                                        ');
	spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                   ');
	spHtml.append('			   <label for="emgyCath_0"></label><input type="text" class="w100per" id="emgyCath_0" name="emgyCath" maxlength="300" />                                                                                                                                                    ');
	spHtml.append('		   </td>                                                                                                                                                                                                                                        ');
	spHtml.append('	   </tr>                                                                                                                                                                                                                                            ');
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">현행범인체포</th>                                                                                                                                                                                                                        ');
	spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                   ');
	spHtml.append('			   <label for="flgtOfdrCath_0"></label><input type="text" class="w100per" id="flgtOfdrCath_0" name="flgtOfdrCath" maxlength="300" />                                                                                                                                                    ');
	spHtml.append('		   </td>                                                                                                                                                                                                                                        ');
	spHtml.append('	   </tr>                                                                                                                                                                                                                                            ');
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">구속영장</th>                                                                                                                                                                                                                        ');
	spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                   ');
	spHtml.append('			   <label for="arstWrnt_0"></label><input type="text" class="w100per" id="arstWrnt_0" name="arstWrnt" maxlength="300" />                                                                                                                                                    ');
	spHtml.append('		   </td>                                                                                                                                                                                                                                        ');
	spHtml.append('	   </tr>                                                                                                                                                                                                                                            ');
	spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
	spHtml.append('		   <th class="C">인치구금</th>                                                                                                                                                                                                                        ');
	spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                   ');
	spHtml.append('			   <label for="inchDetn_0"></label><input type="text" class="w100per" id="inchDetn_0" name="inchDetn" maxlength="300" />                                                                                                                                                    ');
	spHtml.append('		   </td>                                                                                                                                                                                                                                        ');
	spHtml.append('	   </tr>                                                                                                                                                                                                                                            ');
	...
	spHtml.append('			<th class="C first th_line" scope="col" ><span class="table_title">범죄장소</span></th>                                         ');
	spHtml.append('			<td class="L td_line">                                                                                                      ');
	spHtml.append('				<label for="incOccrAreaAddr"></label><input type="text" class="w50per" name="incOccrAreaAddr" id="incOccrAreaAddr" maxlength="300"/>                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
	...
	spHtml.append('			<th class="C first th_line" scope="col" ><span class="table_title">범죄일시</span></th>                                         ');
	spHtml.append('			<td class="L td_line">                                                                                                      ');
	spHtml.append('				<label for="criDt"></label><input type="text" class="w100per" name="criDt" id="criDt" value="" maxlength="300"/>                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
	...
	spHtml.append('			<th class="C">피해자 성명</th>                                         ');
	spHtml.append('			<td class="L">                                                                                                      ');
	spHtml.append('			   <label for="vicmNm"></label><input type="text" class="w100per" id="vicmNm" name="vicmNm" value="" maxlength="300"/>                                                                                                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
	...
	spHtml.append('			<th class="C">피해자 피해정도</th>                                         ');
	spHtml.append('			<td class="L">                                                                                                      ');
	spHtml.append('			   <label for="damgDegr"></label><input type="text" class="w100per" id="damgDegr" name="damgDegr" value="" maxlength="300"/>                                                                                                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
	...
	spHtml.append('			<th class="C">압수번호</th>                                         ');
	spHtml.append('			<td class="L">                                                                                                      ');
	spHtml.append('			   <label for="seizNum"></label><input type="text" class="w100per" id="seizNum" name="seizNum" value="" maxlength="300"/>                                                                                                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
	...
	spHtml.append('			<th class="C">수사미결사건철번호</th>                                         ');
	spHtml.append('			<td class="L">                                                                                                      ');
	spHtml.append('			   <label for="criPendIncFileNum"></label><input type="text" class="w100per" id="criPendIncFileNum" name="criPendIncFileNum" value="" maxlength="300"/>                                                                                                                                                    ');
	spHtml.append('			</td>                                                                                                                       ');
	spHtml.append('		</tr>                                                                                                                            ');
	...
	spHtml.append('<div class="btnArea" id="criArea02">                                                                                                                                                                                                                                ');
	spHtml.append('   <div class="r_btn" id="btnArea_EditButtons"><a href="javascript:saveIncMast();" class="btn_mint save"><span>임시저장</span></a><a href="javascript:insertIncMast();" class="btn_light_blue enrollment"><span>사건등록</span></a></div>                                                                                                                     ');
	spHtml.append('</div>                                                                                                                                                                                                                                               ');
...
function callBackexecuteTaskSuccess(data, callbackFunction) {
	...
	if(data.criStatCd == "22" ){
		alert("정상적으로 처리되었습니다. 수사사건부등록이후에는 송치정보에서 사용하세요.");
	}else{
		alert("정상적으로 처리되었습니다.");
	}
...
function renderSpHtml(data, incSpVOList, subSpTitle){
	...
	$.each(incSpVOList, function(i, incSpVO) {
		...
		var spNmDec = incSpVO.spNm;
		var spAddrDec = incSpVO.spAddr;
		var spCnttNumDec = incSpVO.spCnttNum;
		var spJobDec = incSpVO.spJob;
		var spBsnsNmDec = incSpVO.spBsnsNm;
		...
		spHtml.append('		   <th class="C line_right" rowspan="16"> '+subSpTitleTmp+' 피의자<br/>정보                                                                                                                                                                                             ');
		...
		if(incSpVO.actVioVOList != null){
			$.each(incSpVO.actVioVOList, function(j, ActVioVO) {
				...
				$.each(data.vioTypeCodeList, function(k, vioType) {	
					if(ActVioVO.actVioTypeVOList != null){					
						$.each(ActVioVO.actVioTypeVOList, function(u, TypeCheck) {	
							if(vioType.code.indexOf(TypeCheck.actVioTypeCode) >-1){
								checkYN="Y";
							}
						});
						
						if(checkYN == "Y"){
							vioTypeCodeHtml.append('						<label for="vioType_'+countSp+'_'+i+'_'+j+'_'+k+'"><input type="checkbox" id="vioType_'+countSp+'_'+i+'_'+j+'_'+k+'" value="'+vioType.code+'" name="vioType" checked="true"/>'+vioType.codeName+'</label> </br>                                                                                                       ');
						}else{
							vioTypeCodeHtml.append('						<label for="vioType_'+countSp+'_'+i+'_'+j+'_'+k+'"><input type="checkbox" id="vioType_'+countSp+'_'+i+'_'+j+'_'+k+'" value="'+vioType.code+'" name="vioType" />'+vioType.codeName+'</label>  </br>                                                                                                         ');		
						}
						
						checkYN="N";
					}else{
						vioTypeCodeHtml.append('						<input type="checkbox" id="vioType_'+countSp+'_'+i+'_'+j+'_'+k+'" value="'+vioType.code+'" name="vioType" /><label for="vioType_'+countSp+'_'+i+'_'+j+'_'+k+'">'+vioType.codeName+'</label>   </br>                                                                                                         ');		
				...
				spHtml.append('	   			</li>                                                                                       ');
				spHtml.append('	   			<li><span class="title">위반조항<em class=\"red\">*</em></span>                                                         ');
				spHtml.append('	   				<label for="actVioClaNm_'+countSp+'_'+i+'_'+j+'"></label><input type="text" class="w80per" id="actVioClaNm_'+countSp+'_'+i+'_'+j+'" name="actVioClaNm" value="'+getParamValue(ActVioVO.actVioClaNm)+'" data-type="name" data-optional-value=false maxlength="300" title="위반조항"/>    ');
		...
		//사건에 저장된 위반법률이 없다면 입력하는 화면 노출. 2018.12.10 수정
			...
			spHtml.append('	   			</li>                                                                                   ');	
			spHtml.append('	   			<li><span class="title">위반내용 <em class=\"red\">*</em></span>                                                     ');	
			spHtml.append('	   				<label for="vioCont_'+countSp+'_0_0"></label><input type="text" class="w80per" id="vioCont_'+countSp+'_0_0" name="vioCont" data-type="required" maxlength="300"/>');
			spHtml.append('	   			</li>                                                                                   ');	
			
			spHtml.append('	   			<li><span class="title">위반조항 <em class=\"red\">*</em></span>                                                     ');	
			spHtml.append('	   				<label for="actVioClaNm_'+countSp+'_0_0"></label><input type="text" class="w80per" id="actVioClaNm_'+countSp+'_0_0" name="actVioClaNm" data-type="required"  maxlength="30"/>');
			spHtml.append('	   			</li>                                                                                   ');	
		...
		spHtml.append('		   <th class="C">내사할 사항</th>                                                                                                                                                                                                                       ');
		spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                               ');
		spHtml.append('			   <label for="incText_'+countSp+'_0"></label><input type="text" class="w100per" id="incText_'+countSp+'_0" name="incText" value="'+incSpVO.incText+'" maxlength="300"/>                                                                                                                                                    ');		
		spHtml.append('	  	   </td>                                                                                                                                                                                                                                             ');
		spHtml.append('	   </tr>                                                                                                                                                                                                                                             ');	
		spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
		spHtml.append('		   <th class="C">처리 결과</th>                                                                                                                                                                                                                       ');
		spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                               ');
		spHtml.append('			   <label for="proRst_'+countSp+'_0"></label><input type="text" class="w100per" id="proRst_'+countSp+'_0" name="proRst" value="'+incSpVO.proRst+'" maxlength="300"/>                                                                                                                                                    ');		
		spHtml.append('	  	   </td>                                                                                                                                                                                                                                             ');
		spHtml.append('	   </tr>                                                                                                                                                                                                                                             ');
		...
		spHtml.append('		   <th class="C">비고</th>                                                                                                                                                                                                                       ');
		spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                               ');
		spHtml.append('			   <label for="incComn_'+countSp+'_0"></label><input type="text" class="w100per" id="incComn_'+countSp+'_0" name="incComn" value="'+incSpVO.incComn+'" maxlength="300"/>                                                                                                                                                    ');		
		spHtml.append('	  	   </td>                                                                                                                                                                                                                                             ');
		spHtml.append('	   </tr>                                                                                                                                                                                                                                             ');
		spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
		spHtml.append('		   <th class="C">부당이득</th>                                                                                                                                                                                                                       ');
		spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                               ');
		spHtml.append('			   <label for="unprofit_'+countSp+'_0""></label><input type="text" class="w100per" id="unprofit_'+countSp+'_0"" name="unprofit" value="'+incSpVO.unprofit+'"   maxlength="300"/>                                                                                                                                                    ');		
		spHtml.append('	  	   </td>                                                                                                                                                                                                                                             ');
		spHtml.append('	   </tr>                                                                                                                                                                                                                                             ');
		...
		spHtml.append('		   <th class="C">체포영장</th>                                                                                                                                                                                                                        ');
		spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                   ');
		spHtml.append('			   <label for="cathWrnt_'+countSp+'_0"></label><input type="text" class="w100per" id="cathWrnt_'+countSp+'_0" name="cathWrnt" value="'+incSpVO.cathWrnt+'"  maxlength="300"/>                                                                                                                                                    ');
		spHtml.append('		   </td>                                                                                                                                                                                                                                        ');
		spHtml.append('	   </tr>                                                                                                                                                                                                                                            ');
		spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
		spHtml.append('		   <th class="C">긴급체포</th>                                                                                                                                                                                                                        ');
		spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                   ');
		spHtml.append('			   <label for="emgyCath_'+countSp+'_0"></label><input type="text" class="w100per" id="emgyCath_'+countSp+'_0" name="emgyCath" value="'+incSpVO.emgyCath+'"  maxlength="300" />                                                                                                                                                    ');
		spHtml.append('		   </td>                                                                                                                                                                                                                                        ');
		spHtml.append('	   </tr>                                                                                                                                                                                                                                            ');
		spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
		spHtml.append('		   <th class="C">현행범인체포</th>                                                                                                                                                                                                                        ');
		spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                   ');
		spHtml.append('			   <label for="flgtOfdrCath_'+countSp+'_0"></label><input type="text" class="w100per" id="flgtOfdrCath_'+countSp+'_0" name="flgtOfdrCath" value="'+incSpVO.flgtOfdrCath+'"  maxlength="300" />                                                                                                                                                    ');
		spHtml.append('		   </td>                                                                                                                                                                                                                                        ');
		spHtml.append('	   </tr>                                                                                                                                                                                                                                            ');
		spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
		spHtml.append('		   <th class="C">구속영장</th>                                                                                                                                                                                                                        ');
		spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                   ');
		spHtml.append('			   <label for="arstWrnt_'+countSp+'_0"></label><input type="text" class="w100per" id="arstWrnt_'+countSp+'_0" name="arstWrnt" value="'+incSpVO.arstWrnt+'"  maxlength="300" />                                                                                                                                                    ');
		spHtml.append('		   </td>                                                                                                                                                                                                                                        ');
		spHtml.append('	   </tr>                                                                                                                                                                                                                                            ');
		spHtml.append('	   <tr>                                                                                                                                                                                                                                             ');
		spHtml.append('		   <th class="C">인치구금</th>                                                                                                                                                                                                                        ');
		spHtml.append('		   <td class="L" colspan="3">                                                                                                                                                                                                                   ');
		spHtml.append('			   <label for="inchDetn_'+countSp+'_0"></label><input type="text" class="w100per" id="inchDetn_'+countSp+'_0" name="inchDetn" value="'+incSpVO.inchDetn+'"  maxlength="300" />                                                                                                                                                    ');
		spHtml.append('		   </td>                                                                                                                                                                                                                                        ');
		spHtml.append('	   </tr>                                                                                                                                                                                                                                            ');
...
function syncIncSpVOList(paramMap) {
	...
	$("div[data-name=personinfo]").each(function(i) {
		...
		var incSpVO = {
			...
			,spTpCd : "2"
			,spIdNum : $(this).find("input[name=spIdNum]").val()
			...
			,spNm : $(this).find("input[name=spNm]").val()
			,spAddr : $(this).find("input[name=spAddr]").val()
			,spJob : $(this).find("input[name=spJob]").val()
			,spBsnsNm : $(this).find("input[name=spBsnsNm]").val()
			,spCnttNum : $(this).find("input[name=spCnttNumA]").val() + "-" +  $(this).find("input[name=spCnttNumB]").val() + "-" +  $(this).find("input[name=spCnttNumC]").val()
			,inqOrd : i+1
			...
			,incText : $(this).find("input[name=incText]").val()
			,proRst : $(this).find("input[name=proRst]").val()
			,incComn : $(this).find("input[name=incComn]").val()
			,unprofit : $(this).find("input[name=unprofit]").val()
		}
		...
		$($(this).find(".law")).each(function(j) {
			var ActVioVO = {
					...
					,inqOrd : j+1
					,updateCd : $(this).find("input[name=updateCd]:eq(0)").val()
					,actVioClaNm : $(this).find("input[name=actVioClaNm]").val()
			}
			
			//위반유형
			var actVioTypeVOArray = new Array();
			
			$(this).find("input[name=vioType]").each(function(k){
				if($(this).is(":checked")){
					
					var ActVioTypeVO = {
							inqOrd : k+1
							,actVioTypeCode : $(this).val()
					}
					actVioTypeVOArray.push(ActVioTypeVO);
					
				}
			});
			
			//위반유형 셋팅
			ActVioVO.actVioTypeVOList = actVioTypeVOArray;
...
function syncB0101VOMap() {
	...
	b0101VOMap.rcptDt = $("#rcptDt").val();		//접수일시
	b0101VOMap.dirNm = $("#dirNm").val();		//지휘자
...
function susaPrnBtnClick(){
	var rcptNumList = [];
	for(var i = 1; i <= qcell.getRows("data")+1; i++){
		if(JSON.stringify(qcell.getCellLabel(i,0)) == "true"){
			rcptNumList.push(qcell.getRowData(i).rcptNum);
		}	
	}
	var rcptNumListMap = {
			"rcptNumList":rcptNumList
	}
	
	
	if(rcptNumList.length != 0){
		//수사사건부 레포트 성공함수 호출
		goAjaxDefault("/sjpb/B/prnCheckSusaBkList.face", rcptNumListMap, susaPrnBtnSuccess);
	}else{
...
//수사사건부 출력 성공함수
function susaPrnBtnSuccess(data){
	prn.rexdataset = null;
	prn.rexdataset = data.rexdataset;
	
	var xmlString = objectToXml(prn);
	alert(xmlString);
...
function incPrnBtnClick(){
	...
	if(rcptNumList.length != 0){
		//수사사건부 레포트 성공함수 호출
		goAjaxDefault("/sjpb/B/prnCheckIncBkList.face", rcptNumListMap, incPrnBtnSuccess);
	}else{
		alert("출력할 대상을 선택해주세요.");
		return;
	}
...
//압수부 출력 성공함수
function incPrnBtnSuccess(data){
	prn.rexdataset = null;
	prn.rexdataset = data.rexdataset;
	
	var xmlString = objectToXml(prn);
	$("#reptNm").val("M0220.crf"); //레포트 파일명
	$("#xmlData").val(xmlString);
	//레/포트 호출
	openReportService(reportForm); 
}
