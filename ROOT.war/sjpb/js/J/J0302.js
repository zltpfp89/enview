$(document).ready(function(){
	...
	setAreaMap($("#j_searchList"), J0302SCMap);
	J0302SCMap.fdCdArray = fdCdArray.toString();
	//페이지 진입시 년도별 송치 및 지휘(건/명) 출력시 성공함수 호출
	goAjaxDefault("/sjpb/J/J0302List.face",J0302SCMap,callBackFn_selectListSuccess);

	$("#exceldown").click(function(){
		
		var properties = {
				filename: "년도별_송치_및_지휘수사현황",
...
//년도별_송치_및_지휘수사현황 검색리스트를 가져온다. (ajax)
function fn_searchList() {
	...
	//년도별_송치_및_지휘수사현황 리스트를 가지고 온다.
	goAjaxDefault("/sjpb/J/J0302List.face", J0302SCMap, callBackFn_selectListSuccess);

}

//년도별_송치_및_지휘수사현황 리스트 성공함수. (ajax)
function callBackFn_selectListSuccess(data){
	...
	var QCELLProp = {
			...
			"selectmode": "row",
			"columns" : [ 
				{ width : '10%', key : 'sort', title : ['구분','구분','구분','구분'], sort : true, move : false, resize : false, styleclassname : { data : 'aligncenter fontsize17' } }, 
