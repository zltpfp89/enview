...
function prnData(){
	...
	for(var i = 2; i <= qcell.getRows("data")+1; i++){
		if(JSON.stringify(qcell.getCellLabel(i,1)) == "true"){
			trfDocNumList.push(qcell.getRowData(i).trfDocNum);
		}	
	}
	var trfDocNumListMap = {
			"trfDocNumList":trfDocNumList
	}
	
	if(trfDocNumListMap.length != 0){
		//레포트 성공함수 호출
		goAjaxDefault("/sjpb/M/M0217prnCheckReport.face", trfDocNumListMap, callBackPrnDataSuccess);
	}else{
...
function callBackPrnDataSuccess(data){
	...
	m0217RTMap.rexdataset = data.rexdataset;
	
	var xmlString = objectToXml(m0217RTMap);
	
	$("#reptNm").val("M0217.crf"); //레포트 파일명
...
function selectList(selectPkNum){
	...
	if(parent.viewType == 'inc'){
		//해당사건번호만 노출함 
		m0217SCMap.rcptNumSc = incMap.rcptNum;
		
	}else{
		setAreaMap($("#searchArea"), m0217SCMap);
		
	}
	//2018.11.19 추가 E
	
	goAjax("/sjpb/M/M0217selectList.face", m0217SCMap, function (data){callBackSelectListSuccess(data, selectPkNum)},true);
}
...
function showList(data, selectPkNum){
	...
	var QCELLProp = {
            ...
            "columns"	: [
            	...
            	{width: '150',	key: 'rcptIncNum',			title: ['사건번호'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'recvNm',			title: ['수신처'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'spNm',			title: ['피의자이름'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'fingerNum',			title: ['지문원지작성번호'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'cmctNum',			title: ['구속영장청구번호'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'dvCau',			title: ['발각원인'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'revDate',			title: ['접수일'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYYMMDD", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'cmctDate',			title: ['구속일'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYYMMDD", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'releaseDate',			title: ['석방일'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYYMMDD", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'comnt',			title: ['의견'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'evidItem',			title: ['증거품'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'comn',			title: ['비고'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
            	{width: '100',	key: 'procInput',			title: ['수리전산입력'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
        ...
        if (qcell.getRows("data") > 0) {
        	...
        	if(selectPkNum == null){	//selectPkNum이 없으면 첫번째줄 선택
        		rowIdx = headerRows;
        		//debugger;
        		selectData(qcell.getRowData(rowIdx).trfDocNum);	//TODO 페이지별 수정필요
...
function insertData(){
	...
	//Map 데이터 갱신
	if(syncM0217VOMap(true)){	//TODO 페이지별 수정
		goAjax("/sjpb/M/M0217insertUpdateData.face", m0217VOMap, callBackInsertDataSuccess);
	}
}
...
function callBackInsertDataSuccess(data){
	...
	//성공
	if(data.m0217VO.result == "01"){
		alert("사건 송치부가 등록되었습니다.");
		
		//rowIdx를 맨 위로 설정함. 
		rowIdx = headerRows;
		
		//리스트재조회
		selectList(data.m0217VO.trfDocNum);
		
		
	}else{
		alert(data.m0217VO.errMsg);
...
function updateData(){
	//Map 데이터 갱신
	if(syncM0217VOMap(true)){
		goAjax("/sjpb/M/M0217insertUpdateData.face", m0217VOMap, callBackUpdateDataSuccess);
	}
}
...
function selectData(selectPkNum){
	var reqMap = {
			trfDocNum : selectPkNum		//TODO 페이지마다 수정 
		}
	goAjaxDefault("/sjpb/M/M0217selectData.face", reqMap, callBackSelectDataSuccess);
}
...
function callBackSelectDataSuccess(data){	
	...
	//데이터 셋팅
	getAreaMap($("#contentsArea"), m0217VOMap);
...
function handleUI(){
	...
	//신규 등록일 경우, 저장 버튼 노출 
	if(isNull(m0217VOMap.trfDocNum)){	//TODO 페이지별 수정
	...
	//수정일 경우, 수정 버튼 노출 
		...
		if($("#userId").val() == m0217VOMap.regUserId && parent.viewType == 'inc'){
			areaSetWrite($("#contentsArea"));
	...
	if(colIdx >0 && rowIdx > 0){
		if(colIdx != 1){
			...
			if (rowIdx > headerRows -1) {
				selectData(qcell.getRowData(rowIdx).trfDocNum);	//TODO 페이지별 수정
			}
		}
	}
}
...
//Map 데이터 갱신
function syncM0217VOMap(isValid) {
	...
	if(isValid){
		...
		m0217VOMap.rcptNum = rcptNum;
		m0217VOMap.incSpNum = incSpNum;
		
		
	}
	
	m0217VOMap.dvCau = $("#dvCau").val();
	//데이터셋팅
	setAreaMap($("#contentsArea"), m0217VOMap);
	setFieldValue($("#dvCau"), m0217VOMap.dvCau);
	debugger;
	return true;
...
function requestReport(){
	$("#reptNm").val("M0217.crf"); //레포트 파일명
	$("#SEQNUM").val(m0217VOMap.trfDocNum);
	//레포트 호출
	openReportServiceFSS(reportForm); 
}
//초기화
function initM0217VOMap(){
	m0217VOMap = {
		trfDocNum :  ""
		,docNum :  ""
		,rcptNum :  ""
