...
//출력 
function prnData(){
	var cmctRstrExecFactNumList = [];
	for(var i = 2; i <= qcell.getRows("data")+1; i++){
		if(JSON.stringify(qcell.getCellLabel(i,1)) == "true"){
			cmctRstrExecFactNumList.push(qcell.getRowData(i).cmctRstrExecFactNum);
		}	
	}
	var cmctRstrExecFactNumListMap = {
			"cmctRstrExecFactNumList":cmctRstrExecFactNumList
	}
	
	if(cmctRstrExecFactNumList.length != 0){
		//레포트 성공함수 호출
		goAjaxDefault("/sjpb/M/M0120prnCheckReport.face", cmctRstrExecFactNumListMap, callBackPrnDataSuccess);
	}else{
...
function callBackPrnDataSuccess(data){
	...
	m0120RTMap.rexdataset = data.rexdataset;
...
function selectList(selectPkNum){
	...
	if(parent.viewType == 'inc'){
		m0120SCMap.rcptNumSc = incMap.rcptNum;
	}else{
	...
	goAjax("/sjpb/M/M0120selectList.face", m0120SCMap, function (data){callBackSelectListSuccess(data, selectPkNum)},true);
}
...
function showList(data, selectPkNum){
	...
	var QCELLProp = {
            ...
            "columns"	: [
            	...
            	{width: '7%',	key: 'execMbTitlNm',			title: ['통신제한조치 집행사건','집행자의관직·성명'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	...
            	{width: '15%',	key: 'handDtRst',			title: ['통신제한조치 집행사건','처리일자 및 처리결과'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '10%',	key: 'handRstNotcDt',				title: ['통신제한조치 집행사건','처리결과를 통보받은일자'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '7%',	key: 'notcDt',				title: ['통지일자','통지일자'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize13'}},
...
function callBackInsertDataSuccess(data){
	...
	if(data.m0120VO.result == "01"){
		alert("통신제한조치집행사실통지유예승인신청부가 등록되었습니다.");
