...
//출력 
function prnData(){
	var cmctRstrDelyApprNumList = [];
	for(var i = 2; i <= qcell.getRows("data")+1; i++){
		if(JSON.stringify(qcell.getCellLabel(i,1)) == "true"){
			cmctRstrDelyApprNumList.push(qcell.getRowData(i).cmctRstrDelyApprNum);
		}	
	}
	var cmctRstrDelyApprNumListMap = {
			"cmctRstrDelyApprNumList":cmctRstrDelyApprNumList
	}
	
	if(cmctRstrDelyApprNumList.length != 0){
		//레포트 성공함수 호출
		goAjaxDefault("/sjpb/M/M0121prnCheckReport.face", cmctRstrDelyApprNumListMap, callBackPrnDataSuccess);
	}else{
...
function callBackPrnDataSuccess(data){
	...
	m0121RTMap.rexdataset = data.rexdataset;
	
	var xmlString = objectToXml(m0121RTMap);
	
	$("#reptNm").val("M0121.crf"); //레포트 파일명
...
function selectList(selectPkNum){
	...
	if(parent.viewType == 'inc'){
		m0121SCMap.rcptNumSc = incMap.rcptNum;
	}else{
		setAreaMap($("#searchArea"), m0121SCMap);
	}
	
	goAjax("/sjpb/M/M0121selectList.face", m0121SCMap, function (data){callBackSelectListSuccess(data, selectPkNum)},true);
}
...
function showList(data, selectPkNum){
	...
	var QCELLProp = {
            ...
            "columns"	: [
            	...
            	{width: '7%',	key: 'apprSiNum',				title: ['연번','연번'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '15%',	key: 'rcptIncNum',			title: ['통신제한조치 집행사건','사건번호'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '7%',	key: 'reqDt',				title: ['신청일자','신청일자'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '15%',	key: 'reqPersTitlNm',			title: ['신청자의관직·성명','신청자의관직·성명'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	...
            	{width: '7%',	key: 'handDtRst',			title: ['통신제한조치 집행사건','처리일자 및 처리결과'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '7%',	key: 'notcRecvDt',				title: ['통신제한조치 집행사건','통보받은일자'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '7%',	key: 'apprDt',				title: ['승인일자','승인일자'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '9%',	key: 'clarAftrNotcDt',				title: ['유예사유 해소후 통지일자','유예사유 해소후 통지일자'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize13'}},
        ...
        if (qcell.getRows("data") > 0) {
        	...
        	if(selectPkNum == null){	//selectPkNum이 없으면 첫번째줄 선택
        		rowIdx = headerRows;
        		//debugger;
        		selectData(qcell.getRowData(rowIdx).cmctRstrDelyApprNum);	//TODO 페이지별 수정필요
...
function insertData(){
	...
	//Map 데이터 갱신
	if(syncM0121VOMap(true)){	//TODO 페이지별 수정
		goAjax("/sjpb/M/M0121insertUpdateData.face", m0121VOMap, callBackInsertDataSuccess);
	}
}
...
function callBackInsertDataSuccess(data){
	...
	//성공
	if(data.m0121VO.result == "01"){
		alert("통신제한조치집행사실통지부가 등록되었습니다.");
		
		//rowIdx를 맨 위로 설정함. 
		rowIdx = headerRows;
		
		//리스트재조회
		selectList(data.m0121VO.cmctRstrDelyApprNum);
		
		
	}else{
		alert(data.m0121VO.errMsg);
...
function updateData(){
	//Map 데이터 갱신
	if(syncM0121VOMap(true)){
		goAjax("/sjpb/M/M0121insertUpdateData.face", m0121VOMap, callBackUpdateDataSuccess);
	}
}
...
function selectData(selectPkNum){
	var reqMap = {
			cmctRstrDelyApprNum : selectPkNum		//TODO 페이지마다 수정 
		}
	goAjaxDefault("/sjpb/M/M0121selectData.face", reqMap, callBackSelectDataSuccess);
}
...
function callBackSelectDataSuccess(data){
	...
	//데이터 셋팅
	getAreaMap($("#contentsArea"), m0121VOMap);
...
function handleUI(){
	...
	//신규 등록일 경우, 저장 버튼 노출 
	if(isNull(m0121VOMap.cmctRstrDelyApprNum)){	//TODO 페이지별 수정
	...
	//수정일 경우, 수정 버튼 노출 
		...
		if($("#userId").val() == m0121VOMap.regUserId){
			areaSetWrite($("#contentsArea"));
	...
	if(colIdx >0 && rowIdx > 0){
		if(colIdx != 1){
			...
			if (rowIdx > headerRows -1) {
				selectData(qcell.getRowData(rowIdx).cmctRstrDelyApprNum);	//TODO 페이지별 수정
			}
		}
	}
...
//Map 데이터 갱신
function syncM0121VOMap(isValid) {
	...
	//데이터셋팅
	setAreaMap($("#contentsArea"), m0121VOMap);
