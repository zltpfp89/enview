...
//출력 
function prnData(){
	var cmctCnfmRepyBkNumList = [];
	for(var i = 1; i <= qcell.getRows("data"); i++){
		if(JSON.stringify(qcell.getCellLabel(i,1)) == "true"){
			cmctCnfmRepyBkNumList.push(qcell.getRowData(i).cmctCnfmRepyBkNum);
		}	
	}
	var cmctCnfmRepyBkNumListMap = {
			"cmctCnfmRepyBkNumList":cmctCnfmRepyBkNumList
	}
	
	if(cmctCnfmRepyBkNumList.length != 0){
		//레포트 성공함수 호출
		goAjaxDefault("/sjpb/M/M0125prnCheckReport.face", cmctCnfmRepyBkNumListMap, callBackPrnDataSuccess);
	}else{
...
function callBackPrnDataSuccess(data){
	...
	m0125RTMap.rexdataset = data.rexdataset;
	
	var xmlString = objectToXml(m0125RTMap);
	
	$("#reptNm").val("M0125.crf"); //레포트 파일명
...
function selectList(selectPkNum){
	...
	if(parent.viewType == 'inc'){
		//해당사건번호만 노출함 
		m0125SCMap.rcptNumSc = incMap.rcptNum;
		
	}else{
		setAreaMap($("#searchArea"), m0125SCMap);
		
	}
	//2018.11.19 추가 E
	
	goAjax("/sjpb/M/M0125selectList.face", m0125SCMap, function (data){callBackSelectListSuccess(data, selectPkNum)},true);
}
...
function showList(data, selectPkNum){
	...
	var QCELLProp = {
            ...
            "columns"	: [
            	...
            	{width: '20%',	key: 'rcptIncNum',				title: ['사건번호'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '5%',	key: 'repyBkSiNum',				title: ['연번'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '10%',	key: 'reqDt',				title: ['요청일자'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '10%',	key: 'nmKor',			title: ['요청자'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '10%',	key: 'spNm',	title: ['대상자'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '10%',	key: 'dtaRnge',			title: ['제공받은 자료의 범위'], options:{limit:'number'},		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '10%',	key: 'repyDt',			title: ['회신일자'], options:{limit:'number'},		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '15%',	key: 'repyBkComn',				title: ['비고'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}}	                	
            ],
        ...
        if (qcell.getRows("data") > 0) {
        	...
        	if(selectPkNum == null){	//selectPkNum이 없으면 첫번째줄 선택
        		rowIdx = headerRows;
        		//debugger;
        		selectData(qcell.getRowData(rowIdx).cmctCnfmRepyBkNum);	//TODO 페이지별 수정필요
...
function insertData(){
	...
	//Map 데이터 갱신
	if(syncM0125VOMap(true)){	//TODO 페이지별 수정
		goAjax("/sjpb/M/M0125insertUpdateData.face", m0125VOMap, callBackInsertDataSuccess);
	}
}
...
function callBackInsertDataSuccess(data){
	...
	//성공
	if(data.m0125VO.result == "01"){
		alert("통신사실 확인자료 회신대장이 등록되었습니다.");
		
		//rowIdx를 맨 위로 설정함. 
		rowIdx = headerRows;
		
		//리스트재조회
		selectList(data.m0125VO.cmctCnfmRepyBkNum);
		
		
	}else{
		alert(data.m0125VO.errMsg);
...
function updateData(){
	//Map 데이터 갱신
	if(syncM0125VOMap(true)){
		goAjax("/sjpb/M/M0125insertUpdateData.face", m0125VOMap, callBackUpdateDataSuccess);
	}
}
...
function selectData(selectPkNum){
	var reqMap = {
			cmctCnfmRepyBkNum : selectPkNum		//TODO 페이지마다 수정 
		}
	goAjaxDefault("/sjpb/M/M0125selectData.face", reqMap, callBackSelectDataSuccess);
}
...
function callBackSelectDataSuccess(data){
	...
	//데이터 셋팅
	getAreaMap($("#contentsArea"), m0125VOMap);
...
function handleUI(){
	...
	//신규 등록일 경우, 저장 버튼 노출 
	if(isNull(m0125VOMap.cmctCnfmRepyBkNum)){	//TODO 페이지별 수정
	...
	//수정일 경우, 수정 버튼 노출 
		...
		if($("#userId").val() == m0125VOMap.regUserId && parent.viewType == 'inc'){
			areaSetWrite($("#contentsArea"));
	...
	if(colIdx >0 && rowIdx > 0){
		if(colIdx != 1){
			...
			if (rowIdx > headerRows -1) {
				selectData(qcell.getRowData(rowIdx).cmctCnfmRepyBkNum);	//TODO 페이지별 수정
			}
		}
	}
}
...
//Map 데이터 갱신
function syncM0125VOMap(isValid) {
	...
	//데이터셋팅
	setAreaMap($("#contentsArea"), m0125VOMap);
