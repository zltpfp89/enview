...
//출력 
function prnData(){
	var emgyCmctNotcSendNumList = [];
	for(var i = 2; i <= qcell.getRows("data")+1; i++){
		if(JSON.stringify(qcell.getCellLabel(i,1)) == "true"){
			emgyCmctNotcSendNumList.push(qcell.getRowData(i).emgyCmctNotcSendNum);
		}	
	}
	var emgyCmctNotcSendNumListMap = {
			"emgyCmctNotcSendNumList":emgyCmctNotcSendNumList
	}
	
	if(emgyCmctNotcSendNumList.length != 0){
		//레포트 성공함수 호출
		goAjaxDefault("/sjpb/M/M0119prnCheckReport.face", emgyCmctNotcSendNumListMap, callBackPrnDataSuccess);
	}else{
...
function callBackPrnDataSuccess(data){
	...
	m0119RTMap.rexdataset = data.rexdataset;
	
	var xmlString = objectToXml(m0119RTMap);
	
	$("#reptNm").val("M0119.crf"); //레포트 파일명
...
function selectList(selectPkNum){
	...
	if(parent.viewType == 'inc'){
		m0119SCMap.rcptNumSc = incMap.rcptNum;
	}else{
		setAreaMap($("#searchArea"), m0119SCMap);
	}
	
	goAjax("/sjpb/M/M0119selectList.face", m0119SCMap, function (data){callBackSelectListSuccess(data, selectPkNum)},true);
}
...
function showList(data, selectPkNum){
	...
	var QCELLProp = {
            ...
            "columns"	: [
            	...
            	{width: '10%',	key: 'emgyCmctSiNum',				title: ['연번','연번'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '15%',	key: 'rcptIncNum',			title: ['사건번호','사건번호'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '10%',	key: 'spNm',	title: ['긴급통신제한조치','대상자'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '15%',	key: 'rstrActnTrgtPi',			title: ['긴급통신제한조치','기간'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '15%',	key: 'notcFillPersPosi',			title: ['통보서작성자','직급'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '10%',	key: 'notcFillPersNm',			title: ['통보서작성자','성명'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '10%',	key: 'notcSendDt',				title: ['통보서송부일자','통보서송부일자'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize13'}},
        ...
        if (qcell.getRows("data") > 0) {
        	...
        	if(selectPkNum == null){	//selectPkNum이 없으면 첫번째줄 선택
        		rowIdx = headerRows;
        		//debugger;
        		selectData(qcell.getRowData(rowIdx).emgyCmctNotcSendNum);	//TODO 페이지별 수정필요
...
function insertData(){
	...
	//Map 데이터 갱신
	if(syncM0119VOMap(true)){	//TODO 페이지별 수정
		goAjax("/sjpb/M/M0119insertUpdateData.face", m0119VOMap, callBackInsertDataSuccess);
	}
}
...
function callBackInsertDataSuccess(data){
	...
	//성공
	if(data.m0119VO.result == "01"){
		alert("긴급통신제한조치통보서발송부가 등록되었습니다.");
		
		//rowIdx를 맨 위로 설정함. 
		rowIdx = headerRows;
		
		//리스트재조회
		selectList(data.m0119VO.emgyCmctNotcSendNum);
		
		
	}else{
		alert(data.m0119VO.errMsg);
...
function updateData(){
	//Map 데이터 갱신
	if(syncM0119VOMap(true)){
		goAjax("/sjpb/M/M0119insertUpdateData.face", m0119VOMap, callBackUpdateDataSuccess);
	}
}
...
function selectData(selectPkNum){
	var reqMap = {
			emgyCmctNotcSendNum : selectPkNum		//TODO 페이지마다 수정 
		}
	goAjaxDefault("/sjpb/M/M0119selectData.face", reqMap, callBackSelectDataSuccess);
}
...
function callBackSelectDataSuccess(data){
	...
	//데이터 셋팅
	getAreaMap($("#contentsArea"), m0119VOMap);
...
function handleUI(){
	...
	//신규 등록일 경우, 저장 버튼 노출 
	if(isNull(m0119VOMap.emgyCmctNotcSendNum)){	//TODO 페이지별 수정
		...
		btnHtml.append('<a href="javascript:insertData();" class="btn_light_blue"><span>저장</span></a>');
		
	//수정일 경우, 수정 버튼 노출 
	}else{
		//로그인한 계정이 입력한 데이터일 경우 수정가능 
		if($("#userId").val() == m0119VOMap.regUserId){
			areaSetWrite($("#contentsArea"));
			btnHtml.append('<a href="javascript:updateData();" class="btn_light_blue"><span>수정</span></a>');
	...
	if(colIdx >0 && rowIdx > 0){
		if(colIdx != 1){
			...
			if (rowIdx > headerRows -1) {
				selectData(qcell.getRowData(rowIdx).emgyCmctNotcSendNum);	//TODO 페이지별 수정
			}
		}
	}
}
...
//Map 데이터 갱신
function syncM0119VOMap(isValid) {
	...
	//데이터셋팅
	setAreaMap($("#contentsArea"), m0119VOMap);
