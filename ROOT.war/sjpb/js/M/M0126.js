...
//출력 
function prnData(){
	var cmctCnfmDelyNotcNumList = [];
	for(var i = 2; i <= qcell.getRows("data")+1; i++){
		if(JSON.stringify(qcell.getCellLabel(i,1)) == "true"){
			cmctCnfmDelyNotcNumList.push(qcell.getRowData(i).cmctCnfmDelyNotcNum);
		}	
	}
	var cmctCnfmDelyNotcNumListMap = {
			"cmctCnfmDelyNotcNumList":cmctCnfmDelyNotcNumList
	}
	
	if(cmctCnfmDelyNotcNumList.length != 0){
		//레포트 성공함수 호출
		goAjaxDefault("/sjpb/M/M0126prnCheckReport.face", cmctCnfmDelyNotcNumListMap, callBackPrnDataSuccess);
	}else{
...
function callBackPrnDataSuccess(data){
	...
	m0126RTMap.rexdataset = data.rexdataset;
...
function selectList(selectPkNum){
	...
	if(parent.viewType == 'inc'){
		//해당사건번호만 노출함 
		m0126SCMap.rcptNumSc = incMap.rcptNum;
	...
	goAjax("/sjpb/M/M0126selectList.face", m0126SCMap, function (data){callBackSelectListSuccess(data, selectPkNum)},true);
}
...
function showList(data, selectPkNum){
	...
	var QCELLProp = {
            ...
            "columns"	: [
            	...
            	{width: '5%',	key: 'notcSiNum',				title: ['연번','연번'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	...
            	{width: '9%',	key: 'notcTrgtPers',			title: ['통신사실확인 자료제공요청','통지 대상자'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	...
            	{width: '10%',	key: 'handDtRst',			title: ['통신사실 확인자료제공요청 집행사건','처리일자 및 처리결과'], options:{limit:'number'},		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '7%',	key: 'notcRecvDt',				title: ['통신사실 확인자료제공요청 집행사건','처리결과를 통보받은 일자'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize13'}},	                	
            	{width: '9%',	key: 'notcDt',			title: ['통지일자','통지일자'],		sort:true, move:true, resize: true, options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}}, styleclassname: {data: 'aligncenter fontsize13'}},
...
function callBackInsertDataSuccess(data){
	...
	if(data.m0126VO.result == "01"){
		alert("통신사실 확인자료제공 요청 집행사실통지부가 등록되었습니다.");
...
function handleUI(){
	...
	//수정일 경우, 수정 버튼 노출 
		...
		if($("#userId").val() == m0126VOMap.regUserId && parent.viewType == 'inc'){
			areaSetWrite($("#contentsArea"));
...
function requestReport(){
	$("#reptNm").val("M0206.crf"); //레포트 파일명
	$("#SEQNUM").val(m0126VOMap.cmctCnfmDelyNotcNum);
	//레포트 호출
