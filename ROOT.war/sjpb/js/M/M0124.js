...
//출력 
function prnData(){
	var cmctCnfmReqExecNumList = [];
	for(var i = 2; i <= qcell.getRows("data")+1; i++){
		if(JSON.stringify(qcell.getCellLabel(i,1)) == "true"){
			cmctCnfmReqExecNumList.push(qcell.getRowData(i).cmctCnfmReqExecNum);
		}	
	}
	var cmctCnfmReqExecNumListMap = {
			"cmctCnfmReqExecNumList":cmctCnfmReqExecNumList
	}
	
	if(cmctCnfmReqExecNumList.length != 0){
		//레포트 성공함수 호출
		goAjaxDefault("/sjpb/M/M0124prnCheckReport.face", cmctCnfmReqExecNumListMap, callBackPrnDataSuccess);
	}else{
...
function callBackPrnDataSuccess(data){
	...
	m0124RTMap.rexdataset = data.rexdataset;
	
	var xmlString = objectToXml(m0124RTMap);
	
	$("#reptNm").val("M0124.crf"); //레포트 파일명
...
function selectList(selectPkNum){
	...
	if(parent.viewType == 'inc'){
		//해당사건번호만 노출함 
		m0124SCMap.rcptNumSc = incMap.rcptNum;
		
	}else{
		setAreaMap($("#searchArea"), m0124SCMap);
		
	}
	//2018.11.19 추가 E
	
	goAjax("/sjpb/M/M0124selectList.face", m0124SCMap, function (data){callBackSelectListSuccess(data, selectPkNum)},true);
}
...
function showList(data, selectPkNum){
	...
	var QCELLProp = {
            ...
            "columns"	: [
            	...
            	{width: '100',	key: 'recvNm',			title: ['수신처','수신처'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
            	...
            	{width: '40',	key: 'execNum',				title: ['집행번호','집행번호'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '100',	key: 'apprDocNum',				title: ['허가서 번호','허가서 번호'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '100',	key: 'spNm',			title: ['성명','성명'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '100',	key: 'cmctCnfmExecTrgt',			title: ['통신사실 확인자료제공 요청집행','대상'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '100',	key: 'cmctCnfmExecType',	title: ['통신사실 확인자료제공 요청집행','종류'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '100',	key: 'cmctCnfmExecRnge',			title: ['통신사실 확인자료제공 요청집행','범위'], options:{limit:'number'},		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '100',	key: 'cmctCnfmExecDt',			title: ['집행일시','집행일시'], options: {format: {type:"date",origin:"YYYY-MM-DD hh:mm:ss", rule: "YYYY.MM.DD"}},		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '100',	key: 'cmctCnfmComn',				title: ['비고','비고'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},	                	
        ...
        if (qcell.getRows("data") > 0) {
        	...
        	if(selectPkNum == null){	//selectPkNum이 없으면 첫번째줄 선택
        		rowIdx = headerRows;
        		//debugger;
        		selectData(qcell.getRowData(rowIdx).cmctCnfmReqExecNum);	//TODO 페이지별 수정필요
...
function insertData(){
	...
	//Map 데이터 갱신
	if(syncM0124VOMap(true)){	//TODO 페이지별 수정
		goAjax("/sjpb/M/M0124insertUpdateData.face", m0124VOMap, callBackInsertDataSuccess);
	}
}
...
function callBackInsertDataSuccess(data){
	...
	//성공
	if(data.m0124VO.result == "01"){
		alert("통신사실 확인자료제공 요청집행대장이 등록되었습니다.");
		
		//rowIdx를 맨 위로 설정함. 
		rowIdx = headerRows;
		
		//리스트재조회
		selectList(data.m0124VO.cmctCnfmReqExecNum);
		
		
	}else{
		alert(data.m0124VO.errMsg);
...
function updateData(){
	//Map 데이터 갱신
	if(syncM0124VOMap(true)){
		goAjax("/sjpb/M/M0124insertUpdateData.face", m0124VOMap, callBackUpdateDataSuccess);
	}
}
...
function selectData(selectPkNum){
	var reqMap = {
			cmctCnfmReqExecNum : selectPkNum		//TODO 페이지마다 수정 
		}
	goAjaxDefault("/sjpb/M/M0124selectData.face", reqMap, callBackSelectDataSuccess);
}
...
function callBackSelectDataSuccess(data){
	...
	//데이터 셋팅
	getAreaMap($("#contentsArea"), m0124VOMap);
...
function handleUI(){
	...
	//신규 등록일 경우, 저장 버튼 노출 
	if(isNull(m0124VOMap.cmctCnfmReqExecNum)){	//TODO 페이지별 수정
	...
	//수정일 경우, 수정 버튼 노출 
		...
		if($("#userId").val() == m0124VOMap.regUserId && parent.viewType == 'inc'){
			areaSetWrite($("#contentsArea"));
	...
	if(colIdx >0 && rowIdx > 0){
		if(colIdx != 1){
			...
			if (rowIdx > headerRows -1) {
				selectData(qcell.getRowData(rowIdx).cmctCnfmReqExecNum);	//TODO 페이지별 수정
			}
		}
	}
}
...
//Map 데이터 갱신
function syncM0124VOMap(isValid) {
	...
	//데이터셋팅
	setAreaMap($("#contentsArea"), m0124VOMap);
...
function requestReport(){
	$("#reptNm").val("M0205.crf"); //레포트 파일명
	$("#SEQNUM").val(m0124VOMap.cmctCnfmReqExecNum);
	//레포트 호출
	openReportServiceFSS(reportForm); 
}

//초기화
function initM0124VOMap(){
	m0124VOMap = {
		cmctCnfmReqExecNum : ""
		,execNum : ""
		,apprDocNum : ""
