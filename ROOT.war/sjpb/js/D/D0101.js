...
function callBackFn_d_selectSpVioClaListSuccess(data){
	if(data.vioList == ""){ //위반법률에 대한 데이터가 없을때
		...
		$.each(data.vioList,function(index,vio){
			...
			vioHtml.append("		</li>");	
			vioHtml.append("		<li>");	
			vioHtml.append("			<span class='title'>위반유형</span>");
			vioHtml.append("			<label for='actVioTypeNm'></label>");
			$.each(vio.vioTypeList,function(i,vioType){
				vioHtml.append(vioType.actVioTypeNm);
				if(i+1 != vio.vioTypeList.length){
					vioHtml.append(", ");
				}
			});
			...
			vioHtml.append(vio.actVioClaNm);
