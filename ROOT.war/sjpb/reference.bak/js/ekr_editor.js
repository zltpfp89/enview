...
ekr.file.prototype = {	
	...
	edit_init : function() {
		...
		ekrFile.limitCount  = document.editForm.maxFileCnt.value;
		...
		if( ekrFile.smarteditor) {
			...
			var skinUrl= contextPath + "/board/smarteditor/SmartEditor2NoImageSkin.html";	//사진업로드 불가능한 스마트 에디터 스킨
			
			if( ekrFile.limitCount > 0 && document.setUpload) {	//파일첨부가 가능한 상태이면...
				skinUrl= contextPath + "/board/smarteditor/SmartEditor2SkinEkr.html";	//사진업로드 가능한 스마트 에디터 스킨
			}
			...
			nhn.husky.EZCreator.createInIFrame({
				...
				fOnAppLoad : function(){
					 ...
					 * Date		:	2016.07.07
					 * name 	:	Developwon
					 * subject	:	smartEditor 기본폰트 변경
					 */
					/*
					var sDefaultFont = "돋움"; 
				    var nFontSize = 10;
				    oEditors.getById["editorCntt"].setDefaultFont(sDefaultFont, nFontSize);
				    */
					try {
	...
	handleUpload : function ( text, val) {
		var html = "<li data='" + val + "'><img src=\"/gcgf/reference/images/sub/icon_file.png\" alt=\"" + text + "\" /> " + text + "</li>";
	...
	setForm : function () {
		...
		//파일 찾기만하고 추가버튼을 누르지않았을경우 예외 처리
		if(document.getElementById("selectFileName")){
			var inputFile = document.getElementById("selectFileName");

			if(fn_gcgfMSieversion() > 6 && fn_gcgfMSieversion() < 10 ){
			    if(inputFile.value != "") {
			    	if(!confirm("추가되지않은 파일이 존재합니다. 그래도 저장하시겠습니까?")){
			    		ekrFile.isCancel = true;
			    		return;
			    	}
			    }
