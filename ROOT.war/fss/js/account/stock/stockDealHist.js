$(document).ready(function(){
	...
	$("#exceldownIncPrnBtn").click(function(){
		if(qcellStockDealHist.getRows("data") == "0"){
...
//			url : "/excelDownload.face",
		...
		qcellStockDealHist.excelDownload(properties);
	});
});

var rowIdx;
var qcellStockDealHist;
var stockDealHistVOMap = new Object();
...
function selectList(){
	//최소화 되어 있으면 검색중단
	if(!$('#sheetStockDealHist:visible').length) return;
	
	stockDealHistVOMap.srchCaseNo = $("#srchCaseNo").val();
	stockDealHistVOMap.srchStartDate = $("#srchStartDate").val();
	stockDealHistVOMap.srchEndDate = $("#srchEndDate").val();
	stockDealHistVOMap.dateDiv = $("input[name=dateDiv]").val();
	stockDealHistVOMap.srchDiv = $("#srchDiv").val();
	stockDealHistVOMap.srchMembNo = $("#srchMembNo").val();
	stockDealHistVOMap.srchBranchNo = $("#srchBranchNo").val();
	stockDealHistVOMap.srchAccNo = $("#srchAccNo").val();

	goAjax("/stock/selectStockDealHistList.face", stockDealHistVOMap, callBackSelectListSuccess);
}

//사건관리 화면(리스트)조회 콜백함수
function callBackSelectListSuccess(data){
	if(data.stockDealHistList){
		var QCELLProp = {
	        "parentid" : "sheetStockDealHist",
	        "id"		: "qcellStockDealHist",
	        "rowheight" : {header: 35, data: 30},
	        "data"		: {"input" : data.stockDealHistList},
	        "selectmode": "row",
	        "columns"	: [
	...
	//			{width: '100',	key: 'branchNo',			title: ['BRANCH_NO'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize18'}},
		...
		qcellStockDealHist = QCELL.getInstance("qcellStockDealHist");
		qcellStockDealHist.bind("click", eventFn);
	    
		qcellStockDealHist.setRowStyle(1, {"background-color" : "#c1c8e8", "border-color" : "#a8b0d4"}, "data");   	 
		
		$("#exceldownIncPrnBtn").show();
	} else {
		$("#sheetStockDealHist").html(data.msg);
...
function eventFn(e){
	//선택한 인덱스 가져오기
	rowIdx = qcellStockDealHist.getIdx("row");
	
	//기존 체크항목 해제와 새로 선택한 행 체크박스 선택
	qcellStockDealHist.removeRowStyle(qcellStockDealHist.getIdx("row","focus","previous") == -1?1:qcellStockDealHist.getIdx("row","focus","previous"));
	qcellStockDealHist.setRowStyle(rowIdx, {"background-color" : "#c1c8e8 ", "border-color" : "#a8b0d4 "}, "data");
}
