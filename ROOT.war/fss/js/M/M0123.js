...
//출력 
function prnData(){
	goAjax("/sjpb/M/M0123selectList.face", m0123SCMap, callBackPrnDataSuccess);
}
...
function callBackPrnDataSuccess(data){
	...
	m0123RTMap.rexdataset = data.qCell;
...
function selectList(selectPkNum){
	...
	goAjax("/sjpb/M/M0123selectList.face", m0123SCMap, function (data){callBackSelectListSuccess(data, selectPkNum)});
}
...
function showList(data, selectPkNum){
	...
	var QCELLProp = {
            ...
            "columns"	: [
            	...
            	{width: '20%',	key: 'emgyCmctReqRnge',			title: ['통신사실 확인자료제공 요청집행','범위'], options:{limit:'number'},		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '10%',	key: 'execDt',			title: ['집행일시','집행일시'], options:{limit:'number'},		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},
            	{width: '15%',	key: 'notcDocSendYnDesc',				title: ['사후청구 또는 통보서 발송여부','사후청구 또는 통보서 발송여부'],		sort:true, move:true, resize: true, styleclassname: {data: 'aligncenter fontsize13'}},	                	
...
function handleUI(){
	...
	//수정일 경우, 수정 버튼 노출 
		...
		if($("#userId").val() == m0123VOMap.regUserId){
			areaSetWrite($("#contentsArea"));
