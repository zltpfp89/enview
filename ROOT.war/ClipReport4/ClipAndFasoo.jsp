...
<%
	...
	String Systemid = "SJPB";
	String drm_fsdinit_path_fsd = Enview.getConfiguration().getString("sjpb.drm.key.path", "/DATA/FasooDRM/fsdinit"); // key 폴더 경로
	String domain_id_fsd = Enview.getConfiguration().getString("sjpb.drm.domain.id", "0100000000001610"); // dsdcode
	String Fileid = ""; // 파일 아이디
	int iErrCheck = 0;
	String sErrMessage = "";
...
//response.setHeader("Cache-Control", "max-age=0");
	...
	if (null != passName) {
		if ("R01".equals(passName)) {
			...
			boolean iret = false;
			String ACLID = "II";
			String doExtraPackingFLag = "N";
			String temLeaderId = null;
			
			List teamLeaderList = (List)userInfoMap.get("teamLeaderList");
			
			if(teamLeaderList.size() > 0){
				if(teamLeaderList.contains((String)userInfoMap.get("userId"))){
					doExtraPackingFLag ="Y";
				}
				temLeaderId = teamLeaderList.toString();
				if(temLeaderId != null){
					temLeaderId = temLeaderId.replaceAll("\\[", "").replaceAll("\\]", "").replaceAll(" ", "");
				}
			}
			
			WorkPackager wPackager = new WorkPackager();
			wPackager.setOverWriteFlag(true);
			iret = wPackager.DoPackaging(drm_fsdinit_path_fsd, domain_id_fsd, exportInfo.getExportfilePath(),exportInfo.getExportfilePath(), downloadFile.getName(),  (String)userInfoMap.get("userId"), (String)userInfoMap.get("nmKor"), Systemid, ACLID, Fileid, doExtraPackingFLag,temLeaderId);
			...
			//생성한 파일을 연동문제로 삭제를 못 할 경우 프로그래스만 삭제가능
			//ClipReportExport.deleteToProgress(request, exportInfo);
