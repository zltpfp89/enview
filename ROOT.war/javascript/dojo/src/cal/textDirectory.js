...
dojo.cal.textDirectory.Property = function(line){var left = dojo.string.trim(line.substring(0, line.indexOf(':')));
...
this.params = [];
var arr;
for(var i = 0; i < parameters.length; i++){arr = parameters[i].split("=");
...
for(var j = 0; j < values.length; j++){if(dojo.string.trim(values[j]) != ''){this.params.push([key, dojo.string.trim(values[j])]);
}}
}
if(this.name.indexOf('.') > 0){arr = this.name.split('.');
this.group = arr[0];
this.name = arr[1];
}
this.value = right;
...
var properties = [];
for(var i = 0; i < lines.length; i++){if(dojo.string.trim(lines[i]) == ''){ continue; }
