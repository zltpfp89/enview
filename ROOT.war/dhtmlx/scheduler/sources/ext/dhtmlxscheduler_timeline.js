...
scheduler.render_timeline_event = function(ev, attach){
	...
	if (scheduler.config.drag_resize && !scheduler.config.readonly) {
		...
		var resize_start = "<div class='"+dhx_event_resize+" "+dhx_event_resize+"_start' style='height: "+hb+"px;'></div>";
		var resize_end = "<div class='"+dhx_event_resize+" "+dhx_event_resize+"_end' style='height: "+hb+"px;'></div>";
	...
	if (!attach)
		...
		var parentSection = scheduler._els["dhx_cal_data"][0].firstChild.rows[ind];
		if(parentSection){
			var parent = parentSection.cells[1].firstChild;
...
function x_scale(h){
	...
	var dates = x_dates.call(this, preserve);
...
function reset_scale_height(mode){
...
function set_full_view(mode){
	reset_scale_height.call(this, mode);
