...
hancer.rss.prototype = {
	...
	popupCategory : function() {
		window.open(this.m_contextPath + '/hancer/rss/categoryList.hanc','categories', "width=700px, height=400px, scrollbars=yes, resize=yes" );
	}
}
...
hancer.rss.feed = function(parent) {
	...
	this.m_listUrl	 =	"/hancer/rss/feedList.hanc",
	this.m_detailUrl =	"/hancer/rss/detailFeed.hanc";
	this.m_insertUrl =	"/hancer/rss/insertFeed.hanc";
	this.m_updateUrl =	"/hancer/rss/updateFeed.hanc";
	this.m_deleteUrl =	"/hancer/rss/deleteFeeds.hanc";
...
//		}
	...
	uiForm : function(){
		var param = "feedId=-1";
		param += "&domainId=" + enRss.getDomain();
	...
	update : function(){
		...
		param += "&url=" + feedUrl.val();
		param += "&homepage=" + $('#homepage').val();
		this.m_ajax.send("POST", this.m_contextPath + actionUrl, param, false, {success: function(data) {
...
hancer.rss.entry = function(parent) {
	this.m_parent = parent;
	this.m_ajax = parent.m_ajax;
	this.m_listUrl	 =	"/hancer/rss/entryList.hanc",
	this.m_openUrl =	"/hancer/rss/openEntries.hanc";
	this.m_closeUrl =	"/hancer/rss/closeEntries.hanc";
}
hancer.rss.entry.prototype = {
	...
	uiList : function(){
		...
		var param = "currentPage=" + this.m_currentPage;
		param += "&status=" + this.m_searchType;
		param += "&searchValue=" + this.m_searchValue;
		...
		this.m_ajax.send("POST", this.m_contextPath + this.m_listUrl, param, true, {success: function(htmlData) { 
			...
			$('#entrySearchValue').select();
		}});
	},
	
	closeEntries : function() {
		var ids = ''; 
		$('input[name=entryCheck]').each(function(){
			var $this = $(this);
			if($this.is(':checked')){
				ids += $this.val() + ',';
			}
		...
		if(!confirm("선택된 Entry들을 비공개 하시겠습니까?")) return;
		ids = ids.substring(0, ids.length-1);
		var param = "entryIds=" +ids;
		this.m_ajax.send("POST", this.m_contextPath + this.m_closeUrl, param, true, {success: function(htmlData) { 
			alert(htmlData);
			enEntry.uiList();
		}});
	},
	...
	openEntries : function() {
		...
		ids = ids.substring(0, ids.length-1);
		var param = "entryIds=" +ids;
		this.m_ajax.send("POST", this.m_contextPath + this.m_openUrl, param, true, {success: function(htmlData) { 
			alert(htmlData);
			enEntry.uiList();
		}});
	}
...
hancer.rss.category.prototype = {
	...
	update : function(categoryId) {
		if(!confirm('[' + $('#org_categoryName_' + categoryId).val() + '] 카테고리를  [' + $('#categoryName').val() + ']로 수정하시겠습니까?')) return;
