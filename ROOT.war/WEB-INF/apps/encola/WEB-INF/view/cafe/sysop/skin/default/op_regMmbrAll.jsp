...
<%
request.setAttribute( "isAdmin", "admin".equals(EnviewSSOManager.getUserId(request)));
%>
	  ...
	  <table width="100%">
	    <tr>
		  ...
		  <td align="right">
			...
			<img src="<%=request.getContextPath()%>/cola/cafe/images/button/btn_search.gif" hspace="2" align="absmiddle" style="cursor:hand" onclick="cfOp.regMmbr.search('all_srchForm')"/>
	  ...
	  <table id="grid-table" width="100%" border="0" cellspacing="0" cellpadding="0">
		<thead>
		  ...
		  <tr style="cursor: pointer;">
			...
			<th class="cafegridheader" onclick="cfOp.regMmbr.sort('all_srchForm','reg_datim')"><span><util:message key="cf.title.joinDate"/></span>
			...
			<th class="cafegridheader" onclick="cfOp.regMmbr.sort('all_srchForm','last_access')"><span><span><util:message key="cf.title.last.visited"/></span>
			  <img src="<%=request.getContextPath()%>/cola/cafe/images/<c:if test="${opForm.sortMethod!='ASC'}">tiny_down_arrow.gif</c:if><c:if test="${opForm.sortMethod=='ASC'}">tiny_up_arrow.gif</c:if>" style="margin-left:5px"/></span>
			</th>	
			<th class="cafegridheader" onclick="cfOp.regMmbr.sort('all_srchForm','visit_cnt')"><span>방문횟수</span>
			  <img src="<%=request.getContextPath()%>/cola/cafe/images/<c:if test="${opForm.sortMethod!='ASC'}">tiny_down_arrow.gif</c:if><c:if test="${opForm.sortMethod=='ASC'}">tiny_up_arrow.gif</c:if>" style="margin-left:5px"/></span>
			</th>	
			<th class="cafegridheaderlast" onclick="cfOp.regMmbr.sort('all_srchForm','bltn_cnt')"><span><util:message key="eb.title.bltnCnt"/></span>
			  <img src="<%=request.getContextPath()%>/cola/cafe/images/<c:if test="${opForm.sortMethod!='ASC'}">tiny_down_arrow.gif</c:if><c:if test="${opForm.sortMethod=='ASC'}">tiny_up_arrow.gif</c:if>" style="margin-left:5px"/></span>
			</th>	
		  </tr>
		</thead>
		  ...
		  <tr ch="<c:out value="${status.index}"/>" style="cursor:pointer;" class="<c:if test="{$status % 2 != 0}">cafegridline</c:if>">
			...
			<td align="left" class="cafegrid">
			  <span style="margin-left:10px">
				...
				<c:if test="${isAdmin && mmbr.mmbrGrd != 'X'}">
				<input type="button" value="<util:message key='cf.info.appoint.sysop'/>" onclick="cfOp.regMmbr.setCafeAdmin('<c:out value="${mmbr.userId}"/>'')"/>
				</c:if>
			  </span>
			</td>
		  ...
		  <tr style="cursor:pointer;">
		    <td colspan="100">
			  ...
			  <span style="margin-left:10px">
				...
				<input type="button" value="변경" onclick="cfOp.regMmbr.changeGrade('all')"/>
			  </span>
			  <span style="margin-left:30px">
				<input type="button" value="활동중지" onclick="cfOp.regMmbr.checkChangeState('all','40')"/>
				<input type="button" value="강제탈퇴" onclick="cfOp.regMmbr.checkChangeState('all','51')"/>
			  </span>
			</td>
		  </tr>
		</tbody>
