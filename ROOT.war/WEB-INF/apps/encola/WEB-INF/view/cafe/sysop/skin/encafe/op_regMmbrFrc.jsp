...
<form id="inv_srchForm" name="inv_srchForm" method="POST" onsubmit="return false;">
	<div style="width:680px; height:24px;">
		<div class="board_select_box fl"><span class="oran_title"><util:message key='cf.title.approval.wait'/></span></div>
		<!-- 검색 -->
	...
	<div class="board_line_box">
		<div class="fl" style=" width:280px; height:20px;"> * <util:message key="cf.title.mmbrCnt"/> : <span class="g12_num"><c:out value="${cmntVO.mmbrTotCF}"/></span>&nbsp;/&nbsp;<util:message key='cf.title.currentViews'/> : <span class="g12_num"><c:out value="${opForm.totalSize}"/></span></div>
...
<form id="inv_listForm" name="inv_listForm" style="display:inline" action="" method="post" onsubmit="return false;">
	...
	<div class="table_box">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr class="table_33_line">
				...
				<td width="20%" class="gray_b_text"><util:message key='ev.title.user.Username'/></td>
				<td class="table_space"></td>
				<td width="21%" class="gray_b_text"><util:message key='ev.title.department'/></td>
				<td class="table_space"></td>
				<td width="21%" class="gray_b_text"><util:message key='cf.prop.position'/></td>
				<td class="table_space"></td>
				<td width="15%" class="gray_b_text"><util:message key='ev.prop.batchResult.status'/></td>
				<td class="table_space"></td>
				<td width="15%" class="gray_b_text"><util:message key='cf.title.applicationDate'/></td>
			</tr>
			<c:if test="${empty mmbrList}" var="isMmbrEmpty">
			<tr class="list_line">
					<td colspan="15" height="30" align="center">
						<util:message key='cf.error.not.exist.approval.member'/>
					</td>
				</tr>
			</c:if>
			<c:if test="${!isMmbrEmpty}">
				...
				<tr ch="<c:out value="${count}"/>" class="list_line">
					...
					<td width="21%" height="30" align="center"><util:message key='cf.prop.seoul'/></td>
					<td height="30">&nbsp;</td>
	...
	<!-- 이동조건 -->
		...
		<c:if test="${langKnd eq 'en' }">
			<div class="board_line_box gray_db_text">* <img src="<%=request.getContextPath()%>/kcomwel/Community/images/common/but/but_sg_approve.gif" alt="가입승인" title="가입승인" class="btn h4_space" onclick="javascript:cfKcomwel.checkChangeState('inv','20', 'kcomwel_cafe')"/><img src="<%=request.getContextPath()%>/kcomwel/Community/images/common/but/but_sg_del.gif" alt="삭제" title="삭제" class="btn h4_space" onclick="javascript:cfKcomwel.checkChangeState('inv','21', 'kcomwel_cafe')"/>&nbsp;</div>
			the selected member &nbsp;</div>
		</c:if>
	<!-- 페이지 넘버링 박스 시작 -->
