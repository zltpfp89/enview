		...
		<div id="wrap">
						...
						<form method="post" id="srchCafeForm" name="srchCafeForm" onsubmit="cfHome.searchCafeList(); return false;">
							<div class="searchBg">
								<select id="srchType" name="srchType">
									<option value="nm" <c:if test="${homeForm.srchType == 'nm'}">selected</c:if>>카페이름</option>
									<option value="tag" <c:if test="${homeForm.srchType == 'tag'}">selected</c:if>>카페태그</option>
								</select>
			...
			<div id="gnb">
				<ul>
					...
					<li><a href="javascript:cfHome.initCafeHome('cate.init')">카테고리</a></li>
				</ul>
			</div>
			<!-- gnb//-->
			<!-- container -->
