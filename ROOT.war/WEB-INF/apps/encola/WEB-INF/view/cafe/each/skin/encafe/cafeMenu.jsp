		...
		<c:if test="${ mmbrVO.isSysop || mmbrVO.isAdmin || isHide == false }">
			...
			<c:if test="${ isMenu }">
				<div class="cafe_menu_row cafe_menu_menu cafe_menu_type_<c:out value="${ menu.menuLevel }"/>" parentId="<c:out value="${ menu.parentId }"/>" title="<c:out value="${ menu.menuDesc }"/>">
					<a class="CF0401_baseFontColor CF0402_txtIndnt CF0402_pddngLft CF0402_menuIcon<c:out value="${ menu.cnttType }"/> cafe_menu_link cafe_menu_cntt" id="menu_link_<c:out value="${ menu.menuId }"/>" menuId="<c:out value="${ menu.menuId }"/>" boardId="<c:out value="${ menu.boardId }"/>" onclick="javascript:cfTitle.selectMenu('<c:out value="${ menu.menuId }"/>','<c:out value="${ menu.boardId }"/>')"><c:out value="${ menu.menuNm }"/></a>
					<c:if test="${menu.newBltn}"><div class="cafe_menu_new CF0402_newIcon" > </div> </c:if>
				</div>
			</c:if>
			<c:if test="${ isSep }">
