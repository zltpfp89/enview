...
<div class="board">
	...
	<div class="tsearchArea">
		...
		<c:if test="${boardVO.srchRegYn == 'Y'}">
		  <input type="checkbox" name="srchType" id="srchType" value="RegD"  <c:if test="${boardSttVO.srchType == 'RegD'}">checked</c:if> />작성일
		  <input type=text id=srchBgnReg value=<c:out value="${boardSttVO.srchBgnReg}"/> class="tb_input datepicker" style="width:70" readonly="true">
          <input type=text id=srchEndReg value=<c:out value="${boardSttVO.srchEndReg}"/> class="tb_input datepicker" style="width:70" readonly="true">
		</c:if>			
			<fieldset>
				<div class="inputbox sel_150">
					<p class="txt"></p>
				...
				<div class="searchbtn"><a href="/board/list.brd?boardId=${boardVO.boardId}" class="btn_white"><span>초기화</span></a><a href="#search" onClick="ebList.srchBulletin();" class="btn_gray"><span>검색</span></a></div>
			</fieldset>    
		</form>
		<!-- search -->
	</div>
	  ...
	  <table cellpadding="0" cellspacing="0" summary="게시판리스트" class="list web" > 
		...
		<thead>
		    <tr>
	          ...
	          <c:if test="${extnMapper.ext_str0Yn == 'Y' && extnMapper.ext_str0TtlYn == 'Y'}">  
	          	<th class="C"><span class="table_title"><c:out value="${extnMapper.ext_str2Ttl}"/></span></th> 
	          </c:if>
          ...
          <td class="L">
			...
			<c:if test="${list.delFlag == 'N'}"><%--삭제표시되지 않은 정상적인 글일때만--%>
			  ...
			  <a style=cursor:pointer OnMouseOver=this.style.textDecoration='underline' OnMouseOut=this.style.textDecoration='none'
			    ...
			    <span class="title">
			      <c:if test="${extnMapper.ext_str0Yn == 'Y' && extnMapper.ext_str0TtlYn == 'Y'}">  
			    	<span style="font-weight: bold;"><c:if test="${!empty list.bltnExtnVO}">[<c:out value="${extnVO.ext_str0}"/>]</c:if></span>
			    </c:if>
			    <c:out value="${list.bltnOrgSubj}"/></span>
...
<!--       </form> -->
    ...
    <div class="btnArea">
		...
		<div class="R">
		<a class="btn_white" href ="/board/list.brd?boardId=${boardVO.boardId}"><span>목록</span></a>
			<c:if test="${boardVO.writableYn == 'Y'}">
		<c:if test="${boardVO.mergeType == 'A'}">
		<a class="btn_gray" onClick=ebList.writeBulletin() style=cursor:pointer><span>글쓰기</span></a>
			</c:if>
		</c:if>
		</div>
