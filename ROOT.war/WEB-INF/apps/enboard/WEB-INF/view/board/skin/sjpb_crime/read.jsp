		...
		<table cellpadding="0" cellspacing="0" summary="게시판뷰" class="list" >  
		    ...
		    <tbody>
				...
				<c:if test="${boardVO.extUseYn == 'Y' && !empty list.bltnExtnVO}">
				 ...
				 <c:if test="${boardVO.extUseYn == 'Y' && rsExtnMapper.ext_str0Yn == 'Y'}">
				 <tr>
					<th class="C first td_line" scope="col"><span class="table_title"><c:out value="${rsExtnMapper.ext_str0Ttl}"/></span></th>
					<td class="L td_line" colspan="4"><c:if test="${!empty list.bltnExtnVO}"><c:out value="${rsExtnVO.ext_str0}"/></c:if></td>				 
				</tr>
				</c:if>
			   	<tr>
