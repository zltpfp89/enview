		...
		<c:if test="${empty aaForm.view || aaForm.view == 'list'}">
			<div class="board first">
				<div class="tsearchArea">
					...
					<fieldset>
						<form name="frmBTL" method="post" action="" onsubmit="return false">
							...
							<input type="text" name="srchTableNm" class="txt_100" onfocus="ebUtil.whenSrchFocus(this,'*테이블 어근명*');" onblur="ebUtil.whenSrchBlur(this,'*테이블 어근명*');"
								<c:if test="${empty aaForm.srchTableNm}">value="*테이블 어근명*"</c:if><c:if test="${!empty aaForm.srchTableNm}">value="<c:out value="${aaForm.srchTableNm}"/>"</c:if>>
				...
				<table cellpadding=0 cellspacing=0 border=0 class="table_board">
					...
					<thead>
						<tr>
							<th ch="0" class="first">순번</th>
							<th ch="0"><span class="table_title">테이블 어근명</span></th>
							<th ch="0"><span class="table_title">데이터 건수</span></th>
							<th ch="0"><span class="table_title">최종수정일시</span></th>
						</tr>
					</thead>
					<tbody>
				...
				<div class="btnArea"> 
					<div class="rightArea">
						<a href="javascript:admBrdTblMngr.onBrdTblList('edit','ins')" class="btn_W"><span>추가</span></a>
					</div>
				</div>
				<!-- btnArea//-->
			</div>
		...
		<c:if test="${aaForm.view == 'edit'}">
			<div class="board first">
				<form name="frmBTE" method="post" action="" onsubmit="return false">
					...
					<div class="btnArea"> 
						<div class="rightArea">
							<a href="javascript:admBrdTblMngr.onBrdTblForm('list')" class="btn_B"><span>목록</span></a>
