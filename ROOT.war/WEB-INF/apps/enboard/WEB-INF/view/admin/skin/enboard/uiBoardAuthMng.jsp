							...
							<tbody>
								...
								<tr>
									<td class="L">
										<div>
											<c:forEach items="${actionRepList}" var="actionRep" varStatus="status">
												<input type=checkbox name="authRepGrcdR_<c:out value="${status.index}"/>" value="<c:out value="${actionRep.code}"/>" onclick="aBM.getAuthMngr().checkAuthRepGrcd(this,document.roleAuthForm)">
												<label>&nbsp;<c:out value="${actionRep.codeName}"/></label>&nbsp;&nbsp;
											</c:forEach>
										...
										<div id="authDetailR" style="display:none; padding: 10px 0;">
											<c:forEach items="${actionList}" var="action" varStatus="status">
												<input type=checkbox name="authGrcdR_<c:out value="${status.index}"/>" value="<c:out value="${action.code}"/>" onclick="aBM.getAuthMngr().checkAuthGrcd(this,document.roleAuthForm)">
												<label>&nbsp;<c:out value="${action.codeName}"/></label>&nbsp;&nbsp;
											</c:forEach>
							  		...
							  		<table id="grid-table" cellpadding="0" cellspacing="0" summary="게시판" class="table_board">
										...
										<tr height=30>
											<td class="L">
												<div>
													<c:forEach items="${actionRepList}" var="actionRep" varStatus="status">
														<input type=checkbox name="authRepGrcdG_<c:out value="${status.index}"/>" value="<c:out value="${actionRep.code}"/>" onclick="aBM.getAuthMngr().checkAuthRepGrcd(this,document.groupAuthForm)">
														<label>&nbsp;<c:out value="${actionRep.codeName}"/></label>&nbsp;&nbsp;
													</c:forEach>
												...
												<div id="authDetailG" style="display:none; padding: 10px 0;">
													<c:forEach items="${actionList}" var="action" varStatus="status">
														<input type=checkbox name="authGrcdG_<c:out value="${status.index}"/>" value="<c:out value="${action.code}"/>" onclick="aBM.getAuthMngr().checkAuthGrcd(this,document.groupAuthForm)">
														<label>&nbsp;<c:out value="${action.codeName}"/></label>&nbsp;&nbsp;
													</c:forEach>
													<!-- <input type=checkbox name="authGrcdR_11" value="2048" onclick="aBM.getAuthMngr().checkAuthGrcd(this,document.roleAuthForm)">
													<label>&nbsp;<util:message key="eb.prop.delete.text"/></label>&nbsp;&nbsp; -->
						...
						<table id="grid-table" cellpadding="0" cellspacing="0" summary="게시판" class="table_board">
							...
							<tbody>
			                	<tr>
			                		<td class="L">
			                			<img src='<%=evcp%>/board/images/admin/ic_triangle.gif' style='margin:0px 5px 0px 0px;' align=absmiddle>
			                			<utill:message key="eb.info.assignedAuthority.User"/> <%--현재 선택된 사용자에 부여된 권한입니다. --%>
			                		</td>
		                       	</tr>
								<tr>
								...
								<tr>
									<td class="L">
										<div>
											<c:forEach items="${actionRepList}" var="actionRep" varStatus="status">
												<input type=checkbox name="authRepGrcdU_<c:out value="${status.index}"/>" value="<c:out value="${actionRep.code}"/>" onclick="aBM.getAuthMngr().checkAuthRepGrcd(this,document.userAuthForm)">
												<label>&nbsp;<c:out value="${actionRep.codeName}"/></label>&nbsp;&nbsp;
											</c:forEach>
										...
										<div id="authDetailU" style="display:none; padding: 10px 0;">
											<c:forEach items="${actionList}" var="action" varStatus="status">
												<input type=checkbox name="authGrcdU_<c:out value="${status.index}"/>" value="<c:out value="${action.code}"/>" onclick="aBM.getAuthMngr().checkAuthGrcd(this,document.userAuthForm)">
												<label>&nbsp;<c:out value="${action.codeName}"/></label>&nbsp;&nbsp;
											</c:forEach>
											<input type=checkbox name="authGrcdU_11" value="2048" onclick="aBM.getAuthMngr().checkAuthGrcd(this,document.userAuthForm)">
											<label>&nbsp;<util:message key="eb.prop.delete.text"/></label>&nbsp;&nbsp;
										</div>
									</td>
								</tr>																		
							...
							<c:forEach items="${gradeList}" var="grade">
								<tr>
									...
									<td class="L">
										<div>
											<c:forEach items="${actionRepList}" var="actionRep" varStatus="status">
												<input type=checkbox name="authRepGrcd<c:out value="${grade.code}"/>_<c:out value="${status.index}"/>" value="<c:out value="${actionRep.code}"/>"
														 <%
										...
										<div id="authDetail<c:out value="${grade.code}"/>" style="display:none;border-top: 1px solid lightgray;padding: 5px">
											<c:forEach items="${actionList}" var="action" varStatus="status">
												...
												<input type=checkbox name="authGrcd<c:out value="${grade.code}"/>_<c:out value="${status.index}"/>" value="<c:out value="${action.code}"/>"
													 <%
													 CodebaseVO gcdVO	= (CodebaseVO)pageContext.getAttribute("grade");
											...
											<c:if test="${grade.code == 'X'}">
												<div style="display:inline-block;">
												<input type=checkbox name="authGrcdX_11" value="2048"
													 <%
													 CodebaseVO gcdVO = (CodebaseVO)pageContext.getAttribute("grade");
...
<c:if test="${aaForm.view == 'existList'}">
	<c:if test="${aaForm.act == 'role'}">
		<form id="gradeRoleMngForm" name="gradeRoleMngForm" onsubmit="return false">
			<table id="grid-table" cellpadding="0" cellspacing="0" summary="게시판" class="table_board">
				...
				<thead>
					<tr>
						...
						<th class="C"><span class="table_title">역할ID</span></th>
						<th class="C"><span class="table_title">역할명</span></th>
						<th class="C"><span class="table_title">역할설명</span></th>
					</tr>
				</thead>
				<tbody>
					<c:if test="${empty roleList  }">
						<tr>
							<td class="C" colspan="4">등록된 역할이 없습니다.</td>
						</tr>
					</c:if>
			...
			<div class="btnArea">
				<div class="rightArea">
					<a href="javascript:aBM.getAuthMngr().doChooseRGU('role')" class="btn_O"><span>신규</span></a>
					<a href="javascript:aBM.getAuthMngr().doRemRGU('role')" class="btn_O"><span>삭제</span></a>
				</div>
			</div>
			<!-- btnArea//-->
			</c:if>
		...
		<colgroup>
			...
			<col width="40%">
				...
				<thead>
					<tr>
						<th class="first">선택</th>
						<th class="C"><span class="table_title">그룹ID</span></th>
						<th class="C"><span class="table_title">그룹명</span></th>
						<th class="C"><span class="table_title">그룹설명</span></th>
					</tr>
				</thead>
	<tbody>
					<c:if test="${empty groupList  }">
						<tr>
							<td class="C" colspan="4">등록된 그룹이 없습니다.</td>
						</tr>
					</c:if>
			...
			<div class="btnArea">
				<div class="rightArea">
					<a href="javascript:aBM.getAuthMngr().doChooseRGU('group')" class="btn_O"><span>신규</span></a>
					<a href="javascript:aBM.getAuthMngr().doRemRGU('group')" class="btn_O"><span>삭제</span></a>
				</div>
			</div>
			<!-- btnArea//-->
			</c:if>
		...
		<form id="gradeUserMngForm" name="gradeUserMngForm" onsubmit="return false">
			<table id="grid-table" cellpadding="0" cellspacing="0" summary="게시판" class="table_board">
				...
				<thead>
					<tr>
						<th class="first">선택</th>
						<th class="C"><span class="table_title">사용자ID</span></th>
						<th class="C"><span class="table_title">사용자명</span></th>
					</tr>
				</thead>
				<tbody>
			...
			<c:if test="${sessionScope._enview_user_info_.hasAdminRole || sessionScope._enview_user_info_.hasManagerRole || sessionScope._user_domain_.domainId == boardVO.domainId}">
				...
				<div class="btnArea">
					<div class="rightArea">
						<a href="javascript:aBM.getAuthMngr().doChooseRGU('user')" class="btn_B"><span>신규</span></a>
						<a href="javascript:aBM.getAuthMngr().doRemRGU('user')" class="btn_B"><span>삭제</span></a>
					</div>
				</div>
				<!-- btnArea//-->
			</c:if>
			...
			<table id="grid-table" cellpadding="0" cellspacing="0" summary="게시판" class="table_board">
				...
				<thead>
					<tr style="cursor:pointer;">
						...
						<th class="C" ch="0"><span class="table_title">사용자 ID</span></th>
...
<c:if test="${aaForm.view == 'curAuth'}">
	...
	<input type=checkbox name="am<c:out value="${status.index}"/>" value="<c:out value="${action.code}"/>" disabled
		...
		%>
	><label for="am<c:out value="${status.index}"/>">&nbsp;글삭제</label>&nbsp;&nbsp;
	</c:if>
</c:if>
