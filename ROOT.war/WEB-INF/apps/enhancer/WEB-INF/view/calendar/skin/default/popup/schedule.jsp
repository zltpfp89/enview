...
<div id="schedulePops" class="scheduleEdit schedulePops">
	...
	<div id="scheduleDetailHeader" class="schedulePop header">
		<label class="scheduleName ellipsis" id="scheduleName" title="<c:out value="${scheduleLang.name }" escapeXml="false"/>"><c:out value="${scheduleLang.name }" escapeXml="false"/></label><div id="scheduleClose" class="escapeButton ui-icon ui-icon-closethick"></div>
	</div>
	<div id="scheduleDetailContent" class="schedulePop content">
		...
		<div id="scheduleDetailFooter" class="schedulePop footer">
			...
			<div class="scheduleControl">
				<c:if test="${ fn:indexOf(schedule.calPermissions, 'D') > -1 }">
					<div class="scheduleControlButton scheduleDelete" onclick="enCal.deleteSchedule();"><util:message key='hn.title.schedule.delete'/></div>
				</c:if>
				<c:if test="${ fn:indexOf(schedule.calPermissions, 'D') > -1 or schedule.isOwner}">
					<div class="scheduleControlButton scheduleModify" onclick="enCal.modifySchedule();"><util:message key='hn.title.schedule.modify'/></div>
				</c:if>
			</div>
		</div>
	</c:if>
