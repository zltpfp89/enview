...
<div id="scheduleDetails" class="mobile scheduleEdit scheduleDetails<c:if test="${calendar.bilingual && calendar.public }">Public</c:if>">
	...
	<div id="scheduleDetailHeader" class="mobile scheduleDetail<c:if test="${calendar.bilingual && calendar.public }">Public</c:if> header">
	...
	<div id="scheduleDetailContent1" class="mobile scheduleDetail content">
		...
		<c:if test="${calendar.bilingual && calendar.public }">
		...
		<div class="mobile scheduleDate">
			<div id="startDates" class="mobile startDates">
				<input type="text" class="mobile date" readonly="readonly" id="start" name="start" value="<c:out value="${schedule.startDate }"/>" <c:if test="${schedule.rrule != null && schedule.startDate != schedule.repeatStartDate }">disabled="disabled"</c:if>/>
			...
			<div id="endDates" class="mobile endDates">
				<input type="text" class="mobile date" readonly="readonly" id="end" name="end" value="<c:out value="${schedule.endDate }"/>" <c:if test="${schedule.rrule != null && schedule.startDate != schedule.repeatStartDate }">disabled="disabled"</c:if>/>
		...
		<div class="mobile scheduleAlarm">
		...
		<div class="mobile scheduleRrule">
		...
		<div class="mobile scheduleRruleFreq">
		...
		<div class="mobile scheduleRruleByday" id="scheduleRruleByday">
		...
		<div class="mobile scheduleRruleBymonthday" id="scheduleRruleBymonthday">
		...
		<div class="mobile scheduleRruleDate" id="scheduleRruleStartDate">
			<label class="valLabel">시작일:</label>
		...
		<div class="mobile scheduleRruleDate" id="scheduleRruleEndDate">
			<label class="valLabel">종료일:</label>
			<input type="radio" name="untilType" id="untilType_unlimited" value="unlimited" <c:if test="${schedule.until == null }">checked="checked"</c:if> /><label class="valLabel" for="untilType_unlimited">계속 반복</label>
			<input type="radio" name="untilType" id="untilType_limited" value="limited" <c:if test="${schedule.until != null }">checked="checked"</c:if>/><label class="valLabel" for="untilType_limited">반복 종료일:</label>
	...
	<%--
	<c:if test="${(!calendar.bilingual || !calendar.public) && ( schedule.isOwner || fn:indexOf(schedule.calPermissions, 'U') > -1 || fn:indexOf(schedule.permissions, 'U') > -1 || fn:indexOf(schedule.permissions, 'A') > -1 ) }">
		<div id="scheduleDetailContent2" class="mobile scheduleDetail contentR">
			<div id="scheduleUsers" class="mobile scheduleUsers">
			...
			<div id="attends" class="mobile attends">
		...
		<c:if test="${calendar.bilingual && calendar.public }">
			<c:forEach items="${scheduleLangList }" var="lang" varStatus="status">
				<div class="mobile schedulePlaces">
					<label class="mobile schedulePlaceLabel"><util:message key='hn.title.schedule.place'/></label><input class="mobile" type="text" langKnd="<c:out value="${lang.langKnd }"/>" name="placeList" value="<c:out value="${lang.place }"/>"/>
				</div>
				<div class="mobile scheduleComments">
					<label class="mobile scheduleCommentsLabel"><util:message key='hn.title.schedule.comments'/></label><textarea class="mobile" langKnd="<c:out value="${lang.langKnd }"/>" name="commentsList" cols="58" rows="3"><c:out value="${lang.comments }"/></textarea>
				</div>
			</c:forEach>
		</c:if>
		<c:if test="${ !calendar.bilingual || !calendar.public }">
			...
			<div class="mobile scheduleComments">
				<label class="mobile scheduleCommentsLabel"><util:message key='hn.title.schedule.comments'/></label><textarea class="mobile" name="comments" id="scheduleComments"cols="58" rows="3"><c:out value="${scheduleLang.comments }"/></textarea>
			</div>
		</c:if>
	</div>
	<%--
	...
	--%>
	<div id="scheduleDetailFooter" class="mobile scheduleDetail<c:if test="${calendar.bilingual && calendar.public }">Public</c:if> footer">
