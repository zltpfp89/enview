...
<div id="calendarDetails" style="width:380px;" class="calendarEdit calendarDetails<c:if test="${calendar.public }">Public</c:if>">
	...
	<div id="calendarDetailHeader" class="calendarDetail<c:if test="${calendar.public }">Public</c:if> header">
	...
	<div id="calendarDetailContent1" class="calendarDetail content">
		...
		<c:if test="${!calendar.public }">
			<div class="calendarName" id="calendarName" ><label class="calendarNameLabel" for="name"><util:message key="hn.info.title.name.title"/></label><input type="text" style="width:370px;" class="calendarNameInput" maxlength="100" id="name" name="name" value="<c:out value="${calendarLang.name }"/>"/></div>
		</c:if>
	</div>
	<c:if test="${form.cmd != 'MODIFY' || (!calendar.public && ( calendar.isOwner || fn:indexOf(calendar.permissions, 'U') > -1 || fn:indexOf(calendar.permissions, 'A') > -1 )) }">
		...
		<c:if test="${form.isPublic == 'Y' }">
			<c:forEach items="${calendarLangList }" var="lang" varStatus="status">
				<div class="calendarComments">
					<label class="calendarCommentsLabel"><util:message key='hn.title.calendar.comments'/></label><textarea style="width:370px;" langKnd="<c:out value="${lang.langKnd }"/>" name="commentsList" cols="58" rows="3"><c:out value="${lang.comments }"/></textarea>
				</div>
			</c:forEach>
		</c:if>
		<c:if test="${form.isPublic != 'Y' }">
			<div class="calendarComments">
				<label class="calendarCommentsLabel"><util:message key='hn.title.calendar.comments'/></label><textarea style="width:370px;" name="comments"  id="calendarComments"cols="58" rows="3"><c:out value="${calendarLang.comments }"/></textarea>
			</div>
		</c:if>
	</div>
