...
<div id="icalMenu" class="icalMenu">
	...
	<c:if test="${form.action == 'import' }">
		<div id="icalMenuContent" class="icalMenuContent">
			<div class="icalCalendarName">
				<label class="icalCalendarNameLabel"><util:message key='hn.title.export.calendarName'/></label>
				<label class="icsLink"></label>
			</div>
			<div class="icalICSBrowse">
				<form id="icsImportForm" name="icsImport" method="post" action="<%=request.getContextPath() %>/calendar/<c:out value="${form.calendarId}"/>/ics/importSchedule.hanc" enctype="multipart/form-data">
					<input type="text" id="importFileName" class="fileNameInput"/><input type="file" id="icsFile" name="multipartFile" class="fileSelect" onchange="enCal.checkeFileExtension(this);"/>
				</form>
				<input type="hidden" id="icalCalendarId" value="<c:out value="${form.calendarId}"/>"/>
			</div>
		</div>
	</c:if>
</div>