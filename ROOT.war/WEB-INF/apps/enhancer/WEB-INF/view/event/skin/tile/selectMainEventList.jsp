...
<!--
	...
	function initEventMain(){
		...
		if(doc.offsetHeight ==0) {
			...
			pageheight = doc.offsetHeight+400;
			
			
			parent.document.getElementById("body_sub").height = pageheight;
			alert(parent.document.getElementById("body_sub").height);
		}*/
	...
	//번호클릭시 주요이벤트 디스플레이 변경
 	function changemainEvent(idx,changeType){
		...
		for (var i = 1; i < eval(mainEvLen)+1; i++) {
			...
			document.getElementById("eventNum"+i).src = resPath+"/images/eventCommon/mng/event/event_num0"+i+".gif";
		}
		document.getElementById("mainEvent"+idx).style.display = "block";
		document.getElementById("eventNum"+idx).src = resPath+"/images/eventCommon/mng/event/event_num0"+idx+"_on.gif";
	}
	...
	function moreNewEvent(n){
		...
		document.getElementById("eventListFrom").action = actPath+"/selectMainEventList.hanc";
	...
	//SNS링크 공유
	function shareSnsLink(type, idx, linkUrl){ 
		var url = "";
		var width = 1000;
		var hdight = 0;
	...
	//구글켈린더에 일정추가 (Url정보 사이트 마다 변경)
	function addSchedule(seq){
		var url = '';
		var siteUrl ='http://portal.saltware.co.kr';
		if('<%=token%>'==null || '<%=token%>'=='null' || '<%=token%>'=='' || '<%=token%>'=='undefined')
			url = 'https://www.google.com/accounts/AuthSubRequest?next=' + url + '/event/selectGoogleAddEvent.hanc?EVENT_SEQ='+seq+'&scope=http://www.google.com/calendar/feeds/&secure=0&session=1';
		if('<%=token%>'!=null && '<%=token%>'!='null' && '<%=token%>'!='' && '<%=token%>'!='undefined')
					...
					<div class="content">
						...
						<c:forEach  items="${mainEventList}" var="data" varStatus="status">
							<dl id="mainEvent<c:out value="${status.count}"/>" style="display: none;">
								...
								<dd class="date"><c:out value="${data.START_DATE}"/> ~ <c:out value="${data.END_DATE}"/> | <c:out value="${data.START_TIME}"/> ~ <c:out value="${data.END_TIME}"/></dd>
