...
<form id="receivebox" name="receivebox" method="post" onsubmit="return false;">
	...
	<div class="searchForm">
		<div class="work"><util:message key="hn.note.label.receiveBox"/></div>
		<div class="searchButton" onclick="javascript:enNote.searchReceiveNotes(event);"><util:message key="hn.note.label.search"/></div>
		<input type="text" name="searchValue" id="searchValue" class="inputField searchValue" onkeyup="javascript:enNote.searchReceiveNotes(event);">
	...
	<div class="message">
		...
		<div class="unreadNoteMessage"><c:out value="${form.unreadMessage }"/></div>
	</div>
	<div class="controlPanel">
		<div title="delete" class="noteButton deleteButton" onclick="javascript:enNote.deleteReceiveNotes();"><util:message key="hn.note.label.delete"/></div>
		<div title="store" class="noteButton storeButton" onclick="javascript:enNote.storeReceiveNotes();"><util:message key="hn.note.label.store"/></div>
	...
	<div class="columnLayer">
		...
		<div class="senderName"><util:message key="hn.note.label.sender"/></div>
...
<c:if test="${empty form.noteFormList}">
	<div class="noNoteList"><util:message key="hn.note.label.hasNotReceiveNote"/></div>
</c:if>
<c:forEach items="${form.noteFormList}" var="note" varStatus="idx">
		<div class="noteRow row">
			<div class="check column"><input type="checkbox" id="<c:out value="${note.noteId}"/>" name="check" class="checkbox"/></div>
			<div class="senderName column" id="send_<c:out value="${note.noteId }"/>"><a class="senderLink column <c:if test="${note.isBlocked == 1}">blocked</c:if>" href="javascript:enNote.popupMenu(<c:out value="${note.noteId }"/>,'<c:out value="${note.senderName}"/>','<c:out value="${note.senderId}"/>',<c:out value="${ note.isBlocked }"/>, this);"><c:out value="${note.senderName}"/>(<c:out value="${note.senderId}"/>)</a></div>
			<div class="note_title column2"><a class="noteLink<c:if test="${note.states == 0 }"> unread</c:if>" href="javascript:enNote.readReceiveNote('<c:out value="${note.noteId }"/>');"><c:out value="${note.title}"/></a>
