	...
	<form id="calendarForm" name="calendarForm" method="post" action="<%=request.getContextPath()%>/hancer/calendar/detailCalendar.hanc">
		...
		<table id="grid-table" cellpadding="0" cellspacing="0" summary="게시판" class="table_board">
			...
			<c:if test="${isSuperAdmin}">
				<tr>
					<th class="L">도메인 </th>
					<td colspan="3" class="L"><input type="hidden" id="calendarDomainId" name="domainId" value="<c:out value="${calendar.domainId}"/>"/><c:out value="${calendar.domainNm}"/></td>
				</tr>
			</c:if>
			...
			<tr>
				...
				<td colspan="3" class="L">
					<input type="radio" style="cursor:pointer;" disabled="disabled" id="isBilingualY" name="isBilingual" value="1"<c:if test="${calendar.bilingual}">checked="checked"</c:if>/>
					<label for="isBilingualY" style="cursor:pointer;"><util:message key="hn.info.calendar.isBilingual.1"/></label>
					<input type="radio" style="cursor:pointer;" disabled="disabled" d="isBilingualN" name="isBilingual" value="0"<c:if test="${!calendar.bilingual}">checked="checked"</c:if>/>
			...
			<tr>
				<th class="L">사용여부 </th>
				<td colspan="3" class="L">
					<input type="radio" style="cursor:pointer;" disabled="disabled" id="isUseY" name="isUse" value="1"<c:if test="${calendar.use}">checked="checked"</c:if>/>
					<label for="isUseY" style="cursor:pointer;"><util:message key="hn.info.calendar.isUse.1"/></label>
					<input type="radio" style="cursor:pointer;" disabled="disabled" id="isUseN" name="isUse" value="0"<c:if test="${!calendar.use}">checked="checked"</c:if>/>
