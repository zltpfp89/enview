...
<script type="text/javascript" src="<%=request.getContextPath()%>/admin/javascript/gpsManager.js"></script>
<script type="text/javascript" src="<%=request.getContextPath()%>/admin/javascript/gpsManagerGoogleAPI.js"></script>
...
$(function()
		{
			$("#dialog").dialog(
				   {width:370,
				   height:545,position: ["right","bottom"],});
		});
			...
			<div id="UserManager_UserTabPage">
				...
				<div>
					<form id="UserManager_SearchForm" name="UserManager_SearchForm" style="display:inline" action="javascript:aUserManager.doSearch('UserManager_SearchForm')" onkeydown="if(event.keyCode==13) aUserManager.doSearch('UserManager_SearchForm')" method="post">
					  ...
					  <tr>
						<td align="right">
						<input type="text" id="search" name="principalInfo02Cond" size="15" class="webtextfield" value="" onfocus="whenSrchFocus(this,'*<util:message key='en.title.gpsManager.extraUserId'/>*');" onblur="whenSrchBlur(this,'*<util:message key='en.title.gpsManager.extraUserId'/>*');"/> 
	 ...
	 --%>					</td>
				...
				<div class="webgridpanel">
				  ...
				  <table id="grid-table" width="100%" border="0" cellspacing="0" cellpadding="0">
					...
					<c:forEach items="${results}" var="gpsManager" varStatus="status">
						<tr ch="<c:out value="${status.index}"/>" style="cursor:pointer;" class="<c:choose><c:when test="${status.index%2 == 0}">even</c:when><c:otherwise>odd</c:otherwise></c:choose>" onmouseover="whenListMouseOver(this)" onmouseout="whenListMouseOut(this)">
							<td align="center" class="webgridbody">
							    ...
							    <input type="hidden" id="UserManager[<c:out value="${status.index}"/>].userId" value="<c:out value='${gpsManager.userId}'/>"/>
							    <input type="hidden" id="UserManager[<c:out value="${status.index}"/>].latiTude" value="<c:out value='${gpsManager.latiTude}'/>"/>
							    <input type="hidden" id="UserManager[<c:out value="${status.index}"/>].longiTude" value="<c:out value='${gpsManager.longiTude}'/>"/>
							    <input type="hidden" id="UserManager[<c:out value="${status.index}"/>].addRess" value="<c:out value='${gpsManager.addRess}'/>"/>
							</td>
							...
							<td align="left" class="webgridbody" onclick="aUserManager.doSelect(this)">
								<span class="webgridrowlabel" id="UserManager[<c:out value="${status.index}"/>].userId.label">&nbsp;<c:out value="${gpsManager.userId}"/></span>
							</td>
									<td align="left" class="webgridbody" onclick="aUserManager.doSelect(this)">
								<span class="webgridrowlabel" id="UserManager[<c:out value="${status.index}"/>].userId.label">&nbsp;<c:out value="${gpsManager.latiTude}"/></span>
							</td>
									<td align="left" class="webgridbody" onclick="aUserManager.doSelect(this)">
								<span class="webgridrowlabel" id="UserManager[<c:out value="${status.index}"/>].userId.label">&nbsp;<c:out value="${gpsManager.longiTude}"/></span>
							</td>
									<td align="left" class="webgridbody" onclick="aUserManager.doSelect(this)">
								<span class="webgridrowlabel" id="UserManager[<c:out value="${status.index}"/>].userId.label">&nbsp;<c:out value="${gpsManager.addRess}"/></span>
							</td>
						</tr>
					</c:forEach>
					...
					<!-- 		<input type="button" class="btn_pack small"  size="1" value=" 등록  " onClick="aUserManager.doRemove()"/>
					 ...
					 -->		
						<div id="dialog" title="Map">
					             <input type="hidden" readOnly="readOnly" id="gps_userId"/>
					             <input type="hidden" id="gps_latiTude" />
					             <input type="hidden" id="gps_longiTude"/>
					             <br>
					             <div id="map" style="width:340px;height:480px;">
			            </div>
				   </table>
			</div>
