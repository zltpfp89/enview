...
<%
	...
	if( currentPath.indexOf("/sjpb/") != -1) { 
		pageContext.setAttribute("PageTitle", pageContext.getAttribute("PageTitle") + " | 수사지원시스템");
	}
...
<html lang="ko-KR">
	<head>
		...
		<link id="sjpbMainCss" rel="stylesheet" type="text/css" href="${cPath}/fss/css/main.css" />
		<link rel="stylesheet" type="text/css" href="${cPath}/fss/js/Z/dhtmlx/suite/dhtmlx.css" />
		...
		<script type="text/javascript" src="${cPath}/fss/js/Z/jquery.js"></script>
		<script type="text/javascript" src="${cPath}/fss/js/Z/main.js"></script>
		<script type="text/javascript" src="${cPath}/fss/js/Z/dhtmlx/suite/dhtmlx.js"></script>
		<script type="text/javascript" src="${cPath}/fss/js/Z/dhtmlx/suite/thirdparty/excanvas/excanvas.js"></script>
		...
		$( document ).ready(function(){
			...
			$(document).on("click",".lmenuOpen",function(event){
                ...
                var rel = $(link).attr("rel").toString();
                var id =  "ocrt_list_tab" + $(link).attr("id"); // 맨하위이름
                var data_root = $(link).attr("data-root"); // 메뉴 비교값
                if( data_root != null && data_root =='tm006'){
                	window.open("http://sup1.fss.or.kr:9000/fss/html/cm/login/login_SP.jsp");
                	return;
                }
                ...
                if(rel.indexOf(".page") < 0){
	                ...
	                if($("#sub_tab").is("."+data_tab) == false){//처음 메뉴 들어갈때
	                	$("head link#sjpbMainCss").attr("href", '${cPath}/fss/css/contents.css');
...
//			 if(contentHeight <= 685) contentHeight = 750;
			 if(contentHeight <= 685) contentHeight = 610;
...
<div id="header">
		...
		<div class="gnb">
			<div class="wrap">
				<h1><a href="${cPath}/portal" title="금감원 수사정보 포털시스템"><img src="${cPath}/fss/images/logo.png" alt="금융감독원 로고" /></a></h1>
			<ul>
			<c:forEach items="${topMenuList}" var="menu" varStatus="status">
				<c:if test="${!menu.hidden && menu.name != menu2hide1 && menu.name != menu2hide2}">
				<c:if test="${status.index != 0}">
					<c:if test="${menu.name == 'mypage'}"><li class="gnb_title" style="visibility: hidden;"></c:if>
					<c:if test="${menu.name != 'mypage'}"><li class="gnb_title"></c:if>
						<c:set var="lmenu" value="${menu.elements[0]}" />
						<c:set var="subMenu" value="${menu.elements[0].elements[0]}" />
						<c:if test="${not empty subMenu}">
							<a href="javascript:void(0);" class="lmenuOpen" id="${menu.name}_${lmenu.name}_${subMenu.name }" data-root="${menu.name }" data-title="${subMenu.shortTitle}" rel="${subMenu.menuUrl}" rel2="${subMenu.menuType}" target="${subMenu.target}">&nbsp;<c:out value="${menu.shortTitle}"/></a></li>
						</c:if>
						<c:if test="${empty subMenu}">
							<a href="javascript:void(0);" class="lmenuOpen" id="${menu.name }_${lmenu.name }" data-root="${menu.name }" data-title="${lmenu.shortTitle}" rel="${lmenu.menuUrl}" rel2="${lmenu.menuType}"  target="<c:out value="${lmenu.target}"/>"> <c:out value="${menu.shortTitle}"/></a></li>
						</c:if>
					<%--
					<c:if test="${menu.name == 'mypage'&& menu.name != 'tm006'}"><li class="gnb_title" style="visibility: hidden;"><a href="javascript:void(0);">${menu.shortTitle}</a></c:if>
						<c:if test="${not empty menu.elements }">
					        <div class="depth_2">
					            <div class="gnb_wrap">
				            		<div class="title_01"/>
				            			<c:if test="${menu.name != 'mypage'}"><h5>${menu.shortTitle}<br/></h5></c:if>
				            			<c:if test="${menu.name == 'mypage'}"><h5 style="visibility: hidden;">${menu.shortTitle}<br/></h5></c:if>
				            		</div>
											...
											<c:if test="${lmenu.hidden=='false'}">		
												<c:if test="${not empty lmenu.elements}">
													<li class="depth_1"><a href="javascript:void(0);" target="<c:out value="${lmenu.target}"/>"> <c:out value="${lmenu.shortTitle}"/></a>
														<ul class="depth_03" style="float:left;">
												...
												<c:if test="${empty lmenu.elements}">
													<c:if test="${lmenu.name!='myPage'}"><li class="depth_1"><a href="javascript:void(0);" class="lmenuOpen" id="${menu.name }_${lmenu.name }" data-root="${menu.name }" data-title="${lmenu.shortTitle}" rel="${lmenu.menuUrl}" rel2="${lmenu.menuType}"  target="<c:out value="${lmenu.target}"/>"> <c:out value="${lmenu.shortTitle}"/></a></li></c:if>
													<c:if test="${lmenu.name=='myPage'}"><li class="depth_1" style="visibility: hidden;"><a href="javascript:void(0);" class="lmenuOpen" id="${menu.name }_${lmenu.name }" data-root="${menu.name }" data-title="${lmenu.shortTitle}" rel="${lmenu.menuUrl}" rel2="${lmenu.menuType}"  target="<c:out value="${lmenu.target}"/>"> <c:out value="${lmenu.shortTitle}"/></a></li></c:if>
												</c:if>
											</c:if>
										</c:forEach>
									</ul>
			...
			<div class="gnb_btnArea">
				<a href="#" class="mypage" onclick="javascript:$('#mypage_myPage').click();"><span>마이페이지</span></a>
		...
		<div id="body">
								...
								<c:if test="${lmenu.hidden=='false'}">		
									<c:if test="${not empty lmenu.elements}">
										<li class="has-sub"><a href="<c:out value="${lmenu.fullUrl}"/>" class="click off_arr on_arr" target="<c:out value="${lmenu.target}"/>"> <c:out value="${lmenu.shortTitle}"/></a>  <%-- 입건실적 --%>
											<ul id="documents" style="width: 170px;">
												<c:forEach items="${lmenu.elements}" var="subMenu" varStatus="Substatus">
												<c:if test="${subMenu.hidden=='false'}">
													<li class="has-sub"><a href="javascript:void(0);" id="${menu.name}_${lmenu.name}_${subMenu.name}" data-root="${menu.name }" data-title="${subMenu.shortTitle}" data-tab="tab" rel="${subMenu.menuUrl}"  class="click lmenuOpen"  target="${subMenu.target}">&nbsp;<c:out value="${subMenu.shortTitle}"/></a></li>
												</c:if>
												</c:forEach>
											</ul>
										</li>
			...
			<div id="main" class="contents">
				<!-- contents_warp -->
				<div class="wrap all_contente" id="EnviewContentArea">
