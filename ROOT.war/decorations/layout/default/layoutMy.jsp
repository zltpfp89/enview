...
//		System.out.println("############################# isRoot=" + request.getAttribute("isRoot"));
		...
		if( (layoutFragment.getActionMask() & 0x00000001) == 0x00000001 ) {
			columnClass = "column-" + classSuffixMypage;
		}
		else {
			portletClass = portletClass + "-none";
				...
				for(int rowIndex=0; it2.hasNext(); rowIndex++) {
					...
					if( "portlet".equals(f.getType()) ) {
						...
						if( content.length() > 0 ) { // 20120224 Modified
							...
							if( style != null ) { %>
								<div id="Enview_Portlet_<%=f.getId()%>" class="<%=portletClass%>" style="<%=style%>">
<%		    		    	} else { %>
								<div id="Enview_Portlet_<%=f.getId()%>" class="<%=portletClass%>">
<%		    		    	} %>
						    <% if( f.getPreferenceValue("MORE_SRC") == null ) { %>
							<div class="<%=portletHeaderClass%>"><%=_pTitle %></div>
						    <% } else  { %>
							<div class="<%=portletHeaderClass%>">
							<span class='ui-icon ui-icon-plus' more_src="<%=f.getPreferenceValue("MORE_SRC")%>" more_src_target="<%=f.getPreferenceValue("MORE_SRC_TARGET")%>" more_src_width="<%=f.getPreferenceValue("MORE_SRC_WIDTH")%>" more_src_height="<%=f.getPreferenceValue("MORE_SRC_HEIGHT")%>"></span>
							<%=_pTitle %>
							</div>
<%		    		        } %>
<%		    		    			
