		...
		<div id="Enview_Layout_<%=layoutFragment.getId()%>" class="portal-nested-layout">
			<div id='LayoutEditIconArea_<%=layoutFragment.getId()%>' class='root-icon-area' style='display:none;'>
				<a class='portlet-action-button' onclick="javascript:portalPage.showPortletEditor('Enview_Layout_<%=layoutFragment.getId()%>');">
					<img src='<%=request.getContextPath()%>/decorations/layout/images/edit.gif' border='0' align='absmiddle' alt='<util:message key="ev.info.icon.editPortlet"/>' title='<util:message key="ev.info.icon.editPortlet"/>' style='border:none; cursor:hand;'/> 
				</a>
				<a class='portlet-action-button' onclick="javascript:portalPage.showPortletSelector('Enview_Layout_<%=layoutFragment.getId()%>');" >
					<img src='<%=request.getContextPath()%>/decorations/layout/images/selectPortlet.png' border='0' align='absmiddle' alt='<util:message key="ev.info.icon.addPortlet"/>' title='<util:message key="ev.info.icon.addPortlet"/>' style='border:none; cursor:hand;'/> 
				</a>
			</div>
<%		} else { %>
		<div id="Enview_Layout_<%=layoutFragment.getId()%>" class="portal-layout">
			<div id='LayoutEditIconArea_<%=layoutFragment.getId()%>' class='root-icon-area' style='display:none;'>
				<img id='zzzzzzz' src='<%=request.getContextPath()%>/decorations/layout/images/move.png' border='0' align='absmiddle' alt='<util:message key="ev.info.icon.movePortlet"/>'  title='<util:message key="ev.info.icon.movePortlet"/>' style='border:none; cursor:hand;'/>
				<a class='portlet-action-button' onclick="javascript:portalPage.showPortletEditor('Enview_Layout_<%=layoutFragment.getId()%>');">
					<img src='<%=request.getContextPath()%>/decorations/layout/images/edit.gif' border='0' align='absmiddle' alt='<util:message key="ev.info.icon.editPortlet"/>' title='<util:message key="ev.info.icon.editPortlet"/>' style='border:none; cursor:hand;'/> 
				</a>
				<a class='portlet-action-button' onclick="javascript:portalPage.showPortletSelector('Enview_Layout_<%=layoutFragment.getId()%>');" >
				...
				<a class='portlet-action-button' onclick="javascript:portalPage.removePortlet('Enview_Layout_<%=layoutFragment.getId()%>');" >
					<img src='<%=request.getContextPath()%>/decorations/layout/images/icon_del.gif' border='0' align='absmiddle' alt='<util:message key="ev.info.icon.removePortlet"/>' title='<util:message key="ev.info.icon.removePortlet"/>' style='border:none; cursor:hand;'/>
				</a>
			</div>
<%    	}
				...
				for(int rowIndex=0; it2.hasNext(); rowIndex++) {
					...
					if( "portlet".equals(f.getType()) ) {
						...
						if( content.length() > 0 ) { // 20120224 Modified
							...
							if( style != null ) { %>
								<div id="Enview_Portlet_<%=f.getId()%>"  style="<%=style%>">
<%		    		    	}
							else { %>
								<div id="Enview_Portlet_<%=f.getId()%>" >
<%		    		    	} 
