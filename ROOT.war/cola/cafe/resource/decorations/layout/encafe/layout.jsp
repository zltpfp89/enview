...
<%
	if( "portlet".equals(f.getType()) ) {
			...
			String content = "";
			if( f.getOverriddenContent() != null ) {
...
<%
	...
	String columnWidth = fragmentLayout.getColumnWidth(columnIndex);
%>	    		    				
					<div id="Enview_Column_<%=layoutFragment.getId()%>_<%=columnIndex%>" class="column" style="float:<%=columnFloat%>; width:<%=columnWidth%>; min-height:1px;">
<%
	Iterator it2 = column.iterator();
	for(int rowIndex=0; it2.hasNext(); rowIndex++) {
		...
		if( "portlet".equals(f.getType()) ) {
			...
			PortletDefinitionComposite portlet = f.getPortlet();
			if( portlet == null ) {
